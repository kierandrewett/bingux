...
def catalogue(text):
    ...
    for line in text.splitlines():
        if line.startswith('# group: '):
            ...
            if match:
                result.append({'emoji': ''.join(chr(int(code, 16)) for code in codes.split()),
                               'name': match[1], 'group': group, 'keywords': subgroup.replace('-', ' ')})
    return result
...
if __name__ == '__main__':
    ...
    destination.write_text(json.dumps({'version': '17.0', 'license': 'Unicode-3.0', 'emoji': rows},
