...
function encode(entries, session) {
    return JSON.stringify({version: 1, entries: entries.map(entry => ({
        key: entry.historyKey || session + ":" + entry.notification.id,
        session: session, sourceId: entry.notification.id,
        receivedAt: entry.receivedAt, appName: entry.appName,
        desktopEntry: entry.desktopEntry, appIcon: entry.appIcon,
...
function restore(text, current, session) {
    ...
    for (const record of data.entries) {
        ...
        const notification = {id: id, appName: record.appName || "", desktopEntry: record.desktopEntry || "",
            ...
            expireTimeout: 0, lastGeneration: true, dismiss: function() {}};
        entries.push({notification: notification, historyKey: record.key, restoredFromDisk: true,
            appName: notification.appName, desktopEntry: notification.desktopEntry, appIcon: notification.appIcon,
            summary: notification.summary, body: notification.body, image: record.image || "",
            receivedAt: Number(record.receivedAt) || 0, toastVisible: false, timeoutMs: 0,
            deadline: 0, remainingMs: 0, paused: false, actions: []});
    }
    return entries.sort((a, b) => b.receivedAt - a.receivedAt);
}
function largeImage(entry) {
    return /\b(screenshot|screen capture|screen recording)\b/i.test((entry.appName || "") + " " + (entry.summary || ""))
