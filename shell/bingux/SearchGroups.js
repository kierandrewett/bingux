...
function key(result) {
    if (result.providerId === "web" || result.providerId === "web-suggestions" || result.providerId === "web-shortcuts")
        return "web";
...
function ordered(results) {
    ...
    return results
        .map((result, index) => ({ result: result, index: index }))
        .sort((left, right) => {
            const a = key(left.result),
                b = key(right.result);
