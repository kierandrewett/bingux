...
PanelWindow {
    ...
    Timer {
        id: startupSettle
        interval: 150
        onTriggered: if (BinguxPreferences.loaded)
            root.finishStartup()
    }
    Timer {
        id: startupDeadline
    ...
    readonly property int previewPinnedCount: {
        ...
        return source < 0 || dropIndex < 0 ? pinnedGroupCount : pinnedGroupCount + Number(dropPinned) - Number(isPinned(appGroups[source]));
    }
    ...
    function snapshotLayout() {
        return {
            pinnedApps: pinnedApps.slice(),
            order: appOrder.slice(),
            applications: appGroups.filter(group => isPinned(group)).map(group => ({
                        id: group.id,
                        desktopId: group.desktopEntry ? group.desktopEntry.id : group.id
                    }))
        };
    }
    ...
    readonly property var pinnedApps: {
        if (preferences.dockApps)
            return preferences.dockApps.pinnedApps;
        const removed = dockState.unpinnedApps.map(id => pinIdentity(id));
        return [...new Set(settings.pinnedApps.concat(dockState.pinnedApps).map(id => pinIdentity(id)))].filter(id => id.length > 0 && removed.indexOf(id) < 0);
    }
    readonly property var appOrder: preferences.dockApps ? preferences.dockApps.order : dockState.order
    onAppOrderChanged: if (appGroupsInitialised)
        refreshAppGroups()
    ...
    function updatePinPreference(group, pinned) {
        ...
        if (pinned)
            dockState.pinnedApps = dockState.pinnedApps.concat([id]);
        else
            dockState.unpinnedApps = dockState.unpinnedApps.concat([id]);
    }
    ...
    function failLaunch(id, serial, message) {
        ...
        if (group && group.windows.some(window => !attempt.windows.includes(window)))
            return;
        launchFailures = Object.assign({}, launchFailures, {
            [id]: {
                message: message,
    ...
    function beginExternalLaunch(appId, name) {
        ...
        root.launchAttempts = Object.assign({}, root.launchAttempts, {
            [groupId]: {
                serial: serial,
                name: name || entry.name || groupId,
                windows: windows,
                deadline: Date.now() + Theme.launchTimeout,
                reportFailure: false
            }
        });
    ...
    Timer {
        ...
        onTriggered: {
            ...
            for (const id of Object.keys(root.launchFailures)) {
                ...
                const group = root.appGroups.find(item => item.id === id);
                if (group && group.windows.some(window => !failure.windows.includes(window)))
                    root.clearLaunchFailure(id);
            }
        }
    }
    ...
    function desktopEntryFor(appId) {
        ...
        const entry = DesktopEntries.byId(appId) || DesktopEntries.byId(appId + ".desktop") || DesktopEntries.heuristicLookup(appId) || null;
    ...
    function refreshAppGroupsNow() {
        ...
        const observedOrder = root.observedGroupOrder.length > 0 ? root.observedGroupOrder.slice() : previousGroups.map(group => group.id);
        ...
        for (const group of groups) {
            ...
            if (previousGroup) {
                ...
                group.windows.sort((a, b) => {
                    ...
                    return (ai < 0 ? previousWindows.length : ai) - (bi < 0 ? previousWindows.length : bi) || discoveredWindows.indexOf(a) - discoveredWindows.indexOf(b);
                });
            }
    ...
    Component {
        ...
        Process {
            ...
            onExited: (code, status) => {
                if (code !== 0)
                    root.failLaunch(groupId, serial, diagnostic.trim() || "The application launch service failed.");
    ...
    function launch(group, newWindow) {
        ...
        root.launchAttempts = Object.assign({}, root.launchAttempts, {
            [group.id]: {
                serial: serial,
                name: group.desktopEntry?.name || group.id,
                windows: (group.windows || []).slice(),
                deadline: Date.now() + Theme.dockLaunchTimeout
            }
        });
        ...
        root.launchFeedbackToken = LaunchFeedback.begin(group.id, () => {
            ...
            const process = applicationLaunchProcess.createObject(root, {
                command: command,
                groupId: group.id,
                serial: serial
            });
    ...
    Rectangle {
        ...
        NativeEditSurface {
            ...
            entries: {
                ...
                for (let i = 0; i < dockItems.count; i++) {
                    ...
                    if (item)
                        result.push({
                            id: "app:" + root.pinIdentity(item.currentGroup.desktopEntry?.id || item.currentGroup.id),
                            item
                        });
                }
                return result.concat(Object.keys(DesktopEditing.sources).filter(id => root.preferences.layout?.dock.includes(id)).map(id => ({
                            id,
                            item: DesktopEditing.sources[id]
                        })));
            }
        }
        ...
        Flickable {
            ...
            Rectangle {
                ...
                x: (root.draggedId ? root.previewPinnedCount * root.itemSize : dockRow.sectionWidths.width) + Math.max(0, root.previewPinnedCount - 1) * dockRow.spacing + (dockRow.sectionGap + dockRow.spacing) / 2 - width / 2
            ...
            RowLayout {
                ...
                readonly property size sectionWidths: {
                    ...
                    for (let index = 0; index < dockItems.count; index++) {
                        ...
                        if (!item)
                            continue;
                        if (index < root.pinnedGroupCount)
                            pinned += item.transitionProgress;
                        else
                            running += item.transitionProgress;
                    }
                ...
                Repeater {
                    ...
                    model: ScriptModel {
                        ...
                        values: root.appGroups.map(group => ({
                                    id: group.id
                                }))
                        objectProp: "id"
                    }
                    ...
                    delegate: Item {
                        ...
                        function publishRectangle() {
                            ...
                            const rect = root.visible ? Qt.rect(Math.round(position.x), Math.round(position.y), dockIcon.width, dockIcon.height) : Qt.rect(0, 0, 0, 0);
                        ...
                        transform: [
                            Translate {
                                x: root.draggedId === dockButton.currentGroup.id ? (root.settlingDrag && root.settlingId === dockButton.currentGroup.id ? root.settleOffset : root.dragOffset) : 0
                            },
                            ReorderSlide {
                                id: reorderTransform
                        ...
                        Item {
                            ...
                            DockBadge {
                                ...
                                shown: root.draggedId === dockButton.currentGroup.id && root.dropPinned && !root.isPinned(dockButton.currentGroup)
                                ...
                                if (mouse.button === Qt.LeftButton) {
                                    ...
                                    if (action === "launch")
                                        ...
                                        if (window) {
                                            ...
                                            root.launch(dockButton.currentGroup, false);
                                    } else
                                    ...
                                    if (action === "launch")
                                        ...
                                        for (const window of dockButton.currentGroup.windows)
                                            window.close();
                        ...
                        ShellPopup {
                            ...
                            Rectangle {
                                ...
                                ColumnLayout {
                                    ...
                                    MenuSeparator {
                                        visible: appMenu.mediaPlayers.length > 0
                                    }
                                    ...
                                    MenuSeparator {
                                        visible: notificationPreview.visible
                                        implicitHeight: Theme.gap * notificationPreview.presence
                                    }
                                    ...
                                    Repeater {
                                        ...
                                        model: dockButton.currentGroup.windows.length > 0 && dockButton.currentGroup.desktopEntry ? dockButton.currentGroup.desktopEntry.actions : []
    ...
    component MenuAction: ActionButton {
        ...
        Timer {
            id: closeRetry
            interval: 10000
            onTriggered: action.closing = false
        }
            ...
            RowLayout {
                ...
                ActionButton {
                    ...
                    background: Rectangle {
                        ...
                        color: closeWindow.down ? Qt.lighter(Theme.pressed, 1.2) : closeWindow.hovered ? Theme.pressed : "transparent"
