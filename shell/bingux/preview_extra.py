...
def frontmatter(text):
    ...
    try:
        ...
        if not isinstance(data, dict): raise ValueError('Expected a frontmatter mapping')
        facts = {}
        for key, value in list(data.items())[:32]:
            rendered = ', '.join(str(item) for item in value) if isinstance(value, list) and all(not isinstance(item, (dict, list)) for item in value) else json.dumps(value, ensure_ascii=False, default=str) if isinstance(value, (dict, list)) else str(value)
            facts[str(key)[:80]] = rendered[:512]
        return ''.join(lines[end + 1:]), facts, ''
    except (ValueError, TypeError, RecursionError, ImportError) as error:
...
def table(headers, rows):
    ...
    return '<table cellspacing="8"><tr>' + ''.join(cell(h, 'th') for h in headers) + '</tr>' + ''.join('<tr>' + ''.join(cell(v) for v in row) + '</tr>' for row in rows) + '</table>'
...
def preview(path, sanitise):
    ...
    if suffix in ('.csv', '.tsv'):
        ...
        reader = csv.reader(io.StringIO(data[:LIMIT].decode('utf-8-sig', errors='replace')), delimiter='\t' if suffix == '.tsv' else ',')
        rows = []
        for row in reader:
            rows.append(row[:30])
            if len(rows) >= 101: break
        return {'kind': 'text', 'format': 'table', 'text': '', 'renderedText': table(rows[0] if rows else [], rows[1:]), 'truncated': len(data) > LIMIT or len(rows) >= 101}
    if suffix == '.eml':
        with path.open('rb') as stream: data = stream.read(LIMIT + 1)
        ...
        rendered = sanitise(content, 'html', path) if body and body.get_content_type() == 'text/html' else '<pre>' + escape(content) + '</pre>'
        return {'kind': 'text', 'format': 'email', 'text': content, 'renderedText': rendered, 'frontmatter': facts, 'truncated': len(data) > LIMIT}
    rows = []
    if suffix in ('.zip', '.epub'):
        with zipfile.ZipFile(path) as archive:
            entries = archive.infolist()
            for item in entries[:200]: rows.append([item.filename, f'{item.file_size:,} B'])
            count = len(entries)
    else:
        ...
        count = len(rows)
    return {'kind': 'text', 'format': 'archive', 'text': '', 'renderedText': table(['Name', 'Size'], rows), 'truncated': count >= 200,
