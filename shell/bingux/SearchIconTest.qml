...
PanelWindow {
    ...
    OsIconImage {
        ...
        width: 128
        height: 128
        source: row.activationIconSource
    }
    Timer {
        interval: 100
        running: true
    ...
    Timer {
        ...
        onTriggered: {
            if (!preview.captured) {
                console.error("FAIL: native icon capture failed");
                Qt.exit(1);
            } else {
                console.warn("PASS: rendered and captured the OS icon through the real search row");
                Qt.quit();
            }
        }
    }
