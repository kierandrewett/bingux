...
ShellPopup {
    ...
    function focusCalendar() { grid.forceActiveFocus(); }
    ...
    function changePeriod(level, year) {
        ...
        viewLevel = level;
        browseYear = year;
        if (!visible || preparing || Theme.reducedMotion) periodProgress = 1;
        else { periodProgress = 0; periodMotion.start(); }
    }
    function backPeriod() {
    ...
    readonly property string periodTitle: viewLevel === 0 ? renderedMonth.toLocaleDateString(Qt.locale(), "MMMM yyyy")
        : viewLevel === 1 ? String(browseYear)
        : viewLevel === 2 ? (Math.floor(browseYear / 10) * 10) + "–" + (Math.floor(browseYear / 10) * 10 + 9)
        : (Math.floor(browseYear / 100) * 100) + "–" + (Math.floor(browseYear / 100) * 100 + 99)
    ...
    function shiftPeriod(direction) {
        if (viewLevel === 0) shiftMonth(direction);
        else changePeriod(viewLevel, Math.max(1, Math.min(9999, browseYear + direction * (viewLevel === 1 ? 1 : viewLevel === 2 ? 10 : 100))));
    }
    function choosePeriod(year, month) {
        if (viewLevel === 1) {
            const date = new Date(2000, month, 1);
            date.setFullYear(year);
            const last = new Date(date); last.setMonth(month + 1); last.setDate(0);
    ...
    onAboutToOpen: {
        ...
        agendaProgress = 1;
        preparing = false;
        Qt.callLater(() => { if (root.visible) grid.forceActiveFocus(Qt.OtherFocusReason); });
    }
    Timer {
        interval: 30000
        repeat: true
        ...
        onTriggered: {
            const now = new Date();
            if (root.sameDay(now, root.today)) return;
            const followingToday = root.sameDay(root.selectedDate, root.today);
            root.today = now;
            if (followingToday) root.selectDate(now);
        }
    }
    NumberAnimation {
        id: monthMotion
        target: root; property: "monthProgress"; to: 1
        duration: Theme.calendarPageMotion; easing.type: Easing.OutCubic
        onFinished: root.presentMonth()
    }
    SequentialAnimation {
        id: agendaMotion
        NumberAnimation { target: root; property: "agendaProgress"; to: 0; duration: Theme.reducedMotion ? 0 : 60; easing.type: Easing.OutQuad }
    ...
    function updateAgenda() {
        if (!visible || preparing || Theme.reducedMotion || (presentedEvents.length === 0 && dayEvents.length === 0)) { agendaMotion.stop(); syncAgenda(); agendaProgress = 1; }
        else agendaMotion.restart();
    }
    ...
    function presentMonth() {
        ...
        if (sameMonth(displayedMonth, renderedMonth)) { renderedSelection = selectedDate; return; }
    ...
    function eventCommand(event) {
        const command = ["python3", decodeURIComponent(Qt.resolvedUrl("calendar-launch.py").toString().replace(/^file:\/\//, "")),
            "--date", new Date(event.start * 1000).toLocaleDateString(Qt.locale(), "yyyy-MM-dd")];
        const parts = String(event.id || "").split("\n");
        if (parts.length === 3 && parts[0] && parts[1]) {
            if (!parts[2]) parts.pop();
            command.push("--uuid", parts.join(":"));
        }
        return command;
    }
    function openEvent(event) {
        if (!inlineMode) visible = false;
    ...
    function eventTime(event, day) {
        ...
        return (start < dayStart ? "Earlier" : start.toLocaleTimeString(Qt.locale(), "hh:mm")) + " – "
            + (end > dayEnd ? "Later" : end.toLocaleTimeString(Qt.locale(), "hh:mm"));
    }
    CalendarEvents { id: calendarEvents; active: root.visible && root.calendarServiceEnabled && root.eventSource === calendarEvents; month: root.displayedMonth }
    Flickable {
        id: calendarViewport
        ...
        function reveal(item) {
            if (root.inlineMode || !interactive) return;
        ...
        ScrollBar.vertical: ScrollBar { policy: root.inlineMode ? ScrollBar.AlwaysOff : ScrollBar.AsNeeded }
        ColumnLayout {
            id: calendar
            width: calendarViewport.width
            height: root.inlineMode ? implicitHeight : Math.max(calendarViewport.height,
                dateHeading.implicitHeight + monthCard.implicitHeight + agendaHeading.Layout.minimumHeight
                + agendaCard.Layout.minimumHeight + spacing * 3)
            ...
            RowLayout {
                ...
                ColumnLayout {
                    ...
                    Text { text: root.today.toLocaleDateString(Qt.locale(), "dddd"); color: Theme.muted; font.family: Theme.fontFamily; font.pixelSize: Theme.fontSmall }
                    Text { text: root.today.toLocaleDateString(Qt.locale(), root.popupWidth < 220 ? "d MMM" : "d MMMM"); color: Theme.text; font.family: Theme.fontFamily; font.pixelSize: root.popupWidth < 260 ? 20 : 24; font.weight: Font.DemiBold }
                }
                Item { Layout.fillWidth: true }
                IconButton { visible: root.popupWidth >= 240; Layout.fillWidth: false; iconName: "x-office-calendar-symbolic"; label: "Open Calendar"; onClicked: { if (!root.inlineMode) root.visible = false; root.launchCalendar(["gnome-calendar"], []); } }
            }
            Rectangle {
                id: monthCard
                Layout.fillWidth: true
                ...
                ColumnLayout {
                    ...
                    Keys.onEscapePressed: event => {
                        if (root.viewLevel === 0) { event.accepted = false; return; }
                        root.backPeriod();
                    }
                    ...
                    GridLayout {
                        ...
                        Item { visible: !monthHeader.narrow; Layout.fillWidth: true }
                        ActionButton {
                            ...
                            Behavior on opacity { NumberAnimation { duration: Theme.reducedMotion ? 0 : Theme.motion } }
                            Accessible.description: "Return to today's date. Home key in the calendar."
                            onClicked: { root.changePeriod(0, root.today.getFullYear()); root.selectDate(root.today); grid.forceActiveFocus(); }
                        }
                        IconButton { objectName: "previousMonth"; iconName: "go-previous-symbolic"; label: root.viewLevel === 0 ? "Previous month" : root.viewLevel === 1 ? "Previous year" : root.viewLevel === 2 ? "Previous decade" : "Previous century"; onClicked: root.shiftPeriod(-1) }
                        IconButton { objectName: "nextMonth"; iconName: "go-next-symbolic"; label: root.viewLevel === 0 ? "Next month" : root.viewLevel === 1 ? "Next year" : root.viewLevel === 2 ? "Next decade" : "Next century"; onClicked: root.shiftPeriod(1) }
                        WheelHandler {
                            target: null
                            property real accumulated: 0
                            onWheel: event => {
                                accumulated += event.angleDelta.y;
                                if (Math.abs(accumulated) >= 120) { root.shiftPeriod(accumulated > 0 ? -1 : 1); accumulated = 0; }
                                event.accepted = true;
                            }
                        }
                    }
                    Item {
                        ...
                        DayOfWeekRow {
                            ...
                            delegate: Text { required property string shortName; text: shortName; color: Theme.muted; font.family: Theme.fontFamily; font.pixelSize: Theme.fontSmall; horizontalAlignment: Text.AlignHCenter }
                        }
                        CalendarPeriodPicker {
                            ...
                            transform: Translate { y: -root.periodDirection * 12 * root.periodProgress }
                        }
                        CalendarMonthGrid {
                            width: parent.width
                            height: 240
                        ...
                        CalendarPeriodPicker {
                            ...
                            transform: Translate { y: root.periodDirection * 12 * (1 - root.periodProgress) }
                        ...
                        CalendarMonthGrid {
                            ...
                            onClicked: date => { root.chooseDate(date); grid.forceActiveFocus(Qt.MouseFocusReason); }
            ...
            RowLayout {
                ...
                Text { Layout.fillWidth: true; text: root.sameDay(root.selectedDate, root.today) ? "Today" : root.selectedDate.toLocaleDateString(Qt.locale(), "ddd, d MMM"); color: Theme.text; font.family: Theme.fontFamily; font.pixelSize: Theme.fontSize; font.weight: Font.DemiBold }
            ...
            Rectangle {
                ...
                Layout.minimumHeight: root.inlineMode ? implicitHeight
                    : Math.max(Theme.controlHeight * 2, emptyAgenda.visible ? emptyAgenda.implicitHeight + Theme.gap * 2 : 0)
                ...
                ListView {
                    ...
                    Keys.onDownPressed: event => { if (root.inlineMode) event.accepted = false; else scrollBy(Theme.controlHeight); }
                    Keys.onUpPressed: event => { if (root.inlineMode) event.accepted = false; else scrollBy(-Theme.controlHeight); }
                    Keys.onPressed: event => {
                        if (root.inlineMode) return;
                    ...
                    NumberAnimation { id: agendaScroll; target: agenda; property: "contentY"; duration: Theme.calendarMotion; easing.type: Easing.OutCubic }
                    ScrollBar.vertical: ScrollBar { policy: root.inlineMode ? ScrollBar.AlwaysOff : ScrollBar.AsNeeded }
                    delegate: AbstractButton {
                        ...
                        background: ControlCentreButtonSurface { control: eventButton }
                        contentItem: ColumnLayout {
                            id: eventContents
                            spacing: Theme.spaceSmall
                            Text { Layout.fillWidth: true; text: eventButton.modelData.title; textFormat: Text.PlainText; wrapMode: Text.Wrap; maximumLineCount: 2; elide: Text.ElideRight; color: Theme.text; font.family: Theme.fontFamily; font.pixelSize: Theme.fontSize; font.weight: Font.DemiBold }
                            Text { Layout.fillWidth: true; text: root.eventTime(eventButton.modelData, root.agendaDate); color: Theme.muted; font.family: Theme.fontFamily; font.pixelSize: Theme.fontSmall }
                        }
                    }
                }
                ColumnLayout {
                    ...
                    opacity: root.agendaProgress
                    spacing: Theme.spaceSmall
                    Text { Layout.fillWidth: true; horizontalAlignment: Text.AlignHCenter; text: eventSource.loading ? "Loading events…" : eventSource.error ? "Events unavailable" : "No events"; color: Theme.text; font.family: Theme.fontFamily; font.pixelSize: Theme.fontSize }
                    Text { Layout.fillWidth: true; horizontalAlignment: Text.AlignHCenter; wrapMode: Text.WordWrap; text: eventSource.error || (eventSource.available ? "Nothing scheduled for this day" : "Connect a calendar in Online Accounts"); visible: !eventSource.loading; color: Theme.muted; font.family: Theme.fontFamily; font.pixelSize: Theme.fontSmall }
                }
            }
        }
    }
