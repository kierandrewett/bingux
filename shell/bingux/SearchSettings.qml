...
ColumnLayout {
    ...
    function focusFilter() {
        providerFilter.forceActiveFocus();
        providerFilter.selectAll();
    }
    ...
    readonly property var builtins: [
        {
            ...
            description: "Installed applications and their desktop actions.",
            example: "Search by name, description or executable."
        },
        {
            id: "files",
            name: "Files and folders",
            icon: "folder-symbolic",
            description: "Search your indexed locations and recent files.",
            example: "Leave locations blank to use the locations from your system configuration."
        },
        {
            id: "calculation",
            name: "Calculator",
            icon: "accessories-calculator-symbolic",
            description: "Calculate expressions directly in search.",
            example: "Try 24 * 7 or sqrt(144). Activate a result to copy the answer."
        },
        {
            id: "conversions",
            name: "Unit conversions",
            icon: "accessories-calculator-symbolic",
            description: "Convert length, weight, temperature, time and storage.",
            example: "Try 10 km to mi or 32 f to c."
        },
        {
            id: "web",
            name: "Web search",
            icon: "web-browser-symbolic",
            description: "Open searches with your default search engine.",
            example: "Choose the default engine in the Search engines list."
        },
        {
            id: "web-shortcuts",
            name: "Website shortcuts",
            icon: "insert-link-symbolic",
            description: "Search a specific website with a shortcut.",
            example: "Built-in shortcuts: wiki:, gh:, maps: and yt:. Your search engines also have shortcuts."
        },
        {
            id: "external",
            name: "Connected providers",
            icon: "network-workgroup-symbolic",
            description: "Results from providers installed with your system.",
            example: "Provider processes use the manifests in your system configuration. Website engines can be added below without installing a provider."
        }
    ]
    ...
    function storeEngine() {
        ...
        if (!/^[a-z0-9][a-z0-9-]{0,23}$/.test(shortcut) || ["wiki", "gh", "maps", "yt"].includes(shortcut) || engines.some(e => e.id !== editingId && e.shortcut === shortcut)) {
            errorField = "shortcut";
            error = "Choose a unique shortcut, such as docs or music.";
            shortcutField.input.forceActiveFocus();
            return;
        }
        if (!/^https?:\/\/[^\s/@]+(?:\/|\?|#)/.test(url) || url.split("{query}").length !== 2) {
            errorField = "url";
            error = "Enter a website URL with {query} where the search terms go.";
            urlField.input.forceActiveFocus();
            return;
        }
    ...
    function toggleEngine(engine) {
        if (engine.id === settings.draft.search.defaultEngine)
            return;
        settings.update("search", "engines", engines.map(e => e.id === engine.id ? Object.assign({}, e, {
                enabled: !e.enabled
            }) : e));
    }
    FolderDialog {
        ...
        onAccepted: {
            ...
            if (path && !paths.includes(path))
                root.settings.update("search", "fileRoots", paths.concat([path]));
        }
    }
    ColumnLayout {
        ...
        FilterField {
            id: providerFilter
            objectName: "settingsProviderFilter"
            Layout.fillWidth: true
            placeholderText: root.enginesOnly ? "Find a search engine" : "Find a search provider"
            Accessible.name: placeholderText
            onTextEdited: root.filter = text.toLowerCase()
        }
        SettingsHeading {
            visible: root.enginesOnly ? root.matchingEngines === 0 : root.matchingProviders === 0
            title: "No matches"
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                SettingsHeading {
                    ...
                    description: "Search directly with a shortcut, such as ddg: cats."
                }
                ActionButton {
                    objectName: "addSearchEngine"
                    text: "Add"
                    iconName: "list-add-symbolic"
                    onClicked: root.editEngine(null)
                }
            }
            SettingsGroup {
                ...
                Repeater {
                    ...
                    ColumnLayout {
                        ...
                        SettingsRow {
                            title: modelData.name
                            subtitle: modelData.shortcut + ":"
                            valueText: modelData.id === root.settings.draft.search.defaultEngine ? "Default" : ""
                            iconName: "web-browser-symbolic"
                            navigation: true
                            toggleVisible: modelData.id !== root.settings.draft.search.defaultEngine
    ...
    ColumnLayout {
        ...
        ColumnLayout {
            ...
            SettingsHeading {
                title: "Search results"
                description: root.selectedProvider.description
            }
            SettingsGroup {
                SettingsRow {
                    ...
                    iconName: ""
                    toggleVisible: true
                    toggleChecked: !root.settings.draft.search.disabledProviders.includes(root.detail)
                    onToggleRequested: root.settings.setProvider(root.detail, !toggleChecked)
        ...
        SettingsHeading {
            title: "How it works"
            description: root.selectedProvider.example
        }
        ColumnLayout {
            Layout.fillWidth: true
            spacing: Theme.gap
        ...
        ActionButton {
            visible: root.detail === "web" || root.detail === "web-shortcuts"
            text: "Manage search engines"
            onClicked: {
                root.detail = "";
                root.settings.page = "Engines";
            }
        }
    }
    ColumnLayout {
        ...
        ColumnLayout {
            ...
            SettingsHeading {
                ...
                description: "Add a website's search URL. Bingux inserts your search terms at {query}."
            }
            SettingsGroup {
                SettingsField {
                    id: nameField
                    errorText: root.errorField === "name" ? root.error : ""
                    objectName: "engineName"
                    label: "Name"
                    placeholderText: "Wikipedia"
                    text: root.engineName
                    onEdited: value => root.engineName = value
                }
                SettingsField {
                    id: shortcutField
                    errorText: root.errorField === "shortcut" ? root.error : ""
                    hint: "Type this followed by a colon in search."
                    objectName: "engineShortcut"
                    label: "Shortcut"
                    placeholderText: "encyclopedia"
                    text: root.engineShortcut
                    onEdited: value => root.engineShortcut = value
                }
                SettingsField {
                    id: urlField
                    errorText: root.errorField === "url" ? root.error : ""
                    objectName: "engineUrl"
                    label: "Search URL"
                    hint: "Replace the search terms in the URL with {query}."
                    placeholderText: "https://example.com/search?q={query}"
                    text: root.engineUrl
                    onEdited: value => root.engineUrl = value
                }
            }
        }
        SettingsGroup {
            ...
            SettingsRow {
                ...
                onToggleRequested: {
                    root.settings.update("search", "engines", root.engines.map(e => e.id === root.editingId ? Object.assign({}, e, {
                            enabled: true
                        }) : e));
                    root.settings.update("search", "defaultEngine", root.editingId);
                }
                onClicked: if (toggleEnabled)
                    toggleRequested()
            }
        }
        RowLayout {
            ...
            ActionButton {
                ...
                flat: true
                onClicked: {
                    root.settings.update("search", "engines", root.engines.filter(e => e.id !== root.editingId));
                    root.goBack();
                }
            }
            Item {
            ...
            ActionButton {
                objectName: "saveSearchEngine"
                text: root.editingId ? "Save engine" : "Add engine"
                onClicked: root.storeEngine()
            }
        }
    }
}
