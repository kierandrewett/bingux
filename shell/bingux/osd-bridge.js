...
export default function enable(api) {
    ...
    function refreshOutputs() {
        connection.call('org.gnome.Mutter.DisplayConfig', '/org/gnome/Mutter/DisplayConfig',
            'org.gnome.Mutter.DisplayConfig', 'GetCurrentState', null, null,
            Gio.DBusCallFlags.NONE, 1500, null, (bus, result) => {
                if (disposed) return;
                try {
                    const logical = bus.call_finish(result).deepUnpack()[2];
                    outputs = Main.layoutManager.monitors.map(monitor =>
                        logical.find(item => item[0] === monitor.x && item[1] === monitor.y)?.[5].map(spec => spec[0]) ?? []);
                } catch (error) {
    ...
    connection.call('org.gnoblin.Shell', '/org/gnoblin/Shell',
        ...
        Gio.DBusCallFlags.NONE, 1500, null, (bus, result) => {
            ...
            try {
                ...
                wrapper = function (monitorIndex, icon, label, level, maxLevel) {
                    ...
                    const text = String(label ?? '').replace(/[\u0000-\u001f\u007f]/g, ' ').slice(0, 512);
                    connection.emit_signal(null, '/org/gnoblin/Shell', 'org.gnoblin.Shell', 'OsdRequested',
                        new GLib.Variant('(uissddas)', [2, monitorIndex, iconName, text,
                            Number.isFinite(level) ? level : -1,
                            Number.isFinite(maxLevel) ? maxLevel : 1, names]));
                };
