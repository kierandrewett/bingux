...
MouseArea {
    ...
    Component.onCompleted: if (previewSource)
        DesktopEditing.registerSource(widgetId, control)
    Component.onDestruction: if (previewSource)
        DesktopEditing.unregisterSource(widgetId, control)
    ...
    TapHandler {
        ...
        onTapped: {
            ...
            if (!root.control.contains(root.control.mapFromItem(parent, point)))
                return;
            for (let item = root.control.parent; item; item = item.parent)
                if (item.clip && !item.contains(item.mapFromItem(parent, point)))
                    return;
