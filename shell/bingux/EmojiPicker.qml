...
ShellPopup {
    ...
    property var popupPosition: ({x: (width - popupWidth) / 2, y: (height - popupHeight) * 0.32})
    ...
    onSkinToneChanged: if (persistRecent && skinToneReady) tonePreference.setText(JSON.stringify(skinTone))
    ...
    readonly property var categories: [
        {id: "", label: "All emoji", glyph: "⌘"},
        {id: "recent", label: "Recently used", glyph: "◷"},
        {id: "Smileys & Emotion", label: "Smileys", glyph: "😀"},
        {id: "People & Body", label: "People", glyph: "👋"},
        {id: "Animals & Nature", label: "Nature", glyph: "🌿"},
        {id: "Food & Drink", label: "Food", glyph: "🍋"},
        {id: "Travel & Places", label: "Travel", glyph: "🚀"},
    ...
    function open() {
        ...
        if (pendingText || locating) return;
    ...
    function finishPlacement() {
        ...
        anchorDeadline.stop();
        if (inputAnchor) {
            let anchor = {x: inputAnchor.x, y: inputAnchor.y, height: 1};
            ...
            if (caret && frame && Number.isFinite(caret.x) && Number.isFinite(caret.y)
                && caret.height > 0 && caret.height < 200
                && caret.x >= frame.x && caret.x <= frame.x + frame.width
                && caret.y >= frame.y && caret.y + caret.height <= frame.y + frame.height)
                anchor = caret;
            const output = Quickshell.screens.find(s => anchor.x >= s.x && anchor.x < s.x + s.width
    ...
    Timer { id: anchorDeadline; interval: 220; onTriggered: root.finishPlacement() }
    Process {
        id: caretProbe
        command: (Quickshell.env("BINGUX_CARET_HELPER") ? [Quickshell.env("BINGUX_CARET_HELPER")]
            : ["python3", decodeURIComponent(Qt.resolvedUrl("caret-anchor.py").toString().replace(/^file:\/\//, ""))])
            .concat([String(root.inputAnchor ? root.inputAnchor.pid : 0)])
        stdout: StdioCollector {
            onStreamFinished: {
                try { root.caretResult = JSON.parse(text); } catch (_) { root.caretResult = null; }
            }
        }
    ...
    function choose() {
        ...
        if (copyOnSelect) Quickshell.clipboardText = emoji;
    ...
    function moveSelection(delta) {
        ...
        if (top < destination) destination = top;
        else if (bottom > destination + grid.height) destination = bottom - grid.height;
        destination = Math.max(0, Math.min(Math.max(0, grid.contentHeight - grid.height), destination));
        gridScroll.stop();
        if (Theme.reducedMotion) grid.contentY = destination;
        else { gridScroll.to = destination; gridScroll.restart(); }
    }
    NumberAnimation { id: gridScroll; target: grid; property: "contentY"; duration: 140; easing.type: Easing.OutCubic }
    ...
    onVisibleChanged: if (!visible) tonePopup.close()
    onResultsChanged: { gridScroll.stop(); selectedIndex = 0; grid.positionViewAtBeginning(); }
    Timer { id: focusInput; interval: 0; onTriggered: if (root.visible) input.forceActiveFocus() }
    ShortcutSession {
        id: shortcut
    ...
    FileView {
        ...
        onLoaded: {
            try { root.catalogue = EmojiSearch.foldSkinTones(JSON.parse(text()).emoji); }
            catch (_) { root.error = "Could not load emoji."; }
        }
    ...
    FileView {
        ...
        onLoaded: {
            try {
                ...
                if (Array.isArray(data)) root.recent = data.filter(value => typeof value === "string" && value.length < 64).slice(0, 32);
            } catch (_) {}
    ...
    FileView {
        ...
        onLoaded: {
            try {
                ...
                if (Number.isInteger(tone) && tone >= 0 && tone <= 5) root.skinTone = tone;
            } catch (_) {}
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            TextField {
                ...
                background: Rectangle { radius: 8; color: Theme.elevated; border.width: input.activeFocus ? 1 : 0; border.color: Theme.outline }
                Keys.onDownPressed: root.moveSelection(8)
                Keys.onUpPressed: { if (root.selectedIndex < 8) root.focusCategory(); else root.moveSelection(-8); }
            ...
            AbstractButton {
                ...
                Keys.onEscapePressed: root.visible = false
                background: ControlCentreButtonSurface { control: toneButton; selected: tonePopup.opened; radius: 8 }
                contentItem: Text { text: root.skinTones[root.skinTone]; font.family: "Noto Color Emoji"; font.pixelSize: 24; horizontalAlignment: Text.AlignHCenter; verticalAlignment: Text.AlignVCenter }
                ShellTooltip { parent: toneButton; visible: toneButton.hovered && !tonePopup.opened; text: "Skin tone" }
                Popup {
                    id: tonePopup
                    ...
                    onClosed: if (root.visible) input.forceActiveFocus()
                    background: Rectangle { color: Theme.elevated; radius: 10; border.width: 1; border.color: Theme.outline }
                    contentItem: Row {
                        spacing: 2
                        Repeater {
                            id: toneOptions
                            ...
                            AbstractButton {
                                ...
                                onClicked: { root.skinTone = index; tonePopup.close(); }
                                ...
                                background: ControlCentreButtonSurface { control: toneOption; selected: root.skinTone === toneOption.index; radius: 6 }
                                contentItem: Text { text: toneOption.modelData; font.family: "Noto Color Emoji"; font.pixelSize: 24; horizontalAlignment: Text.AlignHCenter; verticalAlignment: Text.AlignVCenter }
                            }
                        }
                    }
                }
        ...
        RowLayout {
            ...
            Repeater {
                ...
                AbstractButton {
                    ...
                    onClicked: { root.category = modelData.id; input.forceActiveFocus(); }
                    ...
                    Keys.onEscapePressed: root.visible = false
                    background: ControlCentreButtonSurface { control: categoryButton; selected: root.category === categoryButton.modelData.id }
                    contentItem: Item {
                        SymbolicIcon {
                    ...
                    ShellTooltip { parent: categoryButton; visible: categoryButton.hovered; text: categoryButton.modelData.label }
                }
            }
        }
        GridView {
            ...
            delegate: AbstractButton {
                ...
                background: ControlCentreButtonSurface { control: emojiButton; selected: emojiButton.index === root.selectedIndex; radius: 8 }
                contentItem: Text { text: emojiButton.modelData.emoji; font.family: "Noto Color Emoji"; font.pixelSize: 28; horizontalAlignment: Text.AlignHCenter; verticalAlignment: Text.AlignVCenter }
            }
            Text {
                anchors.centerIn: parent
        ...
        RowLayout {
            Layout.fillWidth: true
            Text { Layout.fillWidth: true; text: root.error || (root.selectedEmoji ? root.selectedEmoji.name : ""); elide: Text.ElideRight; color: root.error ? "#ff737d" : Theme.muted; font.family: Theme.fontFamily; font.pixelSize: Theme.fontSmall }
            Text { text: root.insertOnSelect ? "Enter to insert" : "Enter to copy"; color: Theme.muted; font.family: Theme.fontFamily; font.pixelSize: Theme.fontSmall }
        }
    }
}
