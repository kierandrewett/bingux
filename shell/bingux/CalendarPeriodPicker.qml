...
GridLayout {
    ...
    function focusCell(index) {
        ...
        if (cell)
            cell.forceActiveFocus(Qt.OtherFocusReason);
    }
    ...
    Repeater {
        ...
        ActionButton {
            ...
            readonly property bool current: root.level === 1 ? root.selectedDate.getFullYear() === cellYear && root.selectedDate.getMonth() === index : root.level === 2 ? root.selectedDate.getFullYear() === cellYear : root.selectedDate.getFullYear() >= cellYear && root.selectedDate.getFullYear() <= cellYear + 9
            ...
            text: root.level === 1 ? new Date(2000, index, 1).toLocaleDateString(Qt.locale(), "MMM") : root.level === 2 ? String(cellYear) : cellYear + "–" + (cellYear + 9)
            Accessible.description: root.level === 1 ? "Show days in " + text + " " + cellYear : root.level === 2 ? "Show months in " + text : "Show years from " + text
            ...
            Keys.onPressed: event => {
                if (event.key === Qt.Key_Left)
                    root.focusCell(index - 1);
                else if (event.key === Qt.Key_Right)
                    root.focusCell(index + 1);
                else if (event.key === Qt.Key_Up)
                    root.focusCell(index - 3);
                else if (event.key === Qt.Key_Down)
                    root.focusCell(index + 3);
                else if (event.key === Qt.Key_PageUp || event.key === Qt.Key_PageDown)
                    root.pageRequested(event.key === Qt.Key_PageUp ? -1 : 1);
