...
ActivityIndicator {
    ...
    label: capture.recording ? capture.elapsedText : finalizing ? "Saving…" : capture.state === "countdown" ? String(capture.countdown) : capture.state === "starting" ? "Starting…" : privacy.elapsedText
    tooltip: finalizing ? "Saving screen recording" : capture.busy && !capture.recording ? "Cancel capture" : "Stop screen recording"
    onClicked: {
        if (capture.busy)
            capture.stop();
        else
