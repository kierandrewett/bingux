...
ColumnLayout {
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                if (worker.requestedScope !== root.scope)
                    return;
                try {
                    ...
                    root.error = "Could not read services";
                }
            }
        }
        stderr: StdioCollector {
            onStreamFinished: if (text.trim() && worker.requestedScope === root.scope)
                root.error = text.trim()
        }
        onExited: if (requestedScope !== root.scope)
            root.refresh()
    }
    RowLayout {
        ...
        SegmentedControl {
            ...
            implicitWidth: 116
            implicitHeight: 30
            accessiblePrefix: "Services scope "
    ...
    ProcessTable {
        ...
        sortKey: "Name"
        descending: false
        records: root.visible ? root.services.map(service => ({
                    key: service.unit,
                    name: service.unit,
        ...
        onContextRequested: (row, selected) => {
            ...
            menu.preferredX = point.x;
            menu.preferredY = point.y;
            menu.visible = true;
    ...
    function act(action) {
        if (actionWorker.running || !selection.length)
            return;
        menu.visible = false;
        error = "";
    ...
    Process {
        ...
        onExited: (code, status) => {
            if (code === 0)
                root.error = "Service action completed";
            root.refresh();
        }
    }
    ShellPopup {
