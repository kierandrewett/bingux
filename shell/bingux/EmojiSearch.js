...
function foldSkinTones(rows) {
    ...
    for (const row of rows) {
        let name = row.name
            .replace(/(?:medium-light|medium-dark|light|medium|dark) skin tone/g, "")
            .replace(/([:,])(?:,|\s)+/g, "$1 ")
            .replace(/[:, ]+$/, "");
...
function search(rows, query, group, recent, skinTone = 0) {
    const display = (row) =>
        row.tones ? Object.assign({}, row, { emoji: row.tones[skinTone] || row.tones[0] || row.emoji }) : row;
    ...
    const result = rows.filter(
        (row) =>
            (!group || group === "recent" || row.group === group) &&
            words.every((word) => {
    ...
    if (words.length) {
        ...
        result.sort(
            (a, b) =>
                Number(b.name.toLocaleLowerCase() === exact) - Number(a.name.toLocaleLowerCase() === exact) ||
