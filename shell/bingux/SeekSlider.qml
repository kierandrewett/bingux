...
Slider {
    ...
    implicitWidth: 160
    HoverHandler { id: pointer; enabled: root.enabled; cursorShape: Qt.PointingHandCursor }
    background: Item {
        id: track
        ...
        Behavior on height { NumberAnimation { duration: Theme.reducedMotion ? 0 : Theme.motion; easing.type: Easing.OutCubic } }
        Behavior on opacity { NumberAnimation { duration: Theme.reducedMotion ? 0 : Theme.motion } }
        Rectangle { anchors.fill: parent; radius: height / 2; color: Theme.outline }
        Rectangle {
            id: preview
        ...
        Rectangle {
            ...
            Behavior on color { ColorAnimation { duration: Theme.reducedMotion ? 0 : Theme.motion } }
        }
        Item {
            id: boost
    ...
    handle: Item {
        ...
        Rectangle {
            ...
            Behavior on scale { NumberAnimation { duration: Theme.reducedMotion ? 0 : Theme.motion; easing.type: Easing.OutCubic } }
            Behavior on opacity { NumberAnimation { duration: Theme.reducedMotion ? 0 : Theme.motion } }
        }
    }
}
