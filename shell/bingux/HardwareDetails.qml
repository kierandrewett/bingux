...
Item {
    ...
    function updateProcesses() {
        if (!visible || page !== "processes" || !hardware) {
            processRecords = [];
            processRevision = -1;
            return;
        }
    ...
    onProcessSortChanged: if (processLoader.item)
        processLoader.item.sortKey = processSort
    ...
    function number(value, unit, digits = 0) {
        return typeof value === "number" && isFinite(value) ? value.toFixed(digits) + unit : "—";
    }
    function bytes(value) {
        return typeof value === "number" ? monitorWidget.systemMetrics.formatBytes(value) : "—";
    }
    ColumnLayout {
        id: gpuSection
        visible: root.page === "gpu"
        width: parent.width
        spacing: 10
        Header {
            title: "GPU"
        ...
        Caption {
            visible: !(root.hardware?.gpus?.length)
            text: "Unavailable"
        }
        Repeater {
            ...
            ColumnLayout {
                ...
                Reading {
                    label: "Usage"
                    reading: root.number(modelData.busyPercent, "%")
                    readingColor: Theme.usageColor(modelData.busyPercent)
                }
                Reading {
                    label: "Temperature"
                    reading: root.number(modelData.temperatureCelsius, "°C")
                }
                Reading {
                    label: "VRAM"
                    readingColor: Theme.usageColor(modelData.memoryTotalBytes > 0 ? modelData.memoryUsedBytes / modelData.memoryTotalBytes * 100 : null)
                    reading: root.bytes(modelData.memoryUsedBytes) + " / " + root.bytes(modelData.memoryTotalBytes)
                }
                Meter {
                    visible: typeof modelData.memoryUsedBytes === "number" && modelData.memoryTotalBytes > 0
                    fraction: modelData.memoryTotalBytes > 0 ? modelData.memoryUsedBytes / modelData.memoryTotalBytes : 0
                }
                ColumnLayout {
                    Layout.fillWidth: true
                    spacing: 10
                    Reading {
                        label: "Power"
                        reading: root.number(modelData.powerWatts, " W", 1)
                    }
                    Reading {
                        label: "Clock"
                        reading: root.number(modelData.clockMhz, " MHz")
                    }
                    Reading {
                        label: "Fan"
                        reading: root.number(modelData.fanRpm, " RPM")
                    }
                    Reading {
                        label: "Driver"
                        reading: modelData.driver || "—"
                    }
                    Reading {
                        label: "PCI"
                        reading: modelData.pciAddress
                    }
                }
            }
        }
    ...
    ColumnLayout {
        ...
        Repeater {
            ...
            ColumnLayout {
                ...
                spacing: 5
                Reading {
                    label: modelData.path === "/" ? "System" : "Home"
                    reading: root.bytes(modelData.availableBytes) + " free"
                    readingColor: Theme.usageColor(modelData.totalBytes > 0 ? (1 - modelData.availableBytes / modelData.totalBytes) * 100 : null)
                }
                Meter {
                    fraction: modelData.totalBytes > 0 ? 1 - modelData.availableBytes / modelData.totalBytes : 0
                }
                HoverHandler {
                    id: storageHover
                }
                ShellTooltip {
                    ...
                    text: modelData.path + " · " + root.bytes(modelData.totalBytes) + " total"
                }
            }
        }
    }
    ...
    function processAction(action) {
        if (!selectedProcess || actionRunner.running)
            return;
        processMenu.visible = false;
        actionMessage = "";
        actionRunner.command = ["python3", decodeURIComponent(Qt.resolvedUrl("process-action.py").toString().replace(/^file:\/\//, "")), "--batch", action];
    ...
    Process {
        id: actionRunner
        stdinEnabled: true
        onStarted: write(JSON.stringify(root.selectedProcesses.map(process => ({
                    pid: process.pid,
                    startTime: process.startTime || 0
                }))) + "\n")
        stdout: StdioCollector {
            onStreamFinished: root.actionMessage = text.trim()
        }
    ...
    Caption {
        ...
        width: parent.width
        visible: root.page === "processes" && text.length > 0
    ...
    ShellPopup {
        ...
        ColumnLayout {
            ...
            Caption {
                Layout.margins: 6
                text: root.selectedProcesses.length > 1 ? root.selectedProcesses.length + " processes selected" : root.selectedProcess ? root.selectedProcess.name + " · " + root.selectedProcess.pid : ""
            }
            ActionButton {
                Layout.fillWidth: true
                flat: true
                alignLeft: true
                text: root.selectedProcesses.length > 1 ? "Copy PIDs" : "Copy PID"
                iconName: "edit-copy-symbolic"
                onClicked: root.processAction("copy")
            }
            ActionButton {
                Layout.fillWidth: true
                flat: true
                alignLeft: true
                text: "Pause"
                objectName: "processAction_pause"
                iconName: "media-playback-pause-symbolic"
                enabled: !actionRunner.running && root.selectedProcesses.length > 0 && root.selectedProcesses.every(process => !!process.startTime)
                onClicked: root.processAction("pause")
            }
            ActionButton {
                Layout.fillWidth: true
                flat: true
                alignLeft: true
                text: "Resume"
                objectName: "processAction_resume"
                iconName: "media-playback-start-symbolic"
                enabled: !actionRunner.running && root.selectedProcesses.length > 0 && root.selectedProcesses.every(process => !!process.startTime)
                onClicked: root.processAction("resume")
            }
            ActionButton {
                Layout.fillWidth: true
                flat: true
                alignLeft: true
                text: root.selectedProcesses.length > 1 ? "End processes" : "End process"
                objectName: "processAction_end"
                iconName: "media-playback-stop-symbolic"
                enabled: !actionRunner.running && root.selectedProcesses.length > 0 && root.selectedProcesses.every(process => !!process.startTime)
                onClicked: root.processAction("end")
            }
            ActionButton {
                Layout.fillWidth: true
                flat: true
                alignLeft: true
                text: "Force quit"
                objectName: "processAction_kill"
                iconName: "process-stop-symbolic"
                enabled: !actionRunner.running && root.selectedProcesses.length > 0 && root.selectedProcesses.every(process => !!process.startTime)
                onClicked: root.processAction("kill")
            }
            Keys.onPressed: event => {
                if (![Qt.Key_Up, Qt.Key_Down].includes(event.key))
                    return;
    ...
    component Header: RowLayout {
        ...
        SymbolicIcon {
            implicitSize: 16
            source: Quickshell.iconPath(header.iconName)
            color: Theme.muted
        }
        Caption {
            text: header.title
    ...
    component Meter: Rectangle {
        ...
        Rectangle {
            ...
            color: Theme.usageColor(parent.fraction * 100, Theme.accent)
        }
    }
