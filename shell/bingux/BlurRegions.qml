...
QtObject {
    ...
    function add(item) {
        registrations = registrations.concat([item]);
        Qt.callLater(flush);
    }
    function remove(item) {
        if (item.published && session.capabilities.includes("blur-regions"))
            session.send(Object.assign({}, item.published, {
                region: null
            }));
    ...
    function flush() {
        if (!session.ready || !session.capabilities.includes("blur-regions"))
            return;
        for (const item of registrations) {
            ...
            if (old && (!record || old.namespace !== record.namespace || JSON.stringify(old.screen) !== JSON.stringify(record.screen)))
                session.send(Object.assign({}, old, {
                    region: null
                }));
            if (record && JSON.stringify(old) !== JSON.stringify(record))
                session.send(record);
