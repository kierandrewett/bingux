...
Singleton {
    ...
    function showControl(name) {
        return effectiveControls[name] === true;
    }
    function setControl(name, shown) {
        ...
        if (BinguxPreferences.data.desktop.controlCentre) {
            BinguxPreferences.saveDesktop({
                controlCentre: Object.assign({}, effectiveControls, {
                    [name]: shown
                })
            });
            return;
        ...
        if (preferencesReady)
            preferences.setText(JSON.stringify(controls));
    }
    ...
    function toggleAwake() {
        error = "";
        if (state.awakeAvailable)
            keepAwake = !keepAwake;
    }
    onMonitoringChanged: if (ready)
    ...
    Process {
        ...
        onExited: code => {
            if (code === 0)
                preferences.path = root.preferencesDirectory + "/controls.json";
        }
    }
    FileView {
        id: preferences
    ...
    Process {
        id: worker
        command: Quickshell.env("BINGUX_CONTROLS_HELPER") ? [Quickshell.env("BINGUX_CONTROLS_HELPER")] : ["python3", "-u", decodeURIComponent(Qt.resolvedUrl("control-centre-services.py").toString().replace(/^file:\/\//, ""))]
        running: true
        stdinEnabled: true
        stdout: SplitParser {
            onRead: line => {
