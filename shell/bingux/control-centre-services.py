...
def properties(bus_type, name, path):
    ...
    response = bus.call_sync(name, path, 'org.freedesktop.DBus.Properties', 'GetAll',
        GLib.Variant('(s)', (name,)), GLib.VariantType.new('(a{sv})'), Gio.DBusCallFlags.NONE, 1500, None)
...
def power_state():
    for name, path in POWER_SERVICES:
        try:
            ...
            return {'available': True, 'profile': data['ActiveProfile'],
                    'profiles': [profile['Profile'] for profile in data['Profiles']], 'service': name, 'path': path}
        except GLib.Error:
            continue
...
def mullvad_state(data):
    ...
    subtitle = {'connected': 'Connected', 'connecting': 'Connecting', 'disconnecting': 'Disconnecting',
                'disconnected': 'Disconnected', 'error': 'Connection error'}.get(state, 'Unavailable')
    ...
    return {'id': 'mullvad', 'name': 'Mullvad', 'connected': state == 'connected', 'subtitle': subtitle,
...
def tailscale_state(data, prefs=None):
    ...
    subtitle = 'Private network connected' if connected else 'Disconnected' if state == 'Stopped' else 'Sign in required' if state == 'NeedsLogin' else 'Unavailable'
    ...
    for peer in [data.get('Self') or {}] + list((data.get('Peer') or {}).values()):
        ...
        provider = 'Mullvad' if 'tag:mullvad-exit-node' in (peer.get('Tags') or []) else 'VPN provider' if location else ''
        nodes.append({'id': peer['ID'], 'name': peer.get('HostName') or peer.get('DNSName', '').rstrip('.'),
                      'dns': peer.get('DNSName', '').rstrip('.'), 'ips': peer.get('TailscaleIPs') or [],
                      'os': peer.get('OS', ''), 'online': bool(peer.get('Online')),
                      'self': peer is data.get('Self'), 'exitOption': bool(peer.get('ExitNodeOption')),
                      'exit': bool(peer.get('ExitNode')), 'provider': provider,
                      'country': location.get('Country', ''), 'countryCode': location.get('CountryCode', ''),
                      'city': location.get('City', '')})
    nodes.sort(key=lambda node: (not node['self'], not node['online'], node['name'].casefold()))
    flags = {'accept-dns': 'CorpDNS', 'accept-routes': 'RouteAll', 'shields-up': 'ShieldsUp',
             'exit-node-allow-lan-access': 'ExitNodeAllowLANAccess'}
    return {'id': 'tailscale', 'name': 'Tailscale', 'connected': connected, 'subtitle': subtitle,
            'canToggle': state in ('Running', 'Stopped'), 'nodes': nodes,
            'exitNode': (prefs or {}).get('ExitNodeID') or (exit_node or {}).get('ID', ''),
            'preferences': {flag: prefs[key] for flag, key in flags.items()
                            if prefs is not None and isinstance(prefs.get(key), bool)}}
...
def vpn_state():
    ...
    for command, parser in [('mullvad', mullvad_state), ('tailscale', tailscale_state)]:
        ...
        try:
            ...
            rows.append({'id': command, 'name': command.capitalize(), 'connected': False, 'subtitle': 'Service unavailable', 'canToggle': False})
    if shutil.which('nmcli'):
        try:
            for line in run(['nmcli', '-t', '-e', 'yes', '-f', 'UUID,TYPE,NAME,DEVICE', 'connection', 'show']).splitlines():
                ...
                rows.append({'id': 'nm:' + fields[0], 'name': fields[2], 'connected': connected,
                             'subtitle': 'Connected' if connected else 'Disconnected', 'canToggle': True})
        except (ValueError, OSError, subprocess.TimeoutExpired):
            pass
    return rows
...
class Controls:
    ...
    def settings_state(self):
        return {'doNotDisturb': not self.notifications.get_boolean('show-banners') if self.notifications else False,
    ...
    def read(self):
        ...
        try:
            ...
            result['awakeAvailable'] = bool(shutil.which('gnome-session-inhibit')) and bus.call_sync(
                'org.freedesktop.DBus', '/org/freedesktop/DBus', 'org.freedesktop.DBus', 'NameHasOwner',
                GLib.Variant('(s)', ('org.gnome.SessionManager',)), None, Gio.DBusCallFlags.NONE, 1000, None).unpack()[0]
        except GLib.Error:
            result['awakeAvailable'] = False
        try:
            result['nightLightActive'] = properties(Gio.BusType.SESSION, 'org.gnome.SettingsDaemon.Color', '/org/gnome/SettingsDaemon/Color').get('NightLightActive', False)
        except GLib.Error:
    ...
    def apply(self, request):
        ...
        if kind in ('dnd', 'nightLight') and isinstance(enabled, bool):
            target, key = (self.notifications, 'show-banners') if kind == 'dnd' else (self.colour, 'night-light-enabled')
            if target is None or not target.is_writable(key) or not target.set_boolean(key, not enabled if kind == 'dnd' else enabled):
                raise ValueError('This setting is unavailable.')
            Gio.Settings.sync()
        elif kind == 'vpn' and isinstance(enabled, bool):
            identity = request.get('id')
            row = next((row for row in self.snapshot.get('vpns', []) if row['id'] == identity and row['canToggle']), None)
            if not row:
            ...
            if identity == 'mullvad':
                ...
                run(['nmcli', '--wait', '10', 'connection', 'up' if enabled else 'down', 'uuid', identity[3:]], timeout=12)
        elif kind == 'power':
            power = power_state()
            profile = request.get('profile')
            if profile not in power['profiles']:
                raise ValueError('This power mode is unavailable.')
            Gio.bus_get_sync(Gio.BusType.SYSTEM, None).call_sync(power['service'], power['path'],
                'org.freedesktop.DBus.Properties', 'Set', GLib.Variant('(ssv)', (power['service'], 'ActiveProfile', GLib.Variant('s', profile))),
            ...
            if setting == 'exit-node':
                ...
                if identity == '':
                    ...
                    peer = next((peer for peer in (data.get('Peer') or {}).values()
                                 if peer.get('ID') == identity and peer.get('ExitNodeOption') and peer.get('Online')), None)
            ...
            elif setting in ('accept-dns', 'accept-routes', 'shields-up', 'exit-node-allow-lan-access') and isinstance(enabled, bool):
