...
Item {
    ...
    function label(value) {
        return formatter ? formatter(value) : value > 99 ? "99+" : String(value);
    }
    function advance() {
        if (!ready || roll.running || displayedValue === value)
            return;
    ...
    Text {
        ...
        y: (root.opticalCenter ? (root.height - currentMetrics.tightBoundingRect.height) / 2 - baselineOffset - currentMetrics.tightBoundingRect.y : (root.height - height) / 2) - root.direction * root.progress * root.height
        text: root.label(root.displayedValue)
        color: root.color
    ...
    Text {
        anchors.horizontalCenter: parent.horizontalCenter
        y: (root.opticalCenter ? (root.height - nextMetrics.tightBoundingRect.height) / 2 - baselineOffset - nextMetrics.tightBoundingRect.y : (root.height - height) / 2) + root.direction * (1 - root.progress) * root.height
        text: root.label(root.nextValue)
        color: root.color
