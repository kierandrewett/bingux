...
TrayMenu {
    ...
    readonly property real menuX: requestedPosition.x + popupWidth + edgeGap <= width - edgeGap
        ? requestedPosition.x + edgeGap
        : requestedPosition.x - popupWidth - edgeGap
    readonly property real menuY: requestedPosition.y + popupHeight + edgeGap <= height - edgeGap
        ? requestedPosition.y + edgeGap
        : requestedPosition.y - popupHeight - edgeGap
    preferredX: menuX
    preferredY: menuY
    compactRows: true
    ...
    function showRequest(payload) {
        ...
        if (request.version !== 1 || !/^\d+$/.test(request.window) ||
                !Number.isFinite(request.x) || !Number.isFinite(request.y) || !Array.isArray(request.actions))
            throw new Error("Invalid window menu request");
        const target = Quickshell.screens.find(s => request.x >= s.x && request.y >= s.y &&
                request.x < s.x + s.width && request.y < s.y + s.height);
        if (!target)
        ...
        const allowed = ["minimize", "maximize", "unmaximize", "interactive-move", "interactive-resize",
            "above", "unabove", "stick", "unstick", "close"];
