...
ColumnLayout {
    ...
    onHoldOrderChanged: if (!holdOrder) reorderDelay.restart()
    Timer { id: reorderDelay; interval: 900; onTriggered: if (!root.holdOrder) root.refresh() }
    ...
    readonly property var sortedRecords: filteredRecords.slice().sort((a, b) => {
        ...
        let delta = 0;
        if (sortKey === "CPU") delta = (a.cpuPercent ?? -1) - (b.cpuPercent ?? -1);
        else if (sortKey === "Memory") delta = a.memoryBytes - b.memoryBytes;
        else if (sortKey === "PID") delta = a.pid - b.pid;
        else delta = String(a.name).localeCompare(String(b.name));
        return (descending ? -delta : delta) || (a.pid ?? 0) - (b.pid ?? 0);
    })
    spacing: 10
    implicitHeight: 360
    function entryKey(record) { return record.key || record.pid + ":" + (record.startTime || 0); }
    function stateName(state) { return ({R: "Running", S: "Sleeping", D: "Waiting", T: "Stopped", t: "Stopped", Z: "Zombie", I: "Idle"})[state] || state || "Unknown"; }
    function stateColor(state) {
        if (["R", "running", "active", "listening"].includes(state)) return Theme.success;
        if (["Z", "failed"].includes(state)) return Theme.danger;
        if (["D", "T", "t", "activating", "deactivating", "start", "start-pre", "start-post", "stop", "stop-sigterm", "stop-sigkill", "auto-restart", "reload"].includes(state)) return Theme.warning;
        return Theme.muted;
    }
    ...
    function selectIndex(index, modifiers = 0) {
        ...
        if (modifiers & Qt.ShiftModifier) {
            let anchor = Array.from({length: rows.count}, (_, i) => rows.get(i).key).indexOf(anchorKey);
            if (anchor < 0) anchor = index;
            for (let i = Math.min(anchor, index); i <= Math.max(anchor, index); i++) selection[rows.get(i).key] = true;
        } else {
            if ((modifiers & Qt.ControlModifier) && selection[key]) delete selection[key];
    ...
    function selectAll() {
        const selection = {};
        for (const process of sortedRecords) selection[entryKey(process)] = true;
        selectedKeys = selection;
    }
    onSearchChanged: { selectedKeys = {}; list.positionViewAtBeginning(); }
    function recordAt(index) {
        const row = rows.get(index);
        return {pid: row.pid, startTime: row.startTime, name: row.name, executable: row.executable, cpuPercent: row.cpuPercent < 0 ? null : row.cpuPercent, memoryBytes: row.memoryBytes, threads: row.threads, state: row.state};
    }
    function refresh(forceOrder = false) {
        if (!rows || !list) return;
        const oldOffset = list.contentY - list.originY;
        const keys = Array.from({length: rows.count}, (_, index) => rows.get(index).key);
        let ordered = sortedRecords;
        if (holdOrder && !forceOrder) {
            const byKey = new Map(sortedRecords.map(process => [entryKey(process), process]));
            ordered = keys.filter(key => byKey.has(key)).map(key => { const process = byKey.get(key); byKey.delete(key); return process; }).concat(Array.from(byKey.values()));
        }
        for (let i = 0; i < ordered.length; i++) {
            ...
            if (i >= keys.length || keys[i] !== key) {
                ...
                else rows.insert(i, record);
                keys.splice(i, 0, key);
            }
            rows.set(i, record);
        }
        if (rows.count > ordered.length) rows.remove(ordered.length, rows.count - ordered.length);
    ...
    RowLayout {
        ...
        Text { visible: root.width >= 260; text: root.selectedRecords.length ? root.selectedRecords.length + " selected" : root.sortedRecords.length + " / " + root.totalCount; color: Theme.muted; font.family: Theme.fontFamily; font.pixelSize: Theme.fontSmall }
    }
    DataTable {
        id: table
        ...
        onKeyPressed: event => {
            if (event.key === Qt.Key_A && (event.modifiers & Qt.ControlModifier)) { root.selectAll(); event.accepted = true; return; }
            if (event.key === Qt.Key_Escape) { root.selectedKeys = {}; event.accepted = true; return; }
            ...
            else if (event.key === Qt.Key_End) next = list.count - 1;
            else return;
        ...
        delegate: Item {
            ...
            readonly property var desktopEntry: root.mode === "processes" ? root.applicationFor({name, executable}) : null
            width: root.tableWidth; height: root.rowHeight
            activeFocusOnTab: false
            function openMenu() { if (!root.selectedKeys[key]) root.selectIndex(index); root.contextRequested(row, root.selectedRecords); }
            Keys.onMenuPressed: openMenu()
            Keys.onPressed: event => {
                if (event.key === Qt.Key_F10 && (event.modifiers & Qt.ShiftModifier)) { openMenu(); event.accepted = true; }
            }
            TableRowSurface { anchors.fill: parent; selected: !!root.selectedKeys[row.key]; hovered: hover.hovered; focused: row.activeFocus }
            Row {
                height: parent.height
                Item {
                    width: root.columnWidths[0]; height: parent.height
                    OsIconImage { x: 6; anchors.verticalCenter: parent.verticalCenter; implicitSize: 18; source: Quickshell.iconPath(row.desktopEntry?.icon || (root.mode === "services" ? "system-run-symbolic" : "application-x-executable"), "application-x-executable") }
                    Column {
                        x: 30
                ...
                Cell { visible: root.mode === "processes"; width: root.columnWidths[1] ?? 0; text: row.cpuPercent >= 0 ? row.cpuPercent.toFixed(1) + "%" : "—" }
                Cell { visible: root.mode === "processes"; width: root.columnWidths[2] ?? 0; text: root.formatBytes(row.memoryBytes) }
                Cell { visible: root.mode === "processes"; width: root.columnWidths[3] ?? 0; text: String(row.pid) }
                Item {
            ...
            MouseArea {
                ...
                onPositionChanged: mouse => {
                    if (!pressed || !root.dragSelecting) return;
                ...
                onReleased: mouse => { root.dragSelecting = false; if (mouse.button === Qt.RightButton) row.openMenu(); }
