...
Item {
    ...
    onServiceChanged: {
        ...
        if (JSON.stringify(next) !== JSON.stringify(nodes)) nodes = next;
    }
    ...
    readonly property var filteredNodes: {
        ...
        let result = (tab === "exit" ? nodes.filter(node => !node.self && node.exitOption
            && (exitSource === "all" || (exitSource === "provider" ? !!node.provider : !node.provider))) : devices).filter(node =>
            !needle || [node.name, node.dns, node.os, node.country, node.city, node.provider].concat(node.ips).join(" ").toLocaleLowerCase().includes(needle));
        if (tab === "exit") result.sort((a, b) => Number(b.id === selectedExit) - Number(a.id === selectedExit)
            || Number(b.online) - Number(a.online) || (a.country || "").localeCompare(b.country || "")
    ...
    readonly property bool canApply: editable && pendingExit !== null && pendingExit.id !== selectedExit
        && (pendingExit.id === "" || nodes.some(node => node.id === pendingExit.id && node.online && node.exitOption))
    implicitHeight: 560
    onTabChanged: { nodeMenu.visible = false; nodeTable.contentX = 0; sortKey = ""; descending = false; query = ""; pendingExit = null; peers.positionViewAtBeginning(); }
    ...
    Keys.onEscapePressed: event => {
        if (pendingExit !== null) pendingExit = null;
        else if (query) query = "";
    ...
    onVisibleChanged: if (!visible) nodeMenu.visible = false
    TrayMenu {
        id: nodeMenu
        objectName: "tailscaleNodeMenu"
        hostItem: root.Window.window ? root.Window.window.contentItem : null
        actions: [{text: "Copy IP address", enabled: !!root.contextNode && root.contextNode.ips.length > 0,
            isSeparator: false, hasChildren: false, checkState: Qt.Unchecked,
            triggered: () => root.copyAddress(root.contextNode)}]
    }
    Timer { id: copiedFeedback; interval: 1800; onTriggered: root.copiedId = "" }
    readonly property var settingsOptions: [
        {id: "accept-dns", name: "Tailscale DNS", description: "Use DNS settings from your tailnet", icon: "network-server-symbolic"},
        {id: "accept-routes", name: "Subnet routes", description: "Reach networks shared by other nodes", icon: "network-wired-symbolic"},
        {id: "exit-node-allow-lan-access", name: "Local network access", description: "Keep LAN access while using an exit node", icon: "network-workgroup-symbolic"},
        {id: "shields-up", name: "Block incoming connections", description: "Reject incoming connections over Tailscale", icon: "security-high-symbolic"}
    ]
    ColumnLayout {
        anchors.fill: parent
        spacing: Theme.gap
        ...
        ControlRow {
            ...
            onToggleRequested: root.services.action({kind: "vpn", id: "tailscale", enabled: !root.service.connected})
        }
        ControlRow {
            visible: root.tab === "exit"
            ...
            onActionTriggered: root.pendingExit = {id: "", name: "Normal connection"}
        }
        RowLayout {
            ...
            FilterField {
                ...
                Keys.onDownPressed: { peers.forceActiveFocus(); peers.currentIndex = 0; }
            }
        }
        SegmentedControl {
        ...
        DataTable {
            ...
            onKeyPressed: event => {
                ...
                if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter) {
                    if (list.currentItem && list.currentItem.enabled) list.currentItem.clicked();
                } else {
            ...
            delegate: AbstractButton {
                ...
                contentItem: Item {
                    ...
                    Item {
                        ...
                        NodeCell { x: 30; width: parent.width - 30; text: nodeRow.title; color: nodeRow.enabled ? Theme.text : Theme.muted }
                    }
                    NodeCell {
                    ...
                    Item {
                        ...
                        Rectangle { x: 6; anchors.verticalCenter: parent.verticalCenter; width: 6; height: 6; radius: 3; color: parent.stateColor }
                        NodeCell { x: 12; width: parent.width - 12; color: parent.stateColor; text: nodeRow.activeExit ? "In use" : nodeRow.modelData.self ? "This PC" : nodeRow.modelData.online ? "Online" : "Offline" }
                    }
                    NodeCell { width: nodeTable.columnWidths[3]; text: root.tab === "exit" ? nodeRow.modelData.provider || "Tailnet" : nodeRow.modelData.os || "—"; color: Theme.muted }
                }
                    RowLayout {
                        anchors.fill: parent
                        ...
                        Rectangle {
                            ...
                            Rectangle { anchors.centerIn: parent; width: 8; height: 8; radius: 4; color: Theme.accent; visible: parent.chosen }
                        }
                    }
                }
                onClicked: {
                    root.peers.currentIndex = index;
                    if (root.tab === "exit" && !activeExit && enabled) root.pendingExit = modelData;
        ...
        RowLayout {
            ...
            ActionButton {
                ...
                onClicked: {
                    root.services.action({kind: "tailscale", setting: "exit-node", value: root.pendingExit.id});
            ...
            ColumnLayout {
                ...
                Repeater {
                    ...
                    ControlRow {
                        ...
                        onToggleRequested: root.services.action({kind: "tailscale", setting: modelData.id, enabled: !root.preferences[modelData.id]})
                    }
                }
                Text {
