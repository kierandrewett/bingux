...
def office_pdf(path):
    ...
    identity = hashlib.sha256(
        f"{path}:{stat.st_ino}:{stat.st_mtime_ns}:{stat.st_ctime_ns}:{stat.st_size}".encode()
    ).hexdigest()
    ...
    try:
        ...
        tables = [
            ...
            for row in connection.execute(
                "SELECT name FROM sqlite_schema WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name LIMIT 100"
            )
        ]
        ...
        return {
            ...
            "table": table,
            "columns": columns,
            "rows": [[cell(value) for value in row] for row in rows[:100]],
            "offset": offset,
    ...
    class ReaderHTML(HTMLParser):
        ...
        def handle_starttag(self, tag, attrs):
            ...
            for key, value in attrs:
                if key in {"colspan", "rowspan"} and value and value.isdigit():
                    ...
                    if parsed.scheme == "" and not parsed.netloc:
                        ...
                        if (
                            local.resolve().is_relative_to(path.parent.resolve())
                            and local.is_file()
                            and local.stat().st_size < 16 * 1024 * 1024
                        ):
            ...
            self.output.append(
                "<"
                + tag
                + "".join(" " + key + '="' + escape(value, quote=True) + '"' for key, value in attributes)
                + ">"
            )
