...
Item {
    ...
    function begin(id, source, window, position) {
        if (running || preparing || !DesktopEditing.active || !source) return;
        widgetId = id;
        preparing = true;
        ...
        const started = source.grabToImage(frame => {
            if (attempt !== generation || !preparing || !DesktopEditing.active) return;
            preparing = false;
            grabbedFrame = frame;
            ...
            editor.dragHotSpot = offset.x >= 0 && offset.y >= 0 && offset.x <= size.width && offset.y <= size.height
                ? offset : Qt.point(size.width / 2, size.height / 2);
            editor.dragGlobal(id, position);
            running = true;
            Drag.active = true;
        }, size);
        if (!started) preparing = false;
    }
    function cancelPending() { preparing = false; generation++; }
