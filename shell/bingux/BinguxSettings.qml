...
Window {
    ...
    function toggleSearch() {
        ...
        if (searchOpen) {
            navigationOpen = true;
            Qt.callLater(() => globalSearch.forceActiveFocus());
        } else {
            globalSearch.clear();
        }
    }
    ...
    readonly property var destinations: [
        {
            ...
            page: "Extensions",
            words: "plugins widgets addons enable disable reload"
        },
        {
            title: "Control Centre",
            description: "Control Centre",
            page: "Controls",
            words: "vpn tailscale bluetooth night light power awake notifications do not disturb"
        },
        {
            title: "Top Bar",
            description: "Desktop",
            page: "TopBar",
            words: "widgets icons labels status cpu memory"
        },
        {
            title: "Dock",
            description: "Desktop",
            page: "Dock",
            words: "applications pinned running icons alignment click scroll"
        },
        {
            title: "Sidebar",
            description: "Desktop",
            page: "Sidebar",
            words: "notes terminal calendar media position edge"
        },
        {
            title: "System monitors",
            description: "Desktop › Top Bar",
            page: "TopBar",
            target: "settingsMetrics",
            words: "cpu memory performance top bar"
        },
        {
            title: "Customise desktop",
            description: "Desktop",
            page: "Desktop",
            target: "customiseDesktop",
            words: "widgets layout control centre vpn bluetooth network"
        },
        {
            title: "Search results",
            description: "Search",
            page: "Providers",
            words: "applications files calculator conversions folders"
        },
        {
            title: "Search engines",
            description: "Search",
            page: "Engines",
            words: "websites shortcuts default duckduckgo custom"
        },
        {
            title: "AI Assistant",
            description: "Search",
            page: "AI",
            words: "pi claude model executable harness answers"
        },
        {
            title: "File previews",
            description: "Search",
            page: "Previews",
            words: "images documents pdf markdown video size limit prepare"
        },
        {
            title: "Dock alignment",
            description: "Desktop › Dock",
            page: "Dock",
            target: "settingsDockAlignment",
            words: "position left centre right"
        },
        {
            title: "Sidebar screen edge",
            description: "Desktop › Sidebar",
            page: "Sidebar",
            target: "settingsSidebarEdge",
            words: "position left right top"
        },
        {
            title: "Prepare nearby previews",
            description: "Search › File Previews",
            page: "Previews",
            target: "settingsPrewarm",
            words: "background fast preload cache"
        },
        {
            title: "Icon size",
            description: "Desktop › Dock",
            page: "Dock",
            target: "settingsDockSize",
            words: "small medium large"
        },
        {
            title: "Click a running app",
            description: "Desktop › Dock",
            page: "Dock",
            target: "settingsDockClick",
            words: "minimise focus launch"
        },
        {
            title: "Middle-click an app",
            description: "Desktop › Dock",
            page: "Dock",
            target: "settingsDockMiddle",
            words: "launch close mouse"
        },
        {
            title: "Scroll over the dock",
            description: "Desktop › Dock",
            page: "Dock",
            target: "settingsDockScroll",
            words: "cycle windows direction natural reverse"
        },
        {
            title: "Maximum file size",
            description: "Search › File Previews",
            page: "Previews",
            target: "settingsPreviewLimit",
            words: "megabytes mb limit large"
        }
    ]
    readonly property var indexedSettings: destinations.concat([
        {
            ...
            description: "Control Centre",
            page: "Controls",
            target: "settingsControlvpn",
            words: "tailscale connections exit node"
        },
        {
            title: "Do Not Disturb",
            description: "Control Centre",
            page: "Controls",
            target: "settingsControldnd",
            words: "notification banners pause"
        },
        {
            title: "Night Light",
            description: "Control Centre",
            page: "Controls",
            target: "settingsControlnightLight",
            words: "blue light display"
        },
        {
            title: "Power mode",
            description: "Control Centre",
            page: "Controls",
            target: "settingsControlpower",
            words: "battery performance"
        },
        {
            title: "Keep awake",
            description: "Control Centre",
            page: "Controls",
            target: "settingsControlawake",
            words: "sleep inhibit"
        },
        {
            ...
            description: "Desktop › Sidebar",
            page: "Sidebar",
            words: "notes tasks calendar terminal media system"
        }
    ])
    ...
    function openDestination(entry) {
        ...
        if (container && entry.target) {
            openContainerCustomise(container.id);
            return;
        }
        if (["Providers", "Engines"].includes(entry.page))
            searchSettings.goBack();
        focusDestination.targetName = entry.target || "";
        focusDestination.restart();
    }
    Timer {
        id: focusDestination
        ...
        onTriggered: {
            const item = targetName ? find(pageContent) : null;
            if (item) {
                item.forceActiveFocus(Qt.TabFocusReason);
                const point = item.mapToItem(pageContent, 0, 0);
                pageScroll.contentY = Math.max(0, Math.min(point.y - 24, pageScroll.contentHeight - pageScroll.height));
            }
        }
    }
    ...
    function undo() {
        ...
        autoSave.stop();
        let patch;
        if (Object.keys(pendingUndo).length) {
            patch = pendingUndo;
            pendingUndo = {};
        } else {
            patch = undoHistory[undoHistory.length - 1];
            undoHistory = undoHistory.slice(0, -1);
        }
        for (const section of Object.keys(patch))
            for (const key of Object.keys(patch[section]))
                update(section, key, patch[section][key], false);
        syncAI();
        save();
    }
    Timer {
        id: autoSave
        interval: 450
        onTriggered: if (root.dirty && !root.busy && !customiser.visible)
            root.save()
    }
    Timer {
        id: savedNotice
        interval: 1800
        onTriggered: if (root.status === "Saved")
            root.status = ""
    }

    property var requestedCustomise: null
    function openContainerCustomise(container) {
        if (!customiser.containerChoices.some(item => item.id === container))
            return;
        if (!shellHosted) {
            requestShellCustomise(container);
            return;
        }
        requestedCustomise = {
            widgetId: "",
    ...
    function showCustomiser() {
        ...
        if (request.options === "Remove")
            customiser.put(request.widgetId, "palette", 0);
        else
            customiser.optionsPage = request.options;
        if (request.container)
            customiser.selectContainer(request.container);
        else if (!request.widgetId)
            customiser.selectedContainer = "control-centre";
    }
    readonly property bool busy: operation !== ""
    function update(section, key, value, recordUndo = true) {
        if (JSON.stringify(draft[section][key]) === JSON.stringify(value))
            return;
        if (recordUndo && !(key in (pendingUndo[section] || {})))
            pendingUndo = Object.assign({}, pendingUndo, {
                [section]: Object.assign({}, pendingUndo[section] || {}, {
                    [key]: JSON.parse(JSON.stringify(draft[section][key]))
                })
            });
        const next = JSON.parse(JSON.stringify(draft));
        next[section][key] = value;
        draft = next;
        changedSettings = Object.assign({}, changedSettings, {
            [section]: Object.assign({}, changedSettings[section] || {}, {
                [key]: value
            })
        });
        dirty = true;
    ...
    function read() {
        ...
        operation = "read";
        backend.command = helper.concat(["read"]);
    ...
    function save() {
        ...
        if (Object.keys(pendingUndo).length)
            undoHistory = undoHistory.concat([pendingUndo]).slice(-30);
        pendingUndo = {};
        changedSettings = {};
        dirty = false;
        receivedReply = false;
    ...
    onVisibleChanged: if (visible && !dirty && !searchSettings.editing)
        read()
    Component.onCompleted: Qt.callLater(() => {
        if (visible && !ready && !busy)
            read();
    })
    Shortcut {
        sequence: "Ctrl+S"
        enabled: root.visible && root.dirty && !root.busy && !searchSettings.editing
    ...
    Shortcut {
        sequence: "Alt+Left"
        enabled: root.visible && root.canGoBack
        onActivated: root.goBack()
    }
    Shortcut {
        sequence: "Escape"
        enabled: root.visible && (root.navigationOpen || root.canGoBack || root.searchOpen)
        onActivated: {
            if (root.searchOpen) {
                root.searchOpen = false;
                globalSearch.clear();
            } else if (root.navigationOpen)
    ...
    Process {
        id: backend
        stdinEnabled: true
        onStarted: if (command[command.length - 1] === "save") {
            write(JSON.stringify(root.submittedDraft));
            stdinEnabled = false;
        }
        onExited: {
            stdinEnabled = true;
            if (!root.receivedReply)
                root.status = "Settings could not be " + (root.operation === "save" ? "saved. Your changes are still here." : "loaded. Try again.");
            if (!root.replySucceeded && root.operation === "save") {
                for (const section of Object.keys(root.submittedDraft))
                    root.changedSettings = Object.assign({}, root.changedSettings, {
                        [section]: Object.assign({}, root.submittedDraft[section], root.changedSettings[section] || {})
                    });
                root.dirty = true;
            }
            root.operation = "";
            if (root.replySucceeded && root.dirty)
                autoSave.restart();
        }
        stdout: SplitParser {
            onRead: line => {
                ...
                if (result.error) {
                    root.status = result.error;
                    return;
                }
                root.ready = true;
                root.replySucceeded = true;
                const merged = JSON.parse(JSON.stringify(result.data));
                for (const section of Object.keys(root.changedSettings))
                    Object.assign(merged[section], root.changedSettings[section]);
                root.draft = merged;
                root.dirty = Object.keys(root.changedSettings).length > 0;
                if (root.operation === "read")
                    root.syncAI();
                if (result.harnesses)
                    root.harnesses = result.harnesses;
                if (result.desktopDefaults)
                    root.desktopDefaults = result.desktopDefaults;
                ...
                if (root.status === "Saved")
                    savedNotice.restart();
                if (backend.command[backend.command.length - 1] === "save")
                    root.saved();
                if (root.requestedCustomise)
                    Qt.callLater(root.showCustomiser);
            }
        }
    }
    Process {
        ...
        stderr: StdioCollector {
            onStreamFinished: if (text.trim())
                console.warn("Customise IPC:", text.trim())
        }
        onExited: code => {
            if (code !== 0)
                root.status = "Could not open desktop customisation.";
            else
                root.visible = false;
        }
    }
    ...
    onBusyChanged: if (!busy && shellCustomisePending) {
        if (ready && !dirty)
            Qt.callLater(root.dispatchShellCustomise);
        else
            shellCustomisePending = false;
    }
    DesktopCustomise {
        id: standaloneCustomiser
        settings: root
        screen: Quickshell.screens.find(s => s.name === root.screen.name) || Quickshell.screens[0]
    }
    ...
    onPageChanged: {
        navigationOpen = false;
        if (["Providers", "Engines"].includes(page)) {
            searchSettings.goBack();
    ...
    MultiEffect {
        ...
        shadowEnabled: true
        shadowBlur: 0.65
        shadowOpacity: root.active ? 0.3 : 0.16
                ...
                IconButton {
                    ...
                    onClicked: {
                        if (root.canGoBack)
                            ...
                            root.navigationOpen = !root.navigationOpen;
                    }
                }
                RowLayout {
                    id: headerActions
                    anchors.right: parent.right
                    anchors.rightMargin: windowControls.width + 19
                    anchors.verticalCenter: parent.verticalCenter
                ...
                ColumnLayout {
                    ...
                    ColumnLayout {
                        ...
                        Section {
                            ...
                            Group {
                                SettingsRow {
                                    title: "Search results"
                                    subtitle: "Applications, files, calculations and conversions"
                                    navigation: true
                                    onClicked: root.page = "Providers"
                                }
                                SettingsRow {
                                    title: "Search engines"
                                    subtitle: "Default website search and custom shortcuts"
                                    navigation: true
                                    onClicked: root.page = "Engines"
                                }
                                SettingsRow {
                                    title: "AI Assistant"
                                    subtitle: "Ask questions with ! using your CLI provider"
                                    valueText: root.aiEnabled ? "On" : "Off"
                                    navigation: true
                                    onClicked: root.page = "AI"
                                }
                                SettingsRow {
                                    title: "File previews"
                                    subtitle: "Preview documents, images and media"
                                    valueText: root.draft.previews.enabled ? "On" : "Off"
                                    navigation: true
                                    onClicked: root.page = "Previews"
                                }
                            }
                        }
                    }
                    ...
                    ColumnLayout {
                        ...
                        Section {
                            ...
                            Group {
                                PreferenceRow {
                                    objectName: "settingsAIEnabled"
                                    title: "AI Assistant"
                                    subtitle: "Start a search with ! to ask a question"
                                    toggleChecked: root.aiEnabled
                                    onToggleRequested: {
                                        root.aiEnabled = !root.aiEnabled;
                                        root.editAI();
                                    }
                                }
                            }
                            Caption {
                                text: "Press Enter to send. Only ! questions are sent to your AI provider."
                        ...
                        Section {
                            ...
                            Group {
                                EntryRow {
                                    label: "Model"
                                    placeholderText: "Use the provider default"
                                    text: root.model
                                    onEdited: value => {
                                        root.model = value;
                                        root.editAI();
                                ...
                                SettingsRow {
                                    ...
                                    navigationRotation: root.advancedOpen ? 90 : 0
                                    iconName: ""
                                    navigation: true
                                ...
                                SettingsReveal {
                                    ...
                                    EntryRow {
                                        objectName: "settingsExecutable"
                                        label: "Executable path"
                                        placeholderText: "Detect automatically"
                                        text: root.executable
                                        onEdited: value => {
                                            root.executable = value;
                                            root.editAI();
                                        }
                                    }
                                }
                        ...
                        Caption {
                            text: "Esc stops a reply. Tools and project access are disabled."
                            Layout.leftMargin: Theme.spaceSmall
                        }
                    }
                    ColumnLayout {
                        ...
                        Section {
                            ...
                            Group {
                                PreferenceRow {
                                    title: "Show previews"
                                    subtitle: "Press Right Arrow on a selected file"
                                    toggleChecked: root.draft.previews.enabled
                                    onToggleRequested: root.update("previews", "enabled", !toggleChecked)
                                }
                                PreferenceRow {
                                    objectName: "settingsPrewarm"
                                    enabled: root.draft.previews.enabled
                                    title: "Prepare nearby previews"
                                    subtitle: "Make nearby results faster to open"
                                    toggleChecked: root.draft.previews.prewarm
                                    onToggleRequested: root.update("previews", "prewarm", !toggleChecked)
                                }
                            }
                        }
                        Section {
                            ...
                            Group {
                                RowLayout {
                                    ...
                                    IconButton {
                                        ...
                                        label: "Decrease preview size limit"
                                        enabled: root.draft.previews.maxMegabytes > 1
                                        onClicked: root.update("previews", "maxMegabytes", root.draft.previews.maxMegabytes - 1)
                                    }
                                    TextField {
                                        ...
                                        font.pixelSize: Theme.fontSize
                                        selectByMouse: true
                                        Accessible.name: "Maximum preview size in megabytes"
                                        ...
                                        background: Rectangle {
                                            radius: 6
                                            color: parent.activeFocus ? Theme.elevated : "transparent"
                                            border.width: parent.activeFocus ? 1 : 0
                                            border.color: Theme.accent
                                        }
                                        onEditingFinished: {
                                            if (acceptableInput)
                                                root.update("previews", "maxMegabytes", parseInt(text));
                                    ...
                                    IconButton {
                                        ...
                                        label: "Increase preview size limit"
                                        enabled: root.draft.previews.maxMegabytes < 20
                                        onClicked: root.update("previews", "maxMegabytes", root.draft.previews.maxMegabytes + 1)
                                    }
                                }
                            }
                            Caption {
                                text: "Larger files will not be previewed. The maximum is 20 MB."
                                Layout.leftMargin: Theme.spaceSmall
                            }
                        }
                        Section {
                        ...
                        Group {
                            SettingsRow {
                                objectName: "settingsSupportedFormats"
                                title: "Supported file types"
                                iconName: ""
                                navigation: true
                                navigationRotation: root.formatsOpen ? 90 : 0
                                onClicked: root.formatsOpen = !root.formatsOpen
                            }
                            SettingsReveal {
                                expanded: root.formatsOpen
                                Caption {
                                    Layout.margins: Theme.padding
                                    text: "Images, video and audio\nPDFs and Office documents\nMarkdown, HTML and source code\nTables, email, archives and SQLite databases"
                                    lineHeight: 1.5
                                }
                            }
                        }
            ...
            ColumnLayout {
                ...
                Item {
                    ...
                    FilterField {
                        ...
                        onAccepted: if (root.searchMatches.length && root.searching)
                            root.openDestination(root.searchMatches[0])
                        Keys.onEscapePressed: {
                            root.searchOpen = false;
                            clear();
                        }
                        Keys.onDownPressed: if (resultItems.count && root.searching)
                            resultItems.itemAt(0).forceActiveFocus()
                    }
                }
                Flickable {
                    Layout.fillWidth: true
                    Layout.preferredHeight: Math.max(0, navigation.height - navigationContent.y - (root.searchOpen ? 48 : 2))
                    contentHeight: sidebarRows.implicitHeight + 12
                    clip: true
                    boundsBehavior: Flickable.StopAtBounds
                    ScrollBar.vertical: ScrollBar {
                        policy: ScrollBar.AsNeeded
                            ...
                            SettingsNavigationRow {
                                ...
                                Keys.onPressed: event => {
                                    if (event.key === Qt.Key_Up && index === 0)
                                        ...
                                        resultItems.itemAt(Math.max(0, Math.min(resultItems.count - 1, index + (event.key === Qt.Key_Down ? 1 : -1)))).forceActiveFocus();
                                    else
                                        return;
                                    event.accepted = true;
                                }
    ...
    Repeater {
        model: [Qt.LeftEdge, Qt.RightEdge, Qt.TopEdge, Qt.BottomEdge, Qt.TopEdge | Qt.LeftEdge, Qt.TopEdge | Qt.RightEdge, Qt.BottomEdge | Qt.LeftEdge, Qt.BottomEdge | Qt.RightEdge]
        MouseArea {
