...
MouseArea {
    ...
    readonly property var expandedEntries: {
        ...
        for (const entry of entries)
            for (const member of (entry.item?.memberEntries || []).concat([entry]))
                if (!result.some(other => other.id === member.id))
                    result.push(member);
        return result;
    }
        ...
        actions: [
            {
                ...
                enabled: pinned || !!DesktopEditing.editor?.appEntry(widgetId.slice(4)),
                triggered: () => {
            ...
            },
            {
                ...
                enabled: true,
                triggered: () => {
                    DesktopEditing.editor.selectedWidget = widgetId;
                    DesktopEditing.editor.optionsPage = "Widget";
                }
            },
            {
                ...
                triggered: () => {
                    DesktopEditing.editor.selectedWidget = widgetId;
                    DesktopEditing.editor.optionsPage = "Move";
                }
            }
    ...
    readonly property rect screenRect: {
        ...
        if (!DesktopEditing.active || !window || !("anchors" in window))
    ...
    function destinationAt(point, id) {
        const group = entries.find(entry => {
            if (!entry.item?.memberEntries || !entry.item.visible || !DesktopEditing.editor.accepts(id, entry.id))
                return false;
    ...
    function insertionIndex(point, id, target = zoneName) {
        ...
        const items = expandedEntries.filter(entry => order.includes(entry.id) && entry.id !== id && entry.item?.visible && (zoneName !== "dock" || entry.id.startsWith("app:") === id.startsWith("app:"))).map(entry => ({
                    entry,
                    p: DesktopEditing.point(entry.item, window, 0, 0)
    ...
    function beginNativeDrag(id, item, position) {
        nativeDrag.begin(id, item, window, position);
    }
    function cancelNativeDrag() {
        nativeDrag.cancelPending();
    }
        ...
        Item {
            ...
            MouseArea {
                ...
                onPositionChanged: mouse => {
                    if (!(pressedButtons & Qt.LeftButton) || moved || Math.abs(mouse.x - pressPoint.x) + Math.abs(mouse.y - pressPoint.y) <= 6)
                        return;
                    moved = true;
                    root.hadDrag = true;
                    root.beginNativeDrag(modelData.id, modelData.item, DesktopEditing.point(entryDragMouse, root.window, mouse.x, mouse.y));
                }
                onReleased: mouse => {
                    mouse.accepted = true;
                    if (!moved) {
                        if (mouse.button === Qt.RightButton)
                            root.inspect(modelData.id);
                        else {
                            const target = modelData.item;
                            if (typeof target?.activateInEditor === "function")
                                target.activateInEditor();
                            else if (modelData.item?.memberEntries)
                                DesktopEditing.editor.selectContainer(modelData.id);
                            else if (root.sourceOnly)
                                DesktopEditing.editor.selectContainer(DesktopEditing.editor.containerFor(modelData.id));
                            else
                                DesktopEditing.editor.optionsPage = "Widget", DesktopEditing.editor.selectedWidget = modelData.id;
                        }
                    }
                    root.pressedId = "";
                    root.hadDrag = false;
    ...
    Repeater {
        ...
        Item {
            ...
            Rectangle {
                ...
                height: 4
                radius: 2
                color: gripHover.hovered ? Theme.accent : Theme.muted
            }
            HoverHandler {
                id: gripHover
            ...
            ShellTooltip {
                parent: grip
                visible: DesktopEditing.active && gripHover.hovered
                text: "Drag " + (DesktopLayout.widget(grip.modelData.id)?.label || "group")
            }
            MouseArea {
                id: groupGripMouse
                anchors.fill: parent
                ...
                Timer {
                    ...
                    onTriggered: {
                        if (!(groupGripMouse.pressedButtons & Qt.LeftButton) || groupGripMouse.moved)
                            return;
                        const surface = grip.parent;
                        groupGripMouse.moved = true;
                        surface.pressedId = modelData.id;
                        surface.hadDrag = true;
                        surface.beginNativeDrag(modelData.id, modelData.item, DesktopEditing.point(grip, surface.window, grip.width / 2, grip.height / 2));
                    }
                }
                onPressed: mouse => {
                    mouse.accepted = true;
                ...
                onPositionChanged: mouse => {
                    ...
                    surface.pressedId = modelData.id;
                    surface.hadDrag = true;
                    surface.beginNativeDrag(modelData.id, modelData.item, DesktopEditing.point(grip, surface.window, mouse.x, mouse.y));
                }
                onReleased: mouse => {
                    mouse.accepted = true;
                    const surface = grip.parent;
    ...
    onReleased: mouse => {
        ...
        if (mouse.button === Qt.RightButton && pressedId) {
            ...
            if (typeof target?.activateInEditor === "function")
                target.activateInEditor();
            else if (pressedEntry?.item?.memberEntries)
                editor.selectContainer(pressedEntry.id);
            else if (sourceOnly)
                editor.selectContainer(editor.containerFor(pressedId || zoneName));
            else
                editor.selectContainer(entries.find(entry => entry.item?.memberEntries && (entry.id === pressedId || entry.item.memberEntries.some(member => member.id === pressedId)))?.id || zoneName);
        }
        pressedId = "";
    }
    onCanceled: {
    ...
    WidgetDropArea {
        enabled: DesktopEditing.active && !root.sourceOnly
        anchors.fill: parent
    ...
    readonly property rect insertionRect: {
        ...
        if (!item)
            return vertical ? Qt.rect(6, 6, width - 12, 3) : Qt.rect(6, 5, 3, height - 10);
        DesktopEditing.observeGeometry(item);
        const p = item.mapToItem(root, 0, 0);
        return vertical ? Qt.rect(Math.max(4, p.x), p.y + (next ? 0 : item.height) - 2, Math.min(width - 8, item.width), 3) : Qt.rect(p.x + (next ? 0 : item.width) - 2, 5, 3, height - 10);
    }
    Rectangle {
        ...
        width: Math.max(0, root.insertionRect.width)
        height: Math.max(0, root.insertionRect.height)
        radius: 1.5
        color: Theme.accent
    }
    Rectangle {
        ...
        radius: typeof root.geometryItem.radius === "number" ? root.geometryItem.radius : typeof root.parent.radius === "number" ? root.parent.radius : 0
        color: "transparent"
        border.width: 1
