...
Pill {
    ...
    readonly property real memoryFraction: sample && sample.memoryTotalBytes > 0 ? Math.max(0, Math.min(1, sample.memoryUsedBytes / sample.memoryTotalBytes)) : 0
    ...
    Timer {
        ...
        onTriggered: root.chartTime = Date.now()
    }
    ...
    function isShown(name) {
        return monitorNames.includes(name) && preferences[name] === true;
    }
    function setShown(name, shown) {
        if (!monitorNames.includes(name) || (!shown && isShown(name) && selectedNames.length <= 1))
            return;
    ...
    function numericFor(name, point) {
        if (point)
            return name === "memory" ? point.memoryUsed : name === "swap" ? point.swapUsed : point[name];
        if (!available || !sample)
            return null;
        ...
        return {
            ...
            temperature: extra.cpuTemperatureCelsius,
            load: extra.load1,
            swap: extra.swapUsedBytes,
            diskRead: extra.diskReadBytesPerSecond,
            diskWrite: extra.diskWriteBytesPerSecond
        }[name];
    }
    function valueFor(name, point) {
        ...
        if (name === "memory" || name === "swap")
            return name === "swap" && (point ? point.swapTotal === 0 : sample.extra.swapTotalBytes === 0) ? "Off" : systemMetrics.formatBytes(value);
        return systemMetrics.formatRate(value);
    }
    function supported(name) {
        return !["temperature", "load", "swap", "diskRead", "diskWrite"].includes(name) || typeof numericFor(name) === "number";
    }
    function maximumFor(name, peak) {
        if (["cpu", "memory", "swap"].includes(name))
            return 100;
        if (name === "temperature")
            return 120;
        if (name === "load")
    ...
    GridLayout {
        ...
        readonly property real barImplicitWidth: {
            ...
            for (let index = 0; index < metricsRepeater.count; index += 2)
                width += Math.ceil(Math.max(metricsRepeater.itemAt(index)?.implicitWidth || 0, metricsRepeater.itemAt(index + 1)?.implicitWidth || 0));
        ...
        columns: root.panelLayout ? Math.max(1, Math.floor((root.width - root.horizontalPadding * 2 + columnSpacing) / (widestReadout + columnSpacing))) : -1
