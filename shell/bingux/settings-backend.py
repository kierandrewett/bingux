...
DEFAULTS = {
    ...
    "desktop": {
        ...
        "dockClick": "toggle",
        "dockMiddleClick": "launch",
        "dockScroll": "cycle",
        "dockScrollDirection": "natural",
        "sidebarEdge": None,
        "layoutVersion": 0,
        "dockApps": None,
        "controlCentre": None,
...
def is_extension_widget(item):
    return isinstance(item, str) and bool(
        re.fullmatch(r"extension:[a-z0-9][a-z0-9._-]{0,127}/[a-z0-9][a-z0-9._-]{0,127}", item)
    )


def is_external_control(item):
    return isinstance(item, str) and (
        is_extension_widget(item)
        or item in EXTERNAL_CONTROL_IDS
        or bool(re.fullmatch(r"(label|icon):[1-9][0-9]{0,3}", item))
    )


CONTROL_ACTIONS = {"control-account", "control-settings", "control-session", "control-lock"}

PORTABLE_CONTROLS = CONTROL_ACTIONS | {
    "control-volume",
    "control-microphone",
    "control-battery",
    "control-media",
    ...
    for group, items in groups.items():
        if (
            ...
            or any(
                ...
                or (item not in CONTROL_GROUPS[group] and not (group == "control-centre" and is_external_control(item)))
                for item in items
            )
...
def validate_desktop(desktop, previous=None):
    ...
    if control_order is not None and (
        not isinstance(control_order, list)
        or any(
            not isinstance(item, str)
            or item not in {"network", "bluetooth", "vpn", "dnd", "nightLight", "power", "awake"}
            for item in control_order
        )
    ...
    if controls is not None and (
        ...
        or set(controls) != {"vpn", "dnd", "nightLight", "power", "awake"}
    ...
    for key, options in desktop["containers"].items() if isinstance(desktop["containers"], dict) else [(None, None)]:
        if (
            ...
            or set(options) != {"display"}
            or options["display"] not in modes
        ):
    ...
    for key, options in desktop["widgetOptions"].items():
        if (
            ...
            or not isinstance(options, dict)
            or set(options) - {"display", "label", "icon", "width"}
        ):
            raise ValueError("Invalid widget display options.")
        if "width" in options and (
            not (key == "control-header-space" or re.fullmatch(r"spacer:[1-9][0-9]{0,3}", key))
            or type(options["width"]) is not int
            or not 8 <= options["width"] <= 160
        ):
    ...
    for key, choices in {
        ...
        "dockClick": ("toggle", "focus", "launch"),
        "dockMiddleClick": ("launch", "close", "none"),
        "dockScroll": ("cycle", "none"),
        "dockScrollDirection": ("natural", "reverse"),
        "sidebarEdge": (None, "left", "right", "top"),
    }.items():
        if desktop[key] not in choices:
            raise ValueError("Invalid desktop behaviour.")
    if type(desktop["dockSize"]) is not int or not 32 <= desktop["dockSize"] <= 80:
        raise ValueError("Dock icons must be between 32 and 80 pixels.")
    layout = desktop["layout"]
    if layout is not None:
        zones = {"top-left", "top-center", "top-right", "dock", "sidebar"}
        widgets = EXTERNAL_CONTROL_IDS
        controls = {
            "control-" + name for name in ("network", "bluetooth", "vpn", "dnd", "nightLight", "power", "awake")
        }
        if not isinstance(layout, dict) or set(layout) != zones:
            raise ValueError("Invalid desktop layout.")
        seen = set()
        for zone, items in layout.items():
            ...
            if len(items) != len(set(items)):
                raise ValueError("A widget can only be placed once.")
            seen.update(items)
...
def validate(data):
    ...
    if not isinstance(disabled, list) or any(p not in PROVIDERS for p in disabled):
        raise ValueError("Unknown search provider.")
    roots = search["fileRoots"]
    if roots is not None and (
        not isinstance(roots, list)
        or len(roots) > 32
        or any(not isinstance(p, str) or not Path(p).is_absolute() or "\0" in p for p in roots)
    ):
        raise ValueError("Use absolute paths for search locations, with at most 32 locations.")
    engines = search["engines"]
    if not isinstance(engines, list) or not 1 <= len(engines) <= 32:
        raise ValueError("Add between 1 and 32 search engines.")
    ids, shortcuts = set(), set()
    for engine in engines:
        if not isinstance(engine, dict) or set(engine) != {"id", "name", "shortcut", "url", "enabled"}:
            raise ValueError("Invalid search engine.")
        for field in ("id", "name", "shortcut", "url"):
            if not isinstance(engine[field], str) or any(ord(c) < 32 for c in engine[field]):
                raise ValueError("Invalid search engine text.")
        if not re.fullmatch(r"[a-z0-9][a-z0-9-]{0,63}", engine["id"]) or engine["id"] in ids:
            raise ValueError("Search engine identifiers must be unique.")
        if (
            not re.fullmatch(r"[a-z0-9][a-z0-9-]{0,23}", engine["shortcut"])
            or engine["shortcut"] in shortcuts
            or engine["shortcut"] in ("wiki", "gh", "maps", "yt")
        ):
            raise ValueError(
                "Choose a unique shortcut with letters, numbers or hyphens. wiki, gh, maps and yt are reserved."
            )
        if not engine["name"].strip() or len(engine["name"]) > 80:
        ...
        if (
            ...
            or url.username
            or url.password
            or len(engine["url"]) > 2048
            or engine["url"].count("{query}") != 1
            or "{query}" in url.netloc
        ):
            raise ValueError("Use an HTTP or HTTPS URL with {query} in its path or query.")
        if type(engine["enabled"]) is not bool:
            raise ValueError("Invalid engine toggle.")
        ids.add(engine["id"])
        shortcuts.add(engine["shortcut"])
    if not any(e["id"] == search["defaultEngine"] and e["enabled"] for e in engines):
        raise ValueError("The default search engine must be enabled.")
    validate_desktop(result["desktop"], current["desktop"])
    ai = search["ai"]
    if ai is not None:
        if not isinstance(ai, dict) or set(ai) - {"harness", "executable", "model"}:
            raise ValueError("Invalid AI settings.")
        if ai.get("harness") not in ("pi", "claude"):
            raise ValueError("Choose Pi or Claude Code.")
        model = ai.get("model", "")
        ...
        if not executable or not Path(executable).is_absolute() or not os.access(executable, os.X_OK):
            raise ValueError("The selected CLI is not installed or executable.")
        ai["executable"] = executable
    for section, fields in [("previews", ("enabled", "prewarm"))]:
        if any(type(result[section][key]) is not bool for key in fields):
            raise ValueError("Invalid toggle value.")
    limit = result["previews"]["maxMegabytes"]
    if type(limit) is not int or not 1 <= limit <= 20:
        raise ValueError("Preview limit must be between 1 and 20 MB.")
    return result
...
def import_layout(snapshot):
    ...
    with (path.parent / "settings.lock").open("a") as lock:
        ...
        if (
            ...
            or snapshot.get("controlCentreReady") is not True
        ):
        ...
        if not already_imported:
            desktop.update(
                layoutVersion=1,
                layout=desktop["layout"] or snapshot["layout"],
        ...
        backup = path.parent / (
            "control-layout-before-import.json" if already_imported else "layout-before-import.json"
        )
...
def wallpaper():
    for key in ("picture-uri-dark", "picture-uri"):
        reply = subprocess.run(
            ["gsettings", "get", "org.gnome.desktop.background", key], capture_output=True, text=True, timeout=3
        )
        ...
        fingerprint = hashlib.sha256(
            f"{source}:{source.stat().st_mtime_ns}:{source.stat().st_size}".encode()
        ).hexdigest()
        ...
        if sys.argv[1:] == ["wallpaper"]:
            ...
            if before["search"] != data["search"]:
                result = subprocess.run(
                    ["systemctl", "--user", "try-restart", "bingux-searchd.service"], capture_output=True, timeout=8
                )
                if result.returncode:
                    warning = "Saved. Restart the search service to apply search settings."
            ...
            print(
                json.dumps(
                    {
                        "data": read(),
                        "desktopDefaults": DEFAULTS["desktop"],
