...
def serve(path):
    ...
    backend = subprocess.Popen([sys.executable, '-u', str(Path(__file__).with_name('capture_backend.py'))],
        ...
        while True:
            for key, _ in selector.select(1):
                if key.fileobj is listener:
                    ...
                    if not chunk:
                        error = json.dumps({'event': 'error', 'message': 'The recording worker stopped unexpectedly. Check the capture service log.'}).encode() + b'\n'
                    ...
                    while b'\n' in pending:
                        ...
                        if kind == 'ready':
                            ready = payload
                        elif kind in ('countdown', 'starting', 'recording', 'finalizing', 'saved', 'error', 'cancelled'):
                            state = payload
    ...
    with path.with_suffix('.lock').open('a') as lock:
        ...
        try:
            ...
            with path.with_suffix('.log').open('ab') as log:
                subprocess.Popen([sys.executable, '-u', __file__, '--serve', str(path)],
