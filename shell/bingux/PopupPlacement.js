...
function desktopCaret(caret, context) {
    ...
    const surface = caret.surface, buffer = context.buffer;
    if (surface && buffer && surface.width > 0 && surface.height > 0) {
        const sx = buffer.width / surface.width, sy = buffer.height / surface.height;
        ...
        if (sx >= 0.5 && sx <= 4 && Math.abs(sx - sy) < 0.02)
            return {x: buffer.x + (caret.x - surface.x) * sx,
                y: buffer.y + (caret.y - surface.y) * sy,
                height: caret.height * sy, source: "caret"};
    }
    return caret;
}
...
function place(anchor, screen, width, height, margin, gap) {
    ...
    const right = screen.width - margin, bottom = screen.height - margin;
    const x = anchor.x - screen.x, y = anchor.y - screen.y;
