...
from preview_limits import MAX_FILE_BYTES as MAX_FILE_BYTES, check_file
...
from gi.repository import GdkPixbuf, Gio, Poppler  # noqa: E402 - Select GI versions before importing their modules.
...
def image_metadata(path):
    ...
    try:
        with Image.open(path) as image:
            ...
            if profile:
                try:
                    result["colourProfile"] = ImageCms.getProfileDescription(
...
def metadata(path):
    ...
    info = Gio.File.new_for_path(str(path)).query_info(
        "standard::content-type,time::created", Gio.FileQueryInfoFlags.NONE, None
    )
    ...
    if info.has_attribute("time::created") and info.get_attribute_uint64("time::created") > 0:
        common["file"]["created"] = datetime.fromtimestamp(
            info.get_attribute_uint64("time::created"), timezone.utc
        ).isoformat()
    if kind == "extra":
    ...
    if result["format"] == "plain" and path.suffix.lower() == ".json" and not result["truncated"]:
        try:
            result["text"] = json.dumps(json.loads(result["text"]), indent=2, ensure_ascii=False)
        except ValueError:
            pass
...
def render(path, page_number, requested_width):
    ...
    if kind == "pdf":
        ...
        surface = cairo.ImageSurface(
            cairo.FORMAT_ARGB32, max(1, math.ceil(page_width * scale)), max(1, math.ceil(page_height * scale))
        )
