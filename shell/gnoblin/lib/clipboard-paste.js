...
export class ClipboardPaste {
    ...
    async paste(text, checkFocus) {
        ...
        try {
            checkFocus();
            const helper = GLib.find_program_in_path("bingux-clipboard-paste");
            if (!helper) throw new Error("bingux-clipboard-paste is not installed on PATH.");
            const launcher = new Gio.SubprocessLauncher({
                flags: Gio.SubprocessFlags.STDIN_PIPE | Gio.SubprocessFlags.STDOUT_PIPE,
            });
            launcher.setenv("GDK_BACKEND", "x11", true);
            child = launcher.spawnv([helper]);
