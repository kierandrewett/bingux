...
def parser():
    ...
    result.add_argument(
        "--any-display",
        action="store_true",
        default=os.environ.get("BINGUX_ANY_DISPLAY") == "1",
        help="Match the selected shell across display connections",
    )
    ...
    capture.add_argument(
        ...
        nargs="?",
        default="open",
        choices=["open", "toggle", "take", "stop", "cancel", "status", "configure", "options"],
    )
    ...
    audio.add_argument(
        ...
        nargs="?",
        default="status",
        choices=["status", "volume", "up", "down", "mute", "unmute", "toggle", "devices", "select"],
    )
    ...
    for name, actions in (
        ("network", ["status", "list", "refresh", "connect", "disconnect"]),
    ...
    media.add_argument(
        ...
        nargs="?",
        default="status",
        choices=["list", "status", "play", "pause", "toggle", "next", "previous", "seek"],
    )
    ...
    for name, actions in (
        ("dock", ["list", "pin", "unpin", "move", "launch", "activate"]),
        ...
        if name == "dock":
            control.add_argument("position", nargs="?", type=int)
    ...
    for name in ("settings", "calendar", "controls", "metrics", "keyboard", "notifications"):
        ...
        if name == "controls":
            actions += ["page", "list", "show", "hide"]
        if name == "keyboard":
            actions += ["next", "previous", "list", "select"]
        if name == "notifications":
            actions += ["list", "dismiss", "clear", "invoke"]
    ...
    sidebar.add_argument(
        ...
        nargs="?",
        default="toggle",
        choices=["open", "close", "toggle", "popout", "dock", "select", "edge", "status"],
    )
...
def invocation(args, cli):
    ...
    if command == "list":
        return ["show"]
    if command in ("status", "reload"):
        call = ["shell", command]
    elif command == "capture":
        options = {
            key: getattr(args, key)
            for key in ("fps", "delay", "quality", "audio", "format", "encoder", "backend", "cursor", "copy")
            if getattr(args, key) is not None
        }
        if args.mode:
            options["kind"] = args.mode
        if args.target:
            options["target"] = args.target
        if args.max_height is not None:
            options["maxHeight"] = args.max_height
        if args.output is not None:
            options["directory"] = str(Path(args.output).expanduser().absolute())
        if args.region is not None:
            if min(args.region[:2]) < 0 or min(args.region[2:]) < 2:
                cli.error("Invalid capture region")
            options["region"] = dict(zip(("x", "y", "width", "height"), args.region))
        if action not in ("open", "configure") and options:
            cli.error("Capture options require open or configure")
        if action == "configure":
            if not options:
                cli.error("capture configure requires options; use capture options to inspect them")
            ...
            call = (
                ["capture", "show", args.mode, args.target]
                if action == "open"
                else ["capture", {"toggle": "open"}.get(action, action)]
            )
    elif command == "apps":
        if action == "launch" and not args.value:
            cli.error("apps launch requires a desktop ID from apps list")
        if args.new_window and action != "launch":
            cli.error("--new-window requires apps launch")
        call = [
            "actions",
            "app",
            "new-window" if args.new_window else action,
            (args.value or "") if action == "launch" else "",
            (args.value or "") if action == "list" else "",
        ]
    elif command == "controls":
        needs_value = action in ("page", "show", "hide")
        if needs_value != (args.value is not None):
            cli.error("This controls action " + ("requires a value" if needs_value else "takes no value"))
        if args.input and (action != "page" or args.value != "audio"):
            cli.error("--input requires controls page audio")
        if action == "page":
            if args.value not in ("network", "bluetooth", "audio", "display", "vpn", "power", "customise"):
                cli.error("Unknown control-centre page")
            call = ["actions", "page", args.value, str(args.input).lower()]
        elif action in ("list", "show", "hide"):
            call = ["actions", "control", action, args.value or ""]
        else:
            call = ["shell", "panel", "controls", action]
    elif command == "keyboard":
        if action == "select":
            if not args.type or not args.id:
                cli.error("keyboard select requires a source type and ID from keyboard list")
        elif args.type is not None or args.id is not None:
            cli.error("Only keyboard select takes a source type and ID")
        if action in ("list", "select"):
            call = ["actions", "keyboard", action, args.type or "", args.id or ""]
        elif action in ("next", "previous"):
            call = ["shell", "keyboardNext" if action == "next" else "keyboardPrevious"]
        else:
            call = ["shell", "panel", "keyboard", action]
    elif command == "audio" and action in ("devices", "select"):
        if (action == "select") != (args.value is not None):
            cli.error("A device ID is required only for audio select")
        call = [
            "actions",
            "device",
            "list" if action == "devices" else "select",
            str(args.input).lower(),
            args.value or "",
        ]
    elif command in ("network", "bluetooth", "vpn", "power", "night-light", "awake"):
        needs_value = action in ("connect", "disconnect", "scan", "set")
        if needs_value != (args.value is not None):
            cli.error("This action " + ("requires a value" if needs_value else "takes no value"))
        if command == "bluetooth":
            if action == "scan" and args.value not in ("on", "off"):
                cli.error("bluetooth scan requires on or off")
            if action in ("connect", "disconnect") and not re.fullmatch(
                r"(?:[0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}", args.value
            ):
                cli.error("Use a Bluetooth device address from bluetooth list")
        if (
            command == "network"
            and needs_value
            and not re.fullmatch(r"[0-9a-fA-F]{8}(?:-[0-9a-fA-F]{4}){3}-[0-9a-fA-F]{12}", args.value)
        ):
            cli.error("Use a saved connection UUID from network list")
        if command in ("power", "night-light", "awake"):
            ...
            call = [
                "actions",
                command,
                "status" if command == "network" and action == "list" else action,
                args.value or "",
            ]
    elif command == "audio":
        numeric = action in ("volume", "up", "down")
        if action == "volume" and args.value is None:
            cli.error("audio volume requires a percentage")
        if not numeric and args.value is not None:
            cli.error("This audio action takes no value")
        try:
            value = float(args.value) if args.value is not None else 5 if numeric else 0
        except ValueError:
            cli.error("Volume must be a number")
        if not math.isfinite(value) or not 0 <= value <= 100:
            cli.error("Volume must be between 0 and 100")
        call = ["actions", "audio", action, str(args.input).lower(), str(value)]
    elif command == "media":
        if action == "seek":
            if args.value is None or not math.isfinite(args.value) or args.value < 0:
                cli.error("media seek requires non-negative seconds")
        elif args.value is not None:
            cli.error("Only media seek accepts a position")
        call = ["actions", "media", action, args.player, str(args.value or 0)]
    elif command in ("dock", "windows"):
        if (action == "list") != (args.id is None):
            cli.error("An item ID is required for actions and omitted for list")
        if command == "dock":
            if action == "move" and (args.position is None or args.position < 0):
                cli.error("dock move requires a zero-based position")
            if action != "move" and args.position is not None:
                cli.error("Only dock move takes a position")
        call = ["actions", "dock" if command == "dock" else "window", action, args.id or ""]
        if command == "dock":
            call.append(str(args.position or 0))
    elif command == "notifications" and action in ("list", "dismiss", "clear", "invoke"):
        if (action in ("dismiss", "invoke")) != (args.id is not None):
            cli.error("Notification ID required only for dismiss or invoke")
        if (action == "invoke") != (args.action_id is not None):
            cli.error("An action ID is required only for invoke")
        call = ["actions", "notification", action, args.id or "", args.action_id or ""]
    elif command == "notifications" and (args.id is not None or args.action_id is not None):
        cli.error("This notification action takes no ID")
    elif command == "search" and action == "query":
        if not args.text:
            cli.error("search query requires text")
        call = ["search", "query", " ".join(args.text)]
    elif command == "search" and args.text:
        cli.error("query text requires search query")
    elif command == "search":
        call = ["search", action]
    elif command == "dnd":
        call = ["shell", "dnd", action]
        ...
        allowed = {
            "edge": ["left", "top", "right"],
            "select": ["terminal", "notes", "monitor", "calendar", "media", "tasks"],
        }
        if action in allowed:
            if args.value not in allowed[action]:
                cli.error(f"sidebar {action} requires one of: {', '.join(allowed[action])}")
        elif args.value is not None:
            cli.error(f"sidebar {action} takes no argument")
        call = ["sidebar", "hide" if action == "close" else action] + ([args.value] if args.value else [])
    elif command == "ipc":
        call = [args.target, args.method, *args.arguments]
    else:
        call = [command, action]
    return ["call", "--", *call]
...
def execute(command, run=subprocess.run, sleep=time.sleep, capture=False):
    for attempt in range(12):
        ...
        if any(line.lstrip().startswith("ERROR quickshell.ipc:") for line in output.splitlines()):
            raise RuntimeError(output)
        if (
            result.returncode
            or output.startswith("No running instances")
            or output in ("Not ready to accept queries yet.", "Function not found.", "Target not found.")
        ):
        ...
        if capture:
            if not isinstance(response, dict):
                raise RuntimeError("Expected a JSON response from the shell")
            return response
        if result.stdout:
...
def refresh_network(prefix, query=execute, sleep=time.sleep, monotonic=time.monotonic):
    ...
    while True:
        ...
        if monotonic() >= deadline:
            raise RuntimeError("Network refresh timed out; use network status to inspect progress")
        sleep(0.1)
    ...
    if target in companions:
        ...
        if path:
            ...
            base = (
                ...
                / "quickshell"
                / (args.config or os.environ.get("BINGUX_CONFIG_NAME", "bingux"))
            )
        path = str(base / companions[target])
    selection = (
        ["--path", path] if path else ["--config", args.config or os.environ.get("BINGUX_CONFIG_NAME", "bingux")]
    )
    try:
