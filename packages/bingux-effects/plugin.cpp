...
class SurfaceFade : public QObject {
  ...
  public:
    ...
    explicit SurfaceFade(QObject* parent = nullptr) : QObject(parent) {
        connection();
    }
    ~SurfaceFade() override {
        detach();
    }
    ...
    bool available() const {
        return connection().manager != nullptr;
    }
    void detach() {
        if (owner) {
    ...
    void attach(QQuickWindow* window) {
        ...
        if (!window || !available())
            return;
        auto* group =
            window->findChild<WindowFades*>("binguxBufferFades", Qt::FindDirectChildrenOnly);
        if (!group) {
...
void WindowFades::synchronise(QQuickWindow* window) {
    ...
    for (auto* entry : items) {
        ...
        if (values.size() == 32 * 5) {
            if (!capacityWarning)
                qWarning("Bingux.Effects: more than 32 visible fade regions in one window");
    ...
    if (!fading)
        values.clear();
    if (surface == previousSurface && values == previous)
        return;
