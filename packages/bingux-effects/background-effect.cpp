...
class BackgroundEffect : public QObject {
  ...
  public:
    ...
    explicit BackgroundEffect(QObject* parent = nullptr) : QObject(parent) {
        if (auto* extension = manager())
            connect(extension, &BackgroundManager::changed, this,
                    &BackgroundEffect::availableChanged);
    }
    ...
    void detach() {
        if (owner) {
            ...
            if (auto* window = qobject_cast<QQuickWindow*>(owner->parent()))
                window->update();
        }
        owner = nullptr;
    }
    void attach(QQuickWindow* window) {
        ...
        if (!window)
            return;
        auto* group = window->findChild<WindowBackground*>("binguxBackgroundEffect",
...
void WindowBackground::synchronise(QQuickWindow* window) {
    ...
    if (!extension || !extension->isActive() || !extension->blur || !native)
        return;
    auto* current = static_cast<wl_surface*>(
    ...
    for (auto* entry : items) {
        ...
        if (!entry->enabled() || !item || !item->isVisible() || item->width() <= 0 ||
            item->height() <= 0)
            continue;
        qreal opacity = 1;
