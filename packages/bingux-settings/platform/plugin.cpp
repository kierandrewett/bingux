...
public:
    ...
    void setMargin(int value) {
        ...
        if (m_appliedMargin < 0) schedule();
        emit changed();
    }
signals:
    void changed();
private:
    void schedule() { QTimer::singleShot(0, this, [this] {
        if (!m_window || !m_window->isVisible()) return;
        if (m_appliedMargin != m_margin) {
            if (auto *native = m_window->nativeInterface<QNativeInterface::Private::QWaylandWindow>()) {
                m_appliedMargin = m_margin;
                native->setCustomMargins(QMargins(m_margin, m_margin, m_margin, m_margin));
            }
        }
        m_window->setMask(QRegion(QRect(QPoint(0, 0), m_window->size()).adjusted(m_margin, m_margin, -m_margin, -m_margin)));
    }); }
...
public:
    void registerTypes(const char *uri) override { qmlRegisterType<WindowFrame>(uri, 1, 0, "WindowFrame"); }
