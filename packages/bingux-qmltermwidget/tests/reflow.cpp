...
static void write(Screen &screen, const std::u32string &text) {
    for (auto c : text) screen.displayCharacter(c);
}
...
static QString text(Screen &screen) {
    ...
    for (int i = 0; i < count; ++i) {
        result += row(screen, i);
        if (!(properties[i] & LINE_WRAPPED)) result += '\n';
    }
    return result.trimmed();
}
int main(int argc, char **argv) {
    ...
    try {
        ...
        expect(row(screen, 0) == "ABCDEF" && row(screen, 1) == "GHIJKL" && row(screen, 2) == "MNOPQ", "shrinking must rewrap existing output");
        write(screen, U"R");
        screen.resizeImage(8, 20);
        expect(row(screen, 0) == "ABCDEFGHIJKLMNOPQR", "growing must join soft wraps and preserve cursor writes");
        screen.nextLine();
        write(screen, U"separate");
        for (int width : {7, 30, 4, 12, 20}) screen.resizeImage(8, width);
        expect(text(screen) == "ABCDEFGHIJKLMNOPQR\nseparate", "hard line breaks must survive repeated resizing");
        ...
        for (int width : {5, 20, 8, 30, 6, 12}) {
            ...
            expect(text(history) == original, "history and visible output must reflow together without loss");
        }
        for (auto *selected : {&screen, &history}) {
            ...
            expect(during[0].rendition & RE_SELECTED, "visible and historical selection must carry a display marker");
            expect(!(during[3].rendition & RE_SELECTED), "selection tint must stop at its boundary");
            ...
            expect(image(*selected)[0] == before[0], "clearing selection must restore the original cell");
        }
        Screen unicode(8, 12);
        write(unicode, U"1234界x");
        unicode.resizeImage(8, 5);
        expect(row(unicode, 0) == "1234" && row(unicode, 1) == QString::fromUtf8("界x"), "double-width glyph must not split across rows");
        unicode.resizeImage(8, 12);
        expect(row(unicode, 0) == QString::fromUtf8("1234界x"), "double-width glyph must survive widening");
        ...
        for (int width : {5, 18, 3, 30}) {
            ...
            expect(image(styled)[0] == firstGlyph, "colours, bold and combining glyphs must survive reflow");
        }
        Screen saved(8, 12);
        write(saved, U"123");
        saved.saveCursor();
        ...
        expect(row(alternate, 1) == "MNOPQ", "alternate-screen rows must remain application controlled");
