...
int main(int argc, char** argv) {
    ...
    check(doc.begin().next().blockFormat().bottomMargin() == 0,
          "Body paragraphs must not gain extra gaps");
    check(doc.begin().blockFormat().bottomMargin() == 8, "Heading bottom spacing was not applied");
    check(doc.begin().next().next().blockFormat().topMargin() == 14,
          "Heading spacing was not applied");
    ...
    doc.undo();
    DocumentSpacing::apply(&doc);
    check(doc.toMarkdown() == original && doc.isRedoAvailable(),
          "Spacing broke undo or discarded redo");
    doc.redo();
