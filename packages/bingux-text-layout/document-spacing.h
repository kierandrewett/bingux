...
public:
    ...
    void setDocument(QQuickTextDocument *document) {
        ...
        if (m_document) disconnect(m_document->textDocument(), nullptr, this, nullptr);
        m_document = document;
        if (document) {
    ...
    static void apply(QTextDocument *doc) {
        ...
        for (auto block = doc->begin(); block.isValid(); block = block.next()) {
            ...
            if (format != block.blockFormat()) changes.append({block, format});
        }
        ...
        if (initial) doc->setUndoRedoEnabled(false);
        QTextCursor edit(doc);
        if (!initial) edit.joinPreviousEditBlock();
        for (const auto &[block, format] : changes) {
            QTextCursor cursor(block);
            cursor.setBlockFormat(format);
        }
        if (!initial) edit.endEditBlock();
        if (initial) doc->setUndoRedoEnabled(undoEnabled);
...
private:
    void schedule() {
        if (m_pending || m_applying) return;
        m_pending = true;
