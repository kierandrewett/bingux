...
static gboolean sample(gpointer data) {
    ...
    if (step % 30 == 0) {
        ...
        if (hover)
            gtk_widget_set_state_flags(button, phase == 2 ? GTK_STATE_FLAG_ACTIVE : GTK_STATE_FLAG_PRELIGHT, FALSE);
        if (phase == 4) {
            model.state &= ~1u;
    ...
    if (node) {
        ...
        GdkTexture *texture = gsk_renderer_render_texture(gtk_native_get_renderer(GTK_NATIVE(reference_window)), node, &bounds);
    ...
    GdkTexture *texture = gdk_memory_texture_new(model.width, model.height, GDK_MEMORY_DEFAULT, bytes, model.width * 4);
...
int main(int argc, char **argv) {
    ...
    adw_header_bar_set_title_widget(ADW_HEADER_BAR(reference_header), adw_window_title_new("GTK comparison", NULL));
    gtk_window_set_titlebar(GTK_WINDOW(reference_window), reference_header);
    gtk_window_set_child(GTK_WINDOW(reference_window), gtk_label_new("Normal GTK4/libadwaita window"));
    ...
    model = (struct FramePaint){.width=524, .height=46, .top=46, .state=61,
        .title="GTK comparison", .view=frame_paint_create()};
