...
frame_paint_create (void)
{
  return NULL;
}
__attribute__ ((weak)) void
frame_paint_destroy (void *view)
{
}
__attribute__ ((weak)) int
frame_paint_regions (const struct FramePaint *m, FrameRegion emit,
                     void *context)
{
  return 0;
}
__attribute__ ((weak)) int
frame_paint_dispatch (void)
{
  return 0;
}
__attribute__ ((weak)) int
frame_paint_timeout (void)
{
  return -1;
}
__attribute__ ((weak)) int
frame_paint_needs_redraw (void *view)
{
  return 1;
}
__attribute__ ((weak)) int
frame_paint_poll (struct pollfd *fds, nfds_t count, int timeout)
{
  return poll (fds, count, timeout);
}
static void
emit_region (void *context, uint32_t action, int x, int y, int w, int h)
{
...
static void
released (void *data, struct wl_buffer *buffer)
{
...
render (struct Frame *frame)
{
  ...
  b->buffer = wl_shm_pool_create_buffer (pool, 0, w, h, w * 4,
  ...
  if (!frame_paint_regions (&frame->model, emit_region, frame))
    for (int i = 0; i < 3; i++)
      if (w - r - (i + 1) * t >= 0)
        gnoblin_window_frame_v1_region (frame->handle, 2 + i,
                                        w - r - (i + 1) * t, 0, t, t);
...
static void
configured (void *data, struct gnoblin_window_frame_v1 *handle,
            uint32_t serial, int32_t w, int32_t h, int32_t top, int32_t right,
            int32_t bottom, int32_t left, uint32_t state, const char *title,
            const char *app, const char *style)
{
  struct Frame *f = data;
  free (f->title);
  free (f->style);
...
static void
interaction (void *data, struct gnoblin_window_frame_v1 *handle,
             uint32_t action, uint32_t pressed)
{
  struct Frame *f = data;
...
static void
closed (void *data, struct gnoblin_window_frame_v1 *handle)
{
...
static const struct gnoblin_window_frame_v1_listener frame_listener
    = { configured, interaction, closed };
static void
offered (void *data, struct gnoblin_window_frame_manager_v1 *manager,
         struct gnoblin_window_frame_v1 *handle)
{
...
static const struct gnoblin_window_frame_manager_v1_listener manager_listener
    = { offered };
static int manager_found;
static void
global_added (void *data, struct wl_registry *registry, uint32_t name,
              const char *interface, uint32_t version)
{
  if (!strcmp (interface, "wl_compositor"))
    ...
    {
      struct gnoblin_window_frame_manager_v1 *manager = wl_registry_bind (
          registry, name, &gnoblin_window_frame_manager_v1_interface, 1);
      gnoblin_window_frame_manager_v1_add_listener (manager, &manager_listener,
                                                    NULL);
      manager_found = 1;
    }
}
static void
global_removed (void *data, struct wl_registry *registry, uint32_t name)
{
}
static const struct wl_registry_listener registry_listener
    = { global_added, global_removed };
static void
load_theme (void)
{
  if (!theme_path)
...
main (int argc, char **argv)
{
  ...
  if (theme_path)
    {
      ...
      if (watch_fd >= 0)
        inotify_add_watch (watch_fd, dirname (copy),
  ...
  if (wl_display_roundtrip (display) < 0 || !compositor || !shm
      || !manager_found)
    {
      fprintf (stderr,
               "No private frame renderer capability on this connection\n");
      return 2;
    }
  while (1)
    {
      ...
      struct pollfd fds[2] = { { wl_display_get_fd (display),
                                 POLLIN | (flushed < 0 ? POLLOUT : 0), 0 },
                               { watch_fd, POLLIN, 0 } };
