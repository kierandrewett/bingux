...
extern "C"
{
  ...
  int frame_paint_regions (const struct FramePaint *, FrameRegion emit,
                           void *context);
  int frame_paint_dispatch (
      void); /* nonzero requests repaint after toolkit/theme changes */
  int
  frame_paint_timeout (void); /* milliseconds; -1 means no toolkit polling */
