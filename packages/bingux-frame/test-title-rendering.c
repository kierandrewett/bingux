...
int main(int argc, char** argv) {
    ...
    GskRenderer* renderer =
        gsk_renderer_new_for_surface(gtk_native_get_surface(GTK_NATIVE(view->window)));
    struct FramePaint model = {.width = argc > 2 ? atoi(argv[2]) : 522,
                               .height = 36,
                               .top = 36,
                               .state = argc > 3 ? atoi(argv[3]) : 61,
                               .title = "Spotify — Title rendering",
                               .view = view,
    ...
    for (int y = 6; y < 30; y++)
        for (int x = 130; x < 360; x++) {
            ...
            if (memcmp(actual + p, expected + p, 4))
                mismatches++;
            compared++;
        }
    ...
    if (argc > 1) {
        cairo_surface_t* surface = cairo_image_surface_create_for_data(
            actual, CAIRO_FORMAT_ARGB32, model.width, model.height, model.width * 4);
