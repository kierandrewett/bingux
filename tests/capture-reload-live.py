...
process = subprocess.Popen(
    [os.environ.get("QUICKSHELL_BIN", "quickshell"), "-p", str(folder)], env=env, stdout=log, stderr=log
)


def call(method, *args):
    result = subprocess.run(
        ["qs", "ipc", "--any-display", "-p", str(folder), "call", "capture", method, *args],
        capture_output=True,
        text=True,
        timeout=3,
        check=True,
...
try:
    ...
    assert call(
        ...
        json.dumps(
            dict(
                kind="recording", target="screen", directory=str(workspace / "recordings"), audio="none", maxHeight=1080
            )
        ),
    )["ok"]
    call("open")
    ...
    info = json.loads(
        subprocess.check_output(["ffprobe", "-v", "error", "-show_entries", "format=duration", "-of", "json", saved])
    )
