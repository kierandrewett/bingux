...
base = [
    ...
    "--any-display",
    "-p",
    str(Path(__file__).resolve().parents[1] / "shell/bingux/CaptureShell.qml"),
    "call",
    "capture",
]
...
w = next(w for w in wins if w["focused"])
r = w["geometry"]
...
def call(path, iface, method, args=None):
    return bus.call_sync(dest, path, iface, method, args, None, Gio.DBusCallFlags.NONE, 3000, None).unpack()
...
try:
            ...
            json.dumps(
                {
                    ...
                    "cursor": False,
                    "delay": 0,
                    "directory": tempfile.mkdtemp(prefix="bingux-window-capture-test-"),
                }
            ),
        )
    )
    time.sleep(1)
    call(s, dest + ".Session", "NotifyPointerMotionRelative", GLib.Variant("(dd)", (-10000.0, -10000.0)))
    call(
        s,
        dest + ".Session",
        "NotifyPointerMotionRelative",
        GLib.Variant("(dd)", (float(r["x"] + r["width"] / 2), float(r["y"] + r["height"] / 2))),
    )
    time.sleep(0.2)
    hovered = json.loads(ipc("status"))
    print("Hovered:", hovered["selectedWindow"])
    assert hovered["selectedWindow"] is not None
    subprocess.run(["grim", "/tmp/bingux-window-picker-hover.png"], check=True)
    for down in [True, False]:
        call(s, dest + ".Session", "NotifyPointerButton", GLib.Variant("(ib)", (272, down)))
    time.sleep(0.3)
    state = json.loads(ipc("status"))
    print("After clicking visible window:", {k: state.get(k) for k in ["opened", "state", "target", "selectedWindow"]})
    ...
    dimensions = json.loads(
        subprocess.check_output(
            ["ffprobe", "-v", "error", "-show_entries", "stream=width,height", "-of", "json", state["savedPath"]],
            text=True,
        )
    )["streams"][0]
