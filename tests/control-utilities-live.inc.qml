    ...
    TestCase {
        ...
        function test_native_utilities() {
            try {
                ...
                tryCompare(dock.margins, "bottom", 64, 2000);
                wait(300);
                ...
                verify(!button.showIcon && button.showLabel);
                ...
                drag(divider, controlCentre.nativeWindow, divider.width / 2, 0.5, Qt.point(dockArea.screenRect.x + 12, dockArea.screenRect.y + 12));
                verify(divider.parent === dock.widgetHost, "Divider drop: " + JSON.stringify(editor.layout));
                compare(divider.width, 1);
                compare(divider.height, Theme.barHeight - 12);
                editor.undo();
                wait(100);
                verify(divider.parent === nativeDividerParent);
                compare(divider.height, 1);
                editor.redo();
                wait(100);
                verify(divider.parent === dock.widgetHost);
                drag(space, controlCentre.nativeWindow, space.width / 2, space.height / 2, Qt.point(top.screenRect.x + 10, top.screenRect.y + 16));
                verify(space.parent === leftControls);
                compare(space.width, 16);
                compare(space.height, Theme.barHeight);
                editor.selectedWidget = space.widgetId;
                editor.widgetOption("width", 48);
                wait(100);
                compare(space.width, 48);
                editor.put(space.widgetId, "control-centre", 1);
                wait(100);
                verify(space.parent === nativeSpaceParent);
                compare(space.width, 48);
                editor.widgetOption("width", undefined);
                wait(100);
                verify(!space.fixedWidth && space.Layout.fillWidth);
                editor.put(space.widgetId, "top-left", 0);
                editor.widgetOption("width", 48);
                wait(100);
                drag(space, topBar, 24, 16, Qt.point(dockArea.screenRect.x + 12, dockArea.screenRect.y + 12));
                verify(space.parent === dock.widgetHost);
                compare(space.width, 48);
                editor.put("control-customise", "top-left", 0);
                wait(100);
                verify(button.parent === leftControls);
                verify(button.showIcon && !button.showLabel);
                compare(button.height, Theme.barHeight);
                editor.selectedContainer = "top-left";
                editor.containerDisplay("text");
                verify(!button.showIcon && button.showLabel);
                editor.selectedWidget = button.widgetId;
                editor.widgetOption("label", "Edit desktop");
                editor.widgetOption("display", "both");
                editor.widgetOption("icon", "starred-symbolic");
                verify(button.showLabel && button.showIcon);
                compare(button.displayedText, "Edit desktop");
                compare(button.displayedIcon, "starred-symbolic");
                editor.cancel();
                wait(200);
                ...
                for (const item of [divider, space, button]) {
                    const sample = sampleComponent.createObject(editor.contentItem, {
                ...
                compare(widgetMenu.widgetId, space.widgetId);
                verify(widgetMenu.anchorWindow === dock);
                widgetMenu.visible = false;
                wait(300);
                gesture(button, topBar, button.width / 2, button.height / 2, ["--shift-right-click"]);
                tryCompare(widgetMenu, "visible", true, 3000);
                compare(widgetMenu.widgetId, button.widgetId);
                widgetMenu.visible = false;
                wait(300);
