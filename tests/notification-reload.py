...
def check_session(directory):
    ...
    process = subprocess.Popen([os.environ.get("QS_TEST_BIN", "quickshell"), "-p", str(directory), "--no-color"], stdout=log, stderr=log)

    def ipc(method):
        return subprocess.check_output([os.environ.get("QS_TEST_BIN", "quickshell"), "ipc", "-p", str(directory), "call", "test", method], stderr=subprocess.DEVNULL, text=True).strip()
    ...
    def notify(title, replace=0, timeout=0):
        result = subprocess.check_output([
            ...
            "Files", str(replace), "system-file-manager", title, "Reload test", "['default', 'Open']", "{}", str(timeout),
        ], text=True)
    ...
    try:
        ...
        assert sorted((entry["id"], entry["summary"], entry["receivedAt"]) for entry in before) == sorted((entry["id"], entry["summary"], entry["receivedAt"]) for entry in after)
        notify("Replaced after reload", replace=ids[0])
        await_value(lambda value: len(value) == 4 and any(entry["summary"] == "Replaced after reload" for entry in value))
        ...
        ipc("dismissFirst")
        await_value(lambda value: len(value) == 3)
        print("PASS: reload retains live actions; full process restart restores history without replay, and new notifications do not overwrite it")
    except Exception:
        log.flush()
