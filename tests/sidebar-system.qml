...
ShellRoot {
    ...
    FloatingWindow {
        ...
        QtObject {
            ...
            property var latest: ({
                    cpuPercent: 36,
                    memoryUsedBytes: 22000000000,
                    memoryTotalBytes: 64000000000,
                    networkReceiveBytesPerSecond: 750000,
                    networkTransmitBytesPerSecond: 54000,
                    extra: {
                        cpuCores: Array.from({
                            length: 16
                        }, (_, id) => ({
                        ...
                        hardware: {
                            cpuModel: "AMD Ryzen test processor with a long model name",
                            cpuMhz: 4100,
                            uptimeSeconds: 99000,
                            kernel: "6.15.0-test",
                            processCount: 200,
                            ...
                            gpus: [
                                {
                                    name: "AMD Radeon graphics with a long model name",
                                    driver: "amdgpu",
                                    pciAddress: "0000:2d:00.0",
                                    busyPercent: 32,
            ...
            property var history: Array.from({
                length: 61
            }, (_, i) => ({
                        at: Date.now() - (60 - i) * 1000,
                        cpu: 20 + Math.sin(i * .5) * 15 + i * .3,
                        memory: 34 + i * .01,
                        memoryUsed: 22000000000,
                        memoryTotal: 64000000000,
                        receive: 650000 + Math.sin(i * .35) * 200000,
                        send: 54000 + Math.cos(i * .3) * 30000
                    }))
            function formatBytes(value) {
                return value > 1e9 ? (value / 1e9).toFixed(1) + "G" : Math.round(value / 1000) + "K";
            }
        ...
        TestCase {
            ...
            function test_state_sort_cycle() {
                ...
                for (let i = 0; i < 5; i++) {
                    ...
                    const states = Array.from({
                        length: 5
                    }, (_, index) => table.recordAt(index).state);
            ...
            function test_layout() {
                ...
                for (const width of [150, 320, 384]) {
                    ...
                    function check(item) {
                        for (const child of item.children) {
                            if (!child.visible)
                                continue;
                            if (child.objectName === "processTableViewport")
                                continue;
                            if (child.width > item.width + 1)
                                console.warn("OVERFLOW", child, child.width, item, item.width);
                ...
                if (Quickshell.env("BINGUX_SYSTEM_SCREENSHOT")) {
                    panel.page = "processes";
                    wait(300);
                    grabImage(monitor).save(Quickshell.env("BINGUX_SYSTEM_SCREENSHOT"));
                }
            }
