    ...
    TestCase {
        ...
        function test_native_groups() {
            try {
                ...
                gesture(settings, dock, settings.width / 2, settings.height / 2, ["--drag-to", DesktopEditing.point(account, controlCentre.nativeWindow, 1, 16).x.toString(), DesktopEditing.point(account, controlCentre.nativeWindow, 1, 16).y.toString()]);
                ...
                drag(header, settings.x + settings.width + 4, 16, DesktopEditing.point(audioRows, controlCentre.nativeWindow, audioRows.width - 2, audioRows.height - 2));
                ...
                editor.undo();
                wait(200);
                drag(output, output.width / 2, output.height / 2, DesktopEditing.point(microphone, controlCentre.nativeWindow, microphone.width - 2, microphone.height - 2));
                ...
                editor.put("controls-audio", "palette", 0);
                wait(150);
                verify(!audioRows.visible);
                editor.put("control-volume", "control-centre", 0);
                wait(150);
                ...
                drag(battery, battery.width / 2, battery.height / 2, Qt.point(dockArea.screenRect.x + 12, dockArea.screenRect.y + 12));
                ...
                editor.undo();
                wait(150);
                compare(battery.parent, header);
                editor.redo();
                wait(150);
                compare(battery.parent, dock.widgetHost);
                editor.selectedContainer = "dock";
                editor.containerDisplay("text");
                editor.selectedWidget = "control-battery";
                editor.widgetOption("label", "Charge");
                ...
                battery.summary = "Battery 84 percent, charging";
                battery.available = true;
                tryCompare(battery, "visible", true, 1000);
                compare(battery.label, "84%");
                ...
                tryCompare(widgetMenu, "visible", true, 3000);
                compare(widgetMenu.widgetId, "control-battery");
                widgetMenu.visible = false;
