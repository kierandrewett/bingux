...
vm.runInContext(
    readFileSync(new URL("../shell/bingux/DesktopLayout.js", import.meta.url), "utf8").replace(/^\.import[^\n]+\n/, ""),
    layout,
);
...
test("moving widgets preserves other placements and never duplicates a widget", () => {
    ...
    assert.equal(
        Object.values(reordered)
            .flat()
            .filter((id) => id === "notifications").length,
        1,
    );
});
...
test("presentation preserves native defaults and supports container inheritance and widget overrides", () => {
    ...
    const desktop = {
        containers: { dock: { display: "text" } },
        widgetOptions: { clock: { display: "inherit", label: "Time" } },
    };
    let face = layout.presentation(desktop, "clock", "dock", "12:30", "clock", false, true);
    assert.equal(face.label, "Time");
    assert.equal(face.showIcon, false);
    assert.equal(face.showText, true);
    ...
    assert.equal(face.icon, "starred-symbolic");
    assert.equal(face.showIcon, true);
    assert.equal(face.showText, false);
    ...
    for (const target of ["dock", "top-left", "top-center", "top-right"]) {
        ...
        assert.equal(
            Object.values(current)
                .flat()
                .filter((id) => id === "control-network").length,
            1,
        );
...
test("native portable controls can move through bar, dock and sidebar without duplicate placements", () => {
    for (const id of [
        "control-account",
        "control-settings",
        "control-session",
        ...
        let current = layout.defaults();
        for (const zone of ["top-left", "top-center", "top-right", "dock", "sidebar", "palette"]) {
...
test("group display inherits its host while child overrides stay independent", () => {
    ...
    assert.equal(face().mode, "both");
    assert.equal(face().label, "Preferences");
});

test("status, decoration and panel widgets accept the control centre", () => {
    assert.deepEqual(
        plain(vm.runInContext("widgets.map(item => item.id).sort()", layout)),
    ...
    for (const id of ["spacer:1", "spring:1", "unknown"]) assert.equal(layout.accepts(id, "control-centre"), false);
});
