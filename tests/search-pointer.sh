...
for reduced in 0 1; do
    BINGUX_REDUCED_MOTION="$reduced" NO_AT_BRIDGE=1 timeout 15s "${QUICKSHELL_BIN:-quickshell}" -p "$test_dir" --no-color > "$test_dir/runtime.log" 2>&1 || { cat "$test_dir/runtime.log"; exit 1; }
    grep -q 'SEARCH_POINTER_PASS' "$test_dir/runtime.log" || { cat "$test_dir/runtime.log"; exit 1; }
