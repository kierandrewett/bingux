...
ShellRoot {
    ...
    function findBadge(name) {
        return icon.children.find(child => child.objectName === name);
    }
    FloatingWindow {
        visible: true
        implicitWidth: 160
        ...
        Item {
            ...
            AppIcon {
                ...
                notifications: test.phase < 2 ? Array.from({
                    length: test.phase === 0 ? 1 : 123
                }, () => ({
                            desktopEntry: "cutout"
                        })) : []
                activeStreams: test.phase === 1 ? [
                    {
                        properties: {
                            "application.id": "cutout"
                        }
                    }
                ] : []
        ...
        onTriggered: {
            ...
            canvas.grabToImage(result => {
                ...
                if (!result.saveToFile(path)) {
                    console.error("CUTOUT_SAVE_FAILED");
                    Qt.quit();
                    return;
                }
                console.info("CUTOUT_SAMPLE " + JSON.stringify({
                    phase: test.phase,
                    path,
                    samples
                }));
