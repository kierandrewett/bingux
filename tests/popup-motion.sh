...
for component in Theme ShellPopup; do
    cp "$repo_dir/shell/bingux/$component.qml" "$test_dir/"
done
printf '%s\n' 'singleton Theme 1.0 Theme.qml' 'ShellPopup 1.0 ShellPopup.qml' > "$test_dir/qmldir"
...
timeout 20s dbus-run-session -- "${QUICKSHELL_BIN:-quickshell}" -p "$test_dir" --no-color > "$test_dir/runtime.log" 2>&1 || { cat "$test_dir/runtime.log"; exit 1; }
