    ...
    PanelWindow {
        ...
        TestCase {
            ...
            function initTestCase() {
                ...
                verify(waitForRendering(before));
            }
            ...
            function capture(name) {
                ...
                if (!prefix)
                    return;
                let saved = false;
                verify(previewHost.grabToImage(result => {
                    saved = result.saveToFile(prefix + "-" + name + ".png");
                }));
                tryVerify(() => saved, 2000);
            }
            function test_individual_presentation_data() {
                const rows = [];
                const ids = ControlLayout.widgets.filter(widget => widget.action).map(widget => widget.id).concat(["control-volume", "control-microphone", "control-battery", "control-media"]).concat(DesktopLayout.controlOrder().map(name => "control-" + name));
                for (const id of ids) {
                    ...
                    for (const container of ["dock", "sidebar", "control-centre"]) {
                        ...
                        if (group !== "control-centre")
                            rows.push({
                                tag: id + "-group-" + container,
                                id,
                                group,
                                container,
                                grouped: true
            ...
            function test_individual_presentation(data) {
                ...
                if (data.container !== "control-centre")
                    editor.layout = Object.assign({}, editor.layout, {
                        [data.container]: [target]
                    });
                const preview = previewComponent.createObject(previewHost, {
                    widgetId: data.id
                });
                try {
                    ...
                    compare(ControlLayout.isAction(data.id) ? item.barStyle : item.barLayout, data.container === "dock", "Sidebar controls keep their full panel layout");
            ...
            function test_status_presentation_data() {
                return ["search", "clock", "controls", "metrics", "keyboard", "notifications", "overflow", "capture", "privacy", "tray"].map(id => ({
                            tag: id,
                            id
                        }));
            }
            function test_status_presentation(data) {
                ...
                try {
                    ...
                    for (const container of ["dock", "control-centre", "sidebar"]) {
                        editor.layout = {
                            [container]: container === "control-centre" ? [] : [data.id]
                        };
                        editor.desktop = {
                            controlLayout: ControlLayout.move(ControlLayout.defaults(), "control-centre", data.id, container === "control-centre" ? 0 : -1),
                            containers: {
                                [container]: {
                                    display: "text"
                                }
                        ...
                        compare(face.presentation.label, "My status");
                        compare(face.presentation.icon, "starred-symbolic");
                        verify(face.presentation.showText && face.presentation.showIcon);
                    }
            ...
            function test_panel_contexts(data) {
                ...
                if (data.container === "control-centre")
                    editor.desktop = Object.assign({}, editor.desktop, {
                        controlLayout: ControlLayout.move(editor.desktop.controlLayout, "control-centre", data.id, 0)
                    });
                else if (data.container !== "palette")
                    editor.layout = {
                ...
                try {
                    ...
                    verify(item.presentation.showText && item.presentation.showIcon);
                    compare(item.presentation.label, "My panel");
                    compare(item.presentation.icon, "starred-symbolic");
                    if (compact) {
                    ...
                    verify(preview.visualItem.width <= preview.width && preview.visualItem.height <= preview.height);
                    if (["notes", "media"].includes(data.id))
                        capture(data.tag);
                } catch (error) {
            ...
            function test_utility_contexts(data) {
                ...
                try {
                    ...
                    if (data.id === "control-divider") {
                        ...
                        verify(compact ? item.width === 1 : item.width > item.height, "Panel dividers remain horizontal");
                        ...
                        compare(item.flexible, false);
                        compare(item.implicitWidth, 41);
                    } else {
            ...
            function test_shared_group_previews() {
                ...
                try {
                    for (const id of ["controls-header", "controls-audio", "controls-tiles"]) {
                        ...
                        preview = previewComponent.createObject(previewHost, {
                            widgetId: id
                        });
                        tryVerify(() => !!preview.previewControl, 1500);
                        ...
                        verify(waitForRendering(preview, 2000));
                        capture(id);
                        verify(preview.visualItem.width <= preview.width && preview.visualItem.height <= preview.height, "The complete group fits inside the preview");
                        if (id === "controls-header") {
                            ...
                            compare(lock.Layout.column, 0);
                            compare(account.Layout.column, 1);
                            editor.layout = Object.assign({}, editor.layout, {
                                dock: [id]
                            });
                            editor.desktop = Object.assign({}, editor.desktop, {
                            ...
                            verify(waitForRendering(preview, 2000));
                            capture("header-dock-labels");
                            editor.layout = DesktopLayout.defaults();
                        }
                        preview.destroy();
                        preview = null;
                        wait(50);
                        for (const key of Object.keys(originalSources))
                            compare(DesktopEditing.sources[key], originalSources[key]);
                        compare(Object.keys(DesktopEditing.sources).length, Object.keys(originalSources).length, "Sample widgets do not replace live drag sources");
                    }
                } finally {
                    if (preview)
                        preview.destroy();
