...
test("input hint completes prefixes and labels other matches without altering the query", () => {
    ...
    vm.runInContext(
        source.slice(source.indexOf("    function inputHint("), source.indexOf("    function chatCandidate(")),
        context,
    );
...
test("mouse movement and keyboard share selection without a stationary pointer stealing it", () => {
    ...
    const context = vm.createContext({
        ...
        root,
        hovered: true,
        point: { position: { x: 10, y: 60 } },
        lastPosition: { x: 9, y: 60 },
        resultsList: {
            width: 660,
            height: 420,
            mapFromItem: (_, x, y) => ({ x, y }),
            contentX: 0,
            contentY: 0,
            indexAt: (x, y) => Math.floor(y / 50),
        },
    });
    vm.runInContext(
        source.slice(
            source.indexOf("                    function selectAtPointer()"),
            source.indexOf("                    onPointChanged:"),
        ),
        context,
    );
...
test("chevron entrance is claimed once per highlighted index, not per query or delegate", () => {
    ...
    vm.runInContext(
        source.slice(
            source.indexOf("    function claimChevronAnimation"),
            source.indexOf("    property bool keyboardSelection"),
        ),
        context,
    );
