...
""")
    ...
    environment = os.environ | {
        ...
        "AI_MARKER": str(base / "called"),
        "AI_CHILD": str(base / "child"),
    }
    for adapter in ("pi", "claude"):
        config = {
            "protocolVersion": 1,
            "commands": {
                "applicationLauncher": ["/usr/bin/true"],
                "fileOpener": ["/usr/bin/true"],
                "clipboard": ["/usr/bin/true"],
            },
            "fileRoots": [],
            "ai": {"harness": adapter, "executable": str(harness)},
        }
        path = base / "search.json"
        path.write_text(json.dumps(config))
        daemon = subprocess.Popen(
            [str(BINARY), "--config", str(path)], env=environment, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        try:
            ...
            def send(kind, request, **fields):
                connection.sendall(
                    (json.dumps({"protocolVersion": 1, "type": kind, "requestId": request} | fields) + "\n").encode()
                )
            ...
            def read(request):
                while True:
                    ...
                    if event.get("requestId") == request:
                        return event
            ...
            assert first < 0.25, first
            assert events[-1]["message"] == "First line\nSecond line"
            assert len(events) >= 2 and time.monotonic() - started > 0.2
            results = query("! follow-up")
            send("activate", "follow", resultId=results[0]["resultId"])
            while read("follow")["type"] != "chat-response":
                pass
            assert len(json.loads(marker.read_text())["messages"]) == 3
            results = query("! cancel")
            send("activate", "cancelled", resultId=results[0]["resultId"])
            ...
            (base / "child").unlink()
            results = query("! fail")
            send("activate", "failure", resultId=results[0]["resultId"])
            while True:
                event = read("failure")
            ...
            print(
                f"PASS {adapter}: explicit activation, first streamed text {first * 1000:.0f} ms, multiline final, history, cancellation, process cleanup, failure, conversion and website providers"
            )
            reader.close()
            connection.close()
        finally:
