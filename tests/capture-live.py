...
try:
    ...
    while time.monotonic() < deadline:
        ...
        if event["event"] == "recording":
            ...
            info = json.loads(subprocess.check_output(["ffprobe", "-v", "error", "-show_entries", "stream=codec_name,width,height:format=duration,size", "-of", "json", event["path"]]))
        ...
        if event["event"] in ("ready", "saved"):
            ...
            if job is None:
                ...
                break
            send(dict(command="capture", target="region", region=dict(x=200, y=200, width=640, height=360), directory=folder, copy=False, **job))
    else:
