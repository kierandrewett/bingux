...
sidebar_file.write_text(sidebar_file.read_text().replace('"file://" + Quickshell.env("HOME") + "/.config/bingux/sidebar.ini"', json.dumps('file://' + str(fixture / 'sidebar.ini'))))
monitor_file = fixture / 'SidebarMonitor.qml'
monitor_file.write_text(monitor_file.read_text().replace('"file://" + Quickshell.env("HOME") + "/.config/bingux/sidebar-system.ini"', json.dumps('file://' + str(fixture / 'system.ini'))))
notes_file = fixture / 'SidebarNotes.qml'
notes_path = fixture / 'notes.ini'
notes_file.write_text(notes_file.read_text().replace('"file://" + Quickshell.env("HOME") + "/.config/bingux/sidebar-notes.ini"', json.dumps('file://' + str(notes_path))))
...
def ipc(method, *args):
    result = subprocess.run(['qs', '-p', str(fixture), 'ipc', 'call', 'sidebar', method, *args],
...
def pointer_button(state):
    snapshot('Main.wm._sidebarPointer.notify_button(GLib.get_monotonic_time(), '
...
def key(keyval):
    for state in ['PRESSED', 'RELEASED']:
        snapshot('Main.wm._sidebarKeyboard.notify_keyval(GLib.get_monotonic_time(), '
...
try:
    ...
    snapshot("global.get_window_actors().find(a => a.meta_window.get_title() === 'Sidebar work-area test').meta_window.maximize();")
    move(640, 400)
    ...
    def button():
        return next((w['rect'] for w in snapshot()['windows']
                     if w['rect']['width'] == 24 and w['rect']['height'] == 72
                     and w['rect']['x'] == base['width'] - 24
                     and w['rect']['y'] <= 400 < w['rect']['y'] + 72), None)
    ...
    for cycle in range(8):
        ...
        wait_for(lambda: ipc('status')['pillVisible'] and ipc('status')['pillHovered'],
                 f'edge re-entry {cycle} enters hover')
    click()
    ...
    assert sidebar_rect['y'] == 0 and sidebar_rect['height'] == 800, f'Sidebar must cover bar and dock areas: {sidebar_rect}'
    ...
    for edge in ['top', 'left', 'right']:
        ...
        if edge == 'right':
            ...
            move(start_x, start_y)
        def pull(distance):
            move(start_x + (distance if edge == 'left' else -distance if edge == 'right' else 0),
        ...
        for distance in [25, 80, 140, 220, 160, 60]:
            ...
            geometry = ipc('status')
            assert (geometry['cornerSize'] > previous_corner) == (distance > previous_distance), 'desktop corner follows the drag size'
            previous_distance, previous_corner = distance, geometry['cornerSize']
            assert geometry['surfaceWidth'] == 1280 and geometry['surfaceHeight'] == (800 - geometry['headerHeight'] if edge == 'top' else 800), f'{edge} backing surface must remain stable: {geometry}'
        pull(60)
        wait_for(lambda: ipc('status')['dragging'],
                 f'{edge} follows held pointer')
        wait_for(lambda: 30 < base[axis] - work_area()[axis] < 90,
                 f'{edge} reserves only the dragged distance')
        expected = base['y'] + 60 if edge == 'top' else 60 if edge == 'left' else base['width'] - 60 - 24
        assert any((w['rect']['width'], w['rect']['height']) == ((72, 24) if edge == 'top' else (24, 72))
                   and abs(w['rect']['y' if edge == 'top' else 'x'] - expected) <= 2
                   for w in snapshot()['windows']), f'{edge} visual handle follows panel'
        if edge != 'top':
        ...
        pull(149)
        pointer_button('RELEASED')
        wait_for(lambda: work_area() == base and not ipc('status')['dragging'],
                 f'{edge} release below 150px closes')
        assert not ipc('status')['open']
        wait_for(lambda: next(w['rect'] for w in snapshot()['windows'] if w['title'] == 'Sidebar work-area test') == base,
        ...
        pointer_button('RELEASED')
        wait_for(lambda: ipc('status')['open'] and ipc('status')['reveal'] == 1,
                 f'{edge} release keeps panel open')
        wait_for(lambda: abs(base[axis] - work_area()[axis] - 300) <= 2, f'{edge} release preserves dragged size')
        wait_for(lambda: work_area()[axis] < base[axis] and work_area()[other] == base[other],
        ...
        for cross in [200, 500, 350]:
            ...
            assert any((w['rect']['width'], w['rect']['height']) == ((72, 24) if edge == 'top' else (24, 72))
                       ...
                       for w in snapshot()['windows']), f'{edge} open pill follows unpressed pointer'
        ...
        for cross, expected_center in [(410, 350), (460, 460), (465, 465), (470, 470), (463, 463)]:
            move(cross if edge == 'top' else 300 if edge == 'left' else base['width'] - 301,
                 base['y'] + 300 if edge == 'top' else cross)
            time.sleep(.22)
            assert any((w['rect']['width'], w['rect']['height']) == ((72, 24) if edge == 'top' else (24, 72))
                       and abs(w['rect']['x' if edge == 'top' else 'y'] - (expected_center - 36)) <= 2
                       for w in snapshot()['windows']), f'{edge} follow threshold'
        grip = next(w['rect'] for w in snapshot()['windows']
        ...
        for right_corner in [False, True]:
            for bottom_corner in [False, True]:
                ...
                grip = next(w['rect'] for w in snapshot()['windows']
                            if (w['rect']['width'], w['rect']['height']) == ((72, 24) if edge == 'top' else (24, 72)))
                move(grip['x'] + (grip['width'] - 1 if right_corner else 1),
        ...
        for distance in [40, 100, 60, 80]:
            pull(distance)
            time.sleep(.04)
        wait_for(lambda: abs(base[axis] - work_area()[axis] - 380) <= 2,
                 f'{edge} persistent grip resizes relative to existing size')
        terminal_axis = 0 if edge == 'top' else 1
        wait_for(lambda: terminal_size(shell_pid)[terminal_axis] > old_terminal_size[terminal_axis],
                 f'{edge} terminal grid resizes before pointer release')
        cross_shift = 70
        move(start_x + (cross_shift if edge == 'top' else 80 if edge == 'left' else -80),
             start_y + (80 if edge == 'top' else cross_shift))
        time.sleep(.22)
        expected_cross = (start_x if edge == 'top' else start_y) + cross_shift - 36
        wait_for(lambda: any((w['rect']['width'], w['rect']['height']) == ((72, 24) if edge == 'top' else (24, 72))
                             and abs(w['rect']['x' if edge == 'top' else 'y'] - expected_cross) <= 2
                             for w in snapshot()['windows']), f'{edge} pill follows cross-axis movement')
        ...
        pointer_button('RELEASED')
        wait_for(lambda: not ipc('status')['dragging'] and abs(base[axis] - work_area()[axis] - 380) <= 2,
        ...
        pull(-231)
        pointer_button('RELEASED')
        wait_for(lambda: not ipc('status')['open'] and work_area() == base,
                 f'{edge} resize below 150px closes')
        ipc('open')
        wait_for(lambda: abs(base[axis] - work_area()[axis] - 380) <= 2,
                 f'{edge} reopen remembers resized extent')
        time.sleep(.4)
    ...
    key(ord('!'))
    wait_for(lambda: '!' in notes_path.read_text() and 'Sidebar note' in notes_path.read_text(), 'restored notes remain editable')
    ipc('hide')
    ...
    print('PASS: click hints, stable backing surface, persistent resize grip, all edges, chrome/window restoration, terminal input, session persistence and shell restart')
