...
ShellRoot {
    ...
    FloatingWindow { implicitWidth: 1; implicitHeight: 1 }
    QtObject {
        id: manager
    ...
    Component { id: windowFactory; Window {} }
    Dock { id: dock; testManager: manager; settings: ({pinnedApps: []}) }
    property int step: 0
    ...
    Timer {
        ...
        onTriggered: {
            try {
                ticks++;
                if (ticks > 100) throw new Error("Startup timed out");
                if (step < 4) {
                    check(!dock.visible, "Do not reveal a partial startup batch");
                    const windows = [];
                    for (let i = 0; i < 8; i++) {
                        const index = step * 8 + i;
                        windows.push(windowFactory.createObject(manager, {appId: 'startup-' + (Quickshell.env('DOCK_REPLAY') ? 31 - index : index)}));
                    }
                    ...
                    const added = windowFactory.createObject(manager, {appId: 'opened-after-startup'});
            ...
            } catch (error) { console.error("DOCK_BEHAVIOUR_FAILED", error.message); Qt.quit(); }
        }
    }
}
