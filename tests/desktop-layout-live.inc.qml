        ...
        function test_live_layout() {
            try {
                ...
                for (const spec of previewSpecs) {
                    const sample = samplePreviewComponent.createObject(editor.contentItem, {
                        widgetId: spec.id
                    });
                ...
                editor.selectedWidget = 'label';
                editor.widgetOption('label', 'Team');
                ...
                editor.change('controlCentre', Object.assign({}, originalControls, {
                    nightLight: false,
                    power: false
                }));
                ...
                editor.widgetOption('display', 'both');
                editor.widgetOption('label', 'Clock');
                ...
                editor.selectedWidget = savedSpace;
                editor.widgetOption('width', 36);
                ...
                if (terminalSidebar.opened && !terminalSidebar.floating)
                ...
                layout['top-left'] = ['search'];
                layout['top-right'] = layout['top-right'].filter(id => id !== 'search');
                ...
                tryCompare(externalSettings, 'busy', false, 4000);
                ...
                for (const edge of ['left', 'top', 'right']) {
                    ...
                    if (edge === 'top')
                        compare(topBar.margins.top + topBar.height, rect.y, 'The top sidebar starts below the bar without overlap');
