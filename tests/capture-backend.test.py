...
spec = importlib.util.spec_from_file_location("capture", Path(__file__).resolve().parents[1] / "shell/bingux/capture_backend.py")
...
class CapturePolicy(unittest.TestCase):
    ...
    def test_shutter_respects_event_sound_preference(self):
        for enabled in (False, True):
            with self.subTest(enabled=enabled), patch.object(capture.shutil, "which", return_value="/usr/bin/canberra-gtk-play"), \
                 patch.object(capture.Gio.SettingsSchemaSource, "get_default") as source, \
    ...
    def test_only_successful_screenshot_save_plays_shutter(self):
        for kind, has_output in (("screenshot", True), ("recording", True), ("screenshot", False)):
            with self.subTest(kind=kind, has_output=has_output), tempfile.TemporaryDirectory() as directory:
                ...
                if has_output: worker.temporary.write_bytes(b"capture")
                worker.cleanup = Mock()
                worker.clear_preview = Mock()
                worker.emit = Mock()
                with patch.object(capture, "play_shutter") as shutter, \
                     patch.object(capture.subprocess, "Popen"), patch.object(capture.select, "select", return_value=([], [], [])):
                    worker.complete()
    ...
    def test_compressed_defaults(self):
        ...
        self.assertEqual((options["encoder"], options["fps"], options["maxHeight"], options["audio"]), ("auto", 30, 1080, "none"))
    ...
    def test_hidpi_region_and_negative_monitor_origin(self):
        ...
        self.assertEqual(capture.region_crop(region, metadata, 2000, 1600), dict(left=200, top=200, right=1000, bottom=800))

    def test_wrong_monitor_rejected(self):
        with self.assertRaises(ValueError):
            capture.region_crop(dict(x=2000, y=0, width=400, height=300), dict(position=(0, 0), size=(1920, 1080)), 1920, 1080)
    ...
    def test_frozen_capture_uses_selected_cursor_variant_and_crop(self):
        with tempfile.TemporaryDirectory(prefix="capture-frozen-test-") as directory:
            ...
            for visible, expected in ((False, (255, 0, 0)), (True, (0, 0, 255))):
                worker.job = capture.settings(dict(previewToken="test-frame", screen="screen", cursor=visible, region=dict(x=2, y=1, width=4, height=2)))
