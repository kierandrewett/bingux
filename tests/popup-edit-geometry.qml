            ...
            try {
                ...
                for (const motion of [popup, sharedMotion]) {
                    ...
                    for (const scale of [0.7, 0.9, 1]) {
                        ...
                        tryVerify(() => Math.abs(surface.screenRect.x - expectedX) < 0.01, 1000, "The drop origin follows the actual popup transform");
                        compare(surface.screenRect.y, expectedY);
                        verify(Math.abs(surface.screenRect.width - popup.body.parent.width * scale) < 0.01, "The drop width follows the visible card");
                        verify(Math.abs(surface.screenRect.height - popup.body.parent.height * scale) < 0.01, "The drop height follows the visible card");
                    }
                }
