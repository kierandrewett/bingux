    ...
    FileView {
        id: captureActions
        path: Quickshell.env("BINGUX_CAPTURE_ACTION_REPORT")
        watchChanges: true
        printErrors: false
    ...
    TestCase {
        ...
        function start(delay) {
            verify(captureTool.configureOptions(JSON.stringify({
                kind: "recording",
                target: "screen",
                delay: delay
            })).ok);
            ...
            if (delay)
                tryCompare(captureTool, "countdown", delay, 3000);
        ...
        function test_moved_capture() {
            ...
            try {
                ...
                compare(captureTool.state, "recording");
                compare(commands("stop"), 0);
                editor.undo();
                tryCompare(captureStatus, "parent", originalParent);
                editor.redo();
                tryCompare(captureStatus, "parent", dock.widgetHost);
                editor.selectedWidget = "capture";
                editor.widgetOption("display", "both");
                editor.widgetOption("label", "Capture status");
                for (const zone of ["dock", "top-left", "control-centre", "sidebar"]) {
                    phase = zone + " placement";
                    if (!editor.visible)
                        editor.open();
                    editor.put("capture", zone, 0);
                    save();
                    ...
                    verify(!captureStatus.interactive);
                    compare(captureStatus.label, "Saving…");
                    gesture(captureStatus, host);
                    ...
                    start(10);
                    compare(captureStatus.label, "10");
                    ...
                    compare(widgetMenu.widgetId, "capture");
