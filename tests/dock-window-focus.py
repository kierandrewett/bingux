...
dock_source = dock_file.read_text().replace('id: dockButton', 'id: dockButton; objectName: "dock-test-button"; property alias testMouse: dockMouse')
...
def ipc(method, *args):
    result = subprocess.run(['qs', '-p', str(fixture), 'ipc', 'call', 'dockProbe', method, *args],
...
try:
    ...
    snapshot("global.get_window_actors().find(a => a.meta_window.get_title() === 'Files last').meta_window.activate(global.get_current_time());")
