...
class Artwork(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.wfile.write(
            base64.b64decode(
                "iVBORw0KGgoAAAANSUhEUgAAAAgAAAAICAIAAABLbSncAAAAEUlEQVR4nGNQONCAFTEMLQkASsZYAa5usi0AAAAASUVORK5CYII="
            )
        )
...
class Player:
    def __init__(self, suffix, desktop):
        ...
        def properties(values):
            return "".join(
                f'<property name="{key}" type="{variant(value).get_type_string()}" access="read"/>'
        ...
        self.bus.call_sync(
            ...
            "org.freedesktop.DBus",
            "RequestName",
            GLib.Variant("(su)", (ROOT_IFACE + "." + suffix, 0)),
            None,
    ...
    def metadata(self, title, track):
        self.player["Metadata"] = {
            "mpris:trackid": GLib.Variant("o", "/track/" + track),
    ...
    def call(self, connection, sender, path, interface, method, parameters, invocation):
        ...
        if method in ("Play", "Pause", "PlayPause"):
            ...
            self.metadata(
                "Second track" if method == "Next" else "Test track", "second" if method == "Next" else "first"
            )
...
try:
    with tempfile.TemporaryDirectory(prefix="bingux-media-test-") as directory:
        ...
        for file in (ROOT / "shell/bingux").iterdir():
            if file.suffix in (".qml", ".js", ".py") or file.name == "qmldir":
                shutil.copy2(file, fixture)
        dock = (
            (fixture / "Dock.qml")
            .read_text()
            .replace(
        ...
        discord_icon.write_bytes(
            base64.b64decode(
                "iVBORw0KGgoAAAANSUhEUgAAAAgAAAAICAIAAABLbSncAAAAEUlEQVR4nGNQONCAFTEMLQkASsZYAa5usi0AAAAASUVORK5CYII="
            )
        )
        (applications / "discord-canary.desktop").write_text(
            f"[Desktop Entry]\nType=Application\nName=Discord Canary\nStartupWMClass=discord\nExec=true\nIcon={discord_icon}\n"
        )
        result_file = fixture / "results.txt"
        environment = os.environ | {
            "XDG_DATA_HOME": str(fixture / "data"),
            "XDG_DATA_DIRS": str(fixture / "data"),
            "XDG_CONFIG_HOME": str(fixture / "config"),
            "BINGUX_MEDIA_TEST_RESULTS": str(result_file),
            "BINGUX_DISCORD_TEST_ICON": str(discord_icon),
        }
    ...
    for player in players:
        if not player.bus.is_closed():
            player.bus.close_sync(None)
