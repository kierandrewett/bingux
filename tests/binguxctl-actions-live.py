...
properties = {
    interface: {
        ...
        "DesktopEntry": GLib.Variant("s", identity),
    ...
    interface + ".Player": {
        ...
        "Rate": GLib.Variant("d", 1),
        "MinimumRate": GLib.Variant("d", 1),
        "MaximumRate": GLib.Variant("d", 1),
        "Volume": GLib.Variant("d", 1),
        "Metadata": GLib.Variant(
            "a{sv}",
            {
                "mpris:trackid": GLib.Variant("o", "/track/test"),
                "mpris:length": GLib.Variant("x", 120000000),
                "xesam:title": GLib.Variant("s", "Test track"),
            },
        ),
        **{
...
for iface, values in properties.items():
    props = "".join(
        f'<property name="{name}" type="{value.get_type_string()}" access="read"/>' for name, value in values.items()
    )
    ops = (
        "".join(f'<method name="{name}"/>' for name in ("Play", "Pause", "Next", "Previous"))
        if iface.endswith(".Player")
        else ""
    )
    if ops:
        ops += '<method name="SetPosition"><arg type="o" direction="in"/><arg type="x" direction="in"/></method>'
    ...
    registrations.append(
        bus.register_object(
            ...
            xml.interfaces[0],
            player_call,
            lambda connection, sender, object_path, iface, name: properties[iface][name],
            None,
        )
    )
bus.call_sync(
...
try:
    ...
    until(
        lambda: methods,
        lambda calls: any(method == "SetPosition" and args == ("/track/test", 12000000) for method, args in calls),
    )
    ...
    subscription = bus.signal_subscribe(
        ...
        lambda *args: invoked.append(args[-1].unpack()),
    )
    notice_id = bus.call_sync(
        "org.freedesktop.Notifications",
        "/org/freedesktop/Notifications",
        "org.freedesktop.Notifications",
        "Notify",
    ...
    state = until(
        lambda: call("notifications", "list"),
        lambda state: any(item["summary"] == identity for item in state["notifications"]),
    )
    ...
    until(
        lambda: call("notifications", "list"),
        lambda state: all(item["id"] != notice_key for item in state["notifications"]),
    )
    print("PASS: normal notification action and dismissal affect only the generated notice", flush=True)

    window = subprocess.Popen(
        ["foot", "--app-id", identity, "--title", identity, "sleep", "60"],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    )

    def windows():
        return call("windows", "list")["windows"]
    ...
    until(
        windows,
        lambda entries: any(
            entry["id"] == window_id and entry["active"] and not entry["minimized"] for entry in entries
        ),
    )
