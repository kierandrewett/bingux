...
app.whenReady().then(async () => {
    ...
    try {
        for (const [id, text, start, end, query, expected] of cases) {
            ...
            clipboard.write({text: "Keep clipboard 😀", html: "<b>Keep rich text</b>", rtf: "{\\rtf1 Keep formatting}", image: nativeImage.createFromBitmap(Buffer.from([23, 45, 67, 255]), {width: 1, height: 1})});
            const saved = new Map(clipboard.availableFormats().map(format => [format, clipboard.readBuffer(format)]));
            await drive("exercise", query);
            for (const [format, bytes] of saved) assert.deepEqual(clipboard.readBuffer(format), bytes, "Clipboard restored: " + format);
        ...
        if (process.env.GNOBLIN_TEST_XWAYLAND === "1") {
            const startHelper = async () => {
                const child = spawn("python3", [path.join(process.env.GNOBLIN_SOURCE, "src/scripts/lib/clipboard-paste.py")],
        ...
        if (process.env.GNOBLIN_TEST_XWAYLAND !== "1") console.log("PASS Electron input blur clears the caret and rejects insertion");
        app.exit(0);
    } catch (error) {
        console.error(error);
        app.exit(1);
