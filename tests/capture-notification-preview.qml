...
ShellRoot {
    ...
    PanelWindow {
        ...
        NotificationStack {
            ...
            presentedEntries: [{notification: {id: 1}, appName: "Capture", desktopEntry: "", appIcon: "screenshot-selection-symbolic",
                summary: "Screenshot saved", body: "Window screenshot.png", image: Quickshell.env("CAPTURE_PREVIEW_IMAGE"),
                actions: ["copy", "save", "discard"].map(id => ({action: {identifier: id, invoke: () => {}}, text: {copy: "Copy", save: "Save As", discard: "Discard"}[id], defaultAction: false})),
                receivedAt: Date.now(), timeoutMs: 0}]
        }
    }
    TestCase {
        when: true
        ...
        function test_preview() {
            ...
            let captured = false;
            card.grabToImage(image => { image.saveToFile("/tmp/capture-notification-card.png"); captured = true; });
