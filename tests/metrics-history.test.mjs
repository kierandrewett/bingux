...
test("outages and null readings break traces without losing real zero usage", () => {
    ...
    assert.deepEqual(
        Array.from(history.segments(points, "cpu"), (segment) => Array.from(segment, (point) => point.cpu)),
        [[10, 0], [30], [50]],
    );
...
test("time windows and network axes use the visible measured values", () => {
    const points = [
        { at: 1000, receive: 99999999 },
        { at: 60000, receive: 0, send: 0 },
        { at: 61000, receive: 2048, send: 4096 },
    ];
...
test("optional hardware samples preserve units and missing-sensor gaps", () => {
    ...
    const point = history.append(
        [],
        {
            ...record,
            extra: {
                cpuTemperatureCelsius: 54.5,
                load1: 2.25,
                swapUsedBytes: 1024,
                swapTotalBytes: 4096,
