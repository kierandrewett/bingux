    ...
    TestCase {
        ...
        function test_preview() {
            ...
            preview.displayedResults = [
                {
                    ...
                    subtitle: Quickshell.env("BINGUX_PREVIEW_FIXTURES") + "/document.pdf",
                ...
                },
                {
                    ...
                    kind: "file",
                    title: "Text preview",
                    subtitle: Quickshell.env("BINGUX_PREVIEW_FIXTURES") + "/note.txt",
                    icon: "text-x-generic"
                }
            ];
            preview.displayedResults = preview.displayedResults.concat([
                {
                    resultId: "photo",
                    providerId: "files",
                    kind: "file",
                    title: "Photo preview",
                    subtitle: Quickshell.env("BINGUX_PREVIEW_FIXTURES") + "/photo.png",
            ...
            if (!Theme.reducedMotion)
                equal(panel.reveal < 1, true, "preview starts with a slide and fade");
            wait(30);
            ...
            if (!Theme.reducedMotion)
                equal(enteringFacts.reveal < 1, true, "file facts animate when metadata arrives");
            wait(1200);
            ...
            if (!Theme.reducedMotion)
                equal(body.zoom < 1.25, true, "zoom starts smoothly instead of jumping");
            ...
            if (!Theme.reducedMotion)
                equal(facts.height > 36, true, "facts collapse smoothly");
            wait(250);
            ...
            for (const file of extraFiles) {
                preview.displayedResults = [
                    {
                        ...
                        kind: "file",
                        title: file.name,
                        subtitle: Quickshell.env("BINGUX_PREVIEW_FIXTURES") + "/" + file.name,
                        icon: "text-x-generic"
                    }
                ];
                preview.selectedIndex = 0;
                ...
                tryVerify(() => {
                    ...
                    return current && current.details !== null && current.details.kind === file.kind;
                }, 12000);
                const current = preview.find(preview.contentItem, "searchPreviewBody");
                equal(current.lightBackground, true, "background preference survives file changes");
                if (file.format)
                    equal(current.details.format, file.format, "formatted document is detected");
                ...
                if (file.kind === "video") {
                    ...
                    if (!Theme.reducedMotion)
                        tryVerify(() => media.playing, 2000);
                    const control = preview.find(preview.contentItem, "previewMediaPlayPause");
                    mouseClick(control);
                    if (!Theme.reducedMotion)
                        equal(media.playing, false, "video controls pause playback");
                    else
                        equal(media.playing, true, "video controls start playback with reduced motion");
                    if (!media.playing)
            ...
            if (!Theme.reducedMotion)
                equal(panel.visible, true, "preview remains visible for its exit animation");
            wait(250);
