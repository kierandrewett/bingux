...
ShellRoot {
    ...
    FileView {
        id: result
        path: Quickshell.env("BINGUX_KEYBOARD_TEST_RESULTS")
    }
    Process {
        id: selectionReader
    ...
    FloatingWindow {
        ...
        TestCase {
            ...
            function test_switching() {
                ...
                verify(selector.menuOpen);
                compare(selector.selectedIndex, 0);
                selector.cycleSource(false);
                compare(selector.selectedIndex, 1);
                selector.cycleSource(false);
                compare(selector.selectedIndex, 2);
                selector.cycleSource(false);
                compare(selector.selectedIndex, 0);
                selector.cycleSource(true);
                compare(selector.selectedIndex, 2);
