    ...
    TestCase {
        ...
        function writable(value) {
            ...
            permissions.resultCode = -1;
            permissions.running = true;
            tryCompare(permissions, "running", false, 2000);
            compare(permissions.resultCode, 0);
        }
        function test_recovery() {
            ...
            try {
                ...
                const mutations = [value => {
                        delete value.desktop.layout.dock;
                    }, value => {
                        value.desktop.layout.dock.push("clock");
                    }, value => {
                        value.desktop.layout.sidebar = ["unknown"];
                    }, value => {
                        value.desktop.layoutVersion = true;
                    }, value => {
                        value.desktop.dockApps = {
                            pinnedApps: [null],
                            order: []
                        };
                    }, value => {
                        value.desktop.widgetOptions = {
                            clock: {
                                label: 42
                            }
                        };
                    }, value => {
                        value.desktop.containers = {
                            dock: {
                                display: "invalid"
                            }
                        };
                    }, value => {
                        value.desktop.dockSize = "large";
                    }];
                ...
                desktopCustomiser.put("clock", "dock", 0);
                writable(false);
