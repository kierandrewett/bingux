...
cp "$repo_dir"/shell/bingux/{NotificationState.qml,NotificationSurface.qml,NotificationStack.qml,NotificationTestButton.qml,ActionButton.qml,SymbolicIcon.qml,Theme.qml} "$test_dir/"
cp "$repo_dir/tests/notification-gestures.qml" "$test_dir/shell.qml"
printf '%s\n' 'singleton Theme 1.0 Theme.qml' 'NotificationState 1.0 NotificationState.qml' 'NotificationSurface 1.0 NotificationSurface.qml' 'NotificationStack 1.0 NotificationStack.qml' 'SymbolicIcon 1.0 SymbolicIcon.qml' 'NotificationTestButton 1.0 NotificationTestButton.qml' 'ActionButton 1.0 ActionButton.qml' > "$test_dir/qmldir"
...
timeout 30s dbus-run-session -- quickshell -p "$test_dir" --no-color > "$test_dir/runtime.log" 2>&1 || { cat "$test_dir/runtime.log"; exit 1; }
