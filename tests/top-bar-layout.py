...
(root / "ShellPopup.qml").write_text(
...
(root / "BarTooltip.qml").write_text(
    "import QtQuick\nQtObject {\n property var anchorItem\n property var barWindow\n property bool requested: false\n property string text\n property bool reorderable: false\n}\n"
)
(root / "config" / "bingux").mkdir(parents=True)
result = root / "result"
process = subprocess.run(
    ["dbus-run-session", "--", os.environ.get("QUICKSHELL_BIN", "qs"), "-p", str(root)],
    env=dict(
        os.environ,
        XDG_CONFIG_HOME=str(root / "config"),
        QT_QPA_PLATFORM="offscreen",
        BINGUX_BAR_LAYOUT_RESULT=str(result),
    ),
    capture_output=True,
    text=True,
    timeout=15,
...
assert not re.search(r"TypeError:|ReferenceError:|Binding loop|Cannot stack", process.stdout + process.stderr), (
