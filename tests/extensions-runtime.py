...
""")
    environment = os.environ | {
        ...
        "XDG_DATA_DIRS": str(folder / "system"),
        "XDG_CONFIG_HOME": str(folder / "config"),
        "QT_QPA_PLATFORM": "offscreen",
        "BINGUX_NO_EXTENSIONS": "0",
    }
    result = subprocess.run(
        [os.environ.get("QS_TEST_BIN", "qs"), "-p", str(shell), "--no-color"],
        env=environment,
    ...
    print(result.stdout)
    if "EXTENSION_TEST_PASS" not in result.stdout or any(
        term in result.stdout for term in ["ReferenceError", "TypeError", "Binding loop", "EXTENSION_TEST_FAIL"]
    ):
        raise SystemExit(1)
