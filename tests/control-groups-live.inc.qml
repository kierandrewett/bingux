    ...
    TestCase {
        ...
        function test_native_group_portability() {
            try {
                ...
                compare(editor.selectedContainer, "controls-header");
                compare(editor.optionsPage, "Container");
                compare(editor.inspectionAnchor.width, header.width);
                editor.optionsPage = "";
                wait(120);
                drag(headerGrip, controlCentre.nativeWindow, headerGrip.width / 2, headerGrip.height / 2, Qt.point(dockArea.screenRect.x + 12, dockArea.screenRect.y + 12));
                compare(header.parent, dock.widgetHost);
                compare(settings.parent, header);
                verify(settings.barLayout && !settings.placed);
                ...
                editor.undo();
                wait(150);
                compare(header.parent, nativeParent);
                verify(!settings.barLayout);
                editor.redo();
                wait(150);
                compare(header.parent, dock.widgetHost);
                ...
                drag(audioGrip, controlCentre.nativeWindow, audioGrip.width / 2, audioGrip.height / 2, Qt.point(dockArea.screenRect.x + 12, dockArea.screenRect.y + 12));
                compare(audio.parent, dock.widgetHost);
                compare(output.parent, audio);
                compare(input.parent, audio);
                compare(audio.height, Theme.barHeight);
                verify(output.x < input.x);
                save();
                capture("audio", audio, dock);
                const mute = findChild(output, "controlMute");
                gesture(mute, dock, mute.width / 2, mute.height / 2, ["--click-only"]);
                ...
                verify(controlCentre.anchorAbove);
                compare(controlCentre.deviceControls.audioTab, "input");
                controlCentre.visible = false;
                wait(300);
                editor.open();
                editor.put("controls-audio", "control-centre", 1);
                wait(200);
                compare(audio.parent, nativeParent);
                verify(input.y > output.y);
                editor.put("controls-tiles", "top-left", 0);
                wait(200);
                compare(quick.parent, leftControls);
                compare(network.parent, quick);
                compare(bluetooth.parent, quick);
                const quickOrder = JSON.stringify(editor.desktop.controlOrder);
                editor.change("controlOrder", []);
                wait(100);
                verify(quick.width >= Theme.barHeight && quick.height >= Theme.barHeight, "Empty groups retain an editor drop target");
                editor.undo();
                wait(100);
                compare(JSON.stringify(editor.desktop.controlOrder), quickOrder);
                compare(quick.height, Theme.barHeight);
                verify(bluetooth.x > network.x);
                save();
                ...
                wait(300);
                editor.open();
                editor.put("controls-tiles", "control-centre", 3);
                save();
                compare(quick.parent, nativeParent);
