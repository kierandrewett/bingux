...
test("frontend and backend import exactly the same native groups", () => {
    ...
    const output = execFileSync(
        ...
        [
            "-c",
            'import importlib.util,json,sys; s=importlib.util.spec_from_file_location("settings",sys.argv[1]); m=importlib.util.module_from_spec(s); s.loader.exec_module(m); print(json.dumps(m.native_control_layout()))',
            backend,
        ],
        { encoding: "utf8" },
    );
...
test("invalid versions, groups and duplicate entries are rejected", () => {
    ...
    for (const mutate of [
        ...
        (value) => value.groups["controls-header"].push("unknown"),
        (value) => (value.groups.extra = []),
        (value) => delete value.groups["controls-audio"],
    ]) {
        const value = plain(model.defaults());
        mutate(value);
        assert.equal(model.valid(value), false);
    }
});
...
test("status and decoration placement shares section order and rejects duplicate ownership", () => {
    ...
    for (const id of ["unknown", "label", "icon:0", "label:10000"]) {
        const layout = model.defaults();
        layout.groups["control-centre"].push(id);
        assert.equal(model.valid(layout), false);
    }
    const excessive = model.defaults();
    excessive.groups["control-centre"] = Array.from({ length: 257 }, (_, index) => "label:" + (index + 1));
