...
for (const monitor of [{x: 0, y: 0, width: 1280, height: 800}, {x: -1501, y: 200, width: 1501, height: 901}]) {
    ...
    for (const [x, y, id] of [[0, .5, 'left'], [.999, .5, 'right'], [0, 0, 'top-left'], [.999, 0, 'top-right'], [0, .999, 'bottom-left'], [.999, .999, 'bottom-right']]) {
        const hit = edges.find(r => layouts.contains(r.hit, monitor.x + x * monitor.width, monitor.y + y * monitor.height));
    ...
    assert(!edges.some(r => layouts.contains(r.hit, monitor.x + monitor.width / 2, monitor.y + monitor.height / 2)), 'Centre does not snap');
}
