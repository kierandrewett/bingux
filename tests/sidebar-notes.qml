...
ShellRoot {
 ...
 FloatingWindow {
  id: window; visible: true; implicitWidth: 300; implicitHeight: 500
  SidebarNotes { id: notes; anchors.fill: parent }
  TestCase {
   when: window.visible
   function typeText(editor, text) {
    editor.forceActiveFocus();
    for (const character of text) keyClick(character);
   }
   function test_markdown() {
    ...
    const restored = component.createObject(window.contentItem, {width: 300, height: 500});
