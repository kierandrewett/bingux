...
ShellRoot {
    ...
    QtObject {
        ...
        property var layout: ({sidebar: ["metrics"]})
    }
    QtObject {
        id: sample
    ...
    FloatingWindow {
        id: window
        implicitWidth: 900; implicitHeight: 500
        color: Theme.background
        Item { id: panel; width: 208; height: 450 }
        WidgetPreview { id: preview; x: 400; width: 320; height: 240; widgetId: "metrics" }
        SystemMetrics {
            id: monitor
            parent: panel
        ...
        TestCase {
            ...
            function test_all_readouts_fit() {
                try {
                    for (const name of monitor.monitorNames) monitor.setShown(name, true);
                    wait(100);
                    ...
                    for (const width of [208, 376, 208]) {
                        ...
                        for (const name of monitor.monitorNames) {
                            ...
                            tryVerify(() => {
                                ...
                                return point.x >= 0 && point.y >= 0 && point.x + readout.width <= monitor.width + 0.01
                                    && point.y + readout.height <= monitor.height + 0.01;
                    ...
                    monitor.presentation = {custom: true, showText: true, showIcon: true,
                        label: "A very long monitor label that must fit its container", icon: "computer-symbolic"};
                ...
                } catch (error) { console.error("METRICS_FIT_FAILED", error.message, error.stack); throw error; }
            }
