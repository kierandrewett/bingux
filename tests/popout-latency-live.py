...
def call(path, method, args=None):
    return bus.call_sync(
        dest,
        path,
        dest + (".Session" if path != "/org/gnome/Mutter/RemoteDesktop" else ""),
        method,
        args,
        None,
...
try:
    call(session, "Start")
    for target, close, modifier, symbol in [
        ("search", "close", 65515, 65515),
        ("switcher", "close", 65513, 65289),
        ("emoji", "close", 65515, 46),
        ("capture", "cancel", 65513, ord("s")),
    ]:
        values = []
        for _ in range(6):
            ...
            def wait(visible):
                while True:
                    ...
                    if (
                        x.get("event") == "ui-state"
                        and x.get("name") == target
                        and (
                            (x.get("state") or {}).get(
                                "acceptingKeyboard"
                                if target == "search"
        ...
        print(
            target,
            "key-to-input-ready ms" if target == "search" else "key-to-visible-state ms",
            list(map(lambda x: round(x, 1), values)),
            "median",
