...
vm.runInContext(readFileSync(new URL('../shell/bingux/SearchGroups.js', import.meta.url), 'utf8')
...
test('groups interleaved sources while retaining within-group ranking', () => {
    ...
    assert.deepEqual(Array.from(context.ordered(rows), row => row.resultId),
...
test('answers come first and custom providers get their own group', () => {
    ...
    assert.deepEqual(Array.from(context.ordered(rows), row => context.key(row)),
...
test('grouping preserves relevance even with an unstable runtime sort', () => {
    ...
    `, unstable);
    vm.runInContext(readFileSync(new URL('../shell/bingux/SearchGroups.js', import.meta.url), 'utf8').replace('.pragma library', ''), unstable);
    const ids = vm.runInContext(`ordered([
