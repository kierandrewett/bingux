    ...
    TestCase {
        ...
        function test_overflow_widgets() {
            try {
                ...
                compare(editor.selectedWidget, "tray");
                compare(editor.optionsPage, "Widget");
                const inspector = findChild(editor.optionsContentItem, "customiseInspector");
                tryVerify(() => inspector.y >= barOverflow.panelY + barOverflow.body.parent.height, 2000, "The inspector clears the whole More popup");
                ...
                nativeInput.capturePath = "";
                editor.optionsPage = "";
                gesture(first, barOverflow.nativeWindow, ["--drag-to", String(area.screenRect.x + 40), String(area.screenRect.y + Theme.barHeight + 12)]);
                ...
                editor.undo();
                compare(editor.containerFor("tray"), "top-right");
                editor.redo();
                compare(editor.containerFor("tray"), "sidebar");
                editor.cancel();
                ...
                compare(widgetMenu.anchorWindow, barOverflow.nativeWindow);
                ...
                tryCompare(barOverflow, "visible", true, 3000);
                tryCompare(barOverflow, "revealScale", 1, 3000);
                verify(barOverflow.anchorAbove);
                compare(barOverflow.anchorWindow, dock);
                tryVerify(() => Math.abs(barOverflow.panelY + barOverflow.body.parent.height - (dock.popupAnchorTop - Theme.gap)) < 1, 3000, "More opens directly above its dock anchor");
                ...
                tryCompare(barOverflow, "revealScale", 1, 3000);
                gesture(first, barOverflow.nativeWindow, ["--right-click"]);
                tryVerify(() => inspector.y + inspector.height <= barOverflow.panelY, 2000, "The inspector clears More when it opens from the dock");
