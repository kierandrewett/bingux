    ...
    QtObject {
        ...
        property var devices: QtObject {
            property var values: [testHeadphones]
        }
    }
    FileView {
        id: layoutReport
    ...
    TestCase {
        ...
        function deviceRow(item) {
            if (item.title === testHeadphones.name && item.clicked)
                return item;
        ...
        function test_moved_bluetooth() {
            try {
                ...
                editor.undo();
                tryCompare(bluetooth, "parent", originalParent);
                editor.redo();
                tryCompare(bluetooth, "parent", dock.widgetHost);
                editor.selectedWidget = "control-bluetooth";
                editor.widgetOption("display", "both");
                editor.widgetOption("label", "Bluetooth devices");
                for (const zone of ["dock", "top-left", "sidebar"]) {
                    ...
                    if (zone === "sidebar") {
                        ...
                        tryCompare(testAdapter, "enabled", false);
                    ...
                    tryCompare(controlCentre, "detailProgress", 1, 3000);
                    wait(200);
                    ...
                    tryCompare(testAdapter, "enabled", false);
                    tryCompare(testAdapter, "discovering", false);
                    gesture(power, controlCentre.nativeWindow);
                    tryCompare(testAdapter, "enabled", true);
                    tryCompare(testAdapter, "discovering", true);
                    ...
                    compare(widgetMenu.widgetId, "control-bluetooth");
