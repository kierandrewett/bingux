...
with tempfile.TemporaryDirectory(prefix="bingux-cutouts-") as directory:
    ...
    qml.write_text(Path(__file__).with_suffix(".qml").read_text().replace(
        "../shell/bingux", (repo / "shell/bingux").as_uri()))
    environment = os.environ | {"BINGUX_COMPOSITOR": "portable",
        "BINGUX_TEST_ICON": (output / "icon.png").as_uri(), "BINGUX_TEST_OUTPUT": directory}
    result = subprocess.run([os.environ.get("QS_TEST_BIN", "qs"), "-p", str(qml)],
        env=environment, capture_output=True, text=True, timeout=30)
    log = result.stdout + result.stderr
    records = [json.loads(line.split("CUTOUT_SAMPLE ", 1)[1])
        for line in log.splitlines() if "CUTOUT_SAMPLE " in line]
    ...
    for record in records:
        ...
        for sample in record["samples"]:
            ...
            assert image.getpixel((x, int(sample["iconY"])))[3] > 240, (record, "Cutout must not remove neighbouring icon pixels")
        if record["phase"] == 3:
