...
ShellRoot {
    ...
    TestCase {
        ...
        function bounds() {
            ...
            check(point.x >= area.x && point.x + viewport.width <= area.x + area.width + .1, "Notification viewport stays inside the desktop boundary");
        ...
        function test_sidebar() {
            ...
            if (!Theme.reducedMotion) {
                ...
                check(Math.abs(footerRight - surface.viewport.x - history.body.width) < .1, "Notification list and footer share the same slide offset");
            }
            ...
            bounds();
            check(history.panelX >= 0 && history.panelX + history.popupWidth <= surface.desktopViewport.width, "Notification centre stays left of the right sidebar");
            check(surface.mask.x === 0 && surface.mask.width === surface.width - 300, "History input mask excludes the sidebar");
            ...
            waitForRendering(card);
            bounds();
            check(surface.mask.x === 300 && surface.mask.width === surface.width - 300, "Left sidebar is excluded from history input and layout");
            ...
            if (!Theme.reducedMotion) {
                wait(150);
                check(history.retained && history.slideOffset > 0 && history.slideOffset < history.width - history.panelX, "Centre remains mapped throughout its eased slide to the right");
                const footerRight = clear.mapToItem(surface.desktopViewport, clear.width, 0).x;
                check(Math.abs(footerRight - surface.viewport.x - history.body.width) < .1, "List and footer slide out together");
            }
            ...
            wait(30);
            if (!Theme.reducedMotion)
                check(history.slideOffset > interruptedOffset, "Closing reverses an interrupted entrance towards the right");
            history.visible = true;
            tryCompare(history, "slideOffset", 0);
