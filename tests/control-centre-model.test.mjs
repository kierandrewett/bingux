...
test('normal network view prioritises the connection in use and avoids duplicate Wi-Fi entries', () => {
    ...
    const sections = model.networkSections(connections, [{name:'Home', connected:true}, {name:'Cafe', connected:false}]);
    assert.deepEqual(Array.from(sections.current, item => item.name), ['Home']);
    assert.deepEqual(Array.from(sections.nearby, item => item.name), ['Cafe']);
    assert.deepEqual(Array.from(sections.other, item => item.name), ['Old network']);
