...
class ControlTests(unittest.TestCase):
    def test_tailscale_nodes_preferences_and_exit_identity(self):
        data = {
            ...
            "Self": {"ID": "self", "HostName": "Desktop", "Online": True},
            "Peer": {
                "key": {
                    "ID": "exit",
                    "HostName": "Gateway",
    ...
    def test_provider_exit_nodes_keep_location_and_are_not_tailnet_devices(self):
        nodes = controls.tailscale_state(
            {
                "Peer": {
                    "vpn": {
                        "ID": "vpn",
                    ...
                    "gateway": {"ID": "gateway", "ExitNodeOption": True},
                }
            }
        )["nodes"]
    ...
    def test_vpn_states_do_not_confuse_private_network_with_exit_node(self):
        ...
        self.assertEqual(
            controls.tailscale_state({"BackendState": "Running", "ExitNodeStatus": {"Online": False}})["subtitle"],
    ...
    def test_mullvad_transition_and_lockdown_states_are_explicit(self):
        ...
        self.assertIn(
            "traffic blocked",
            controls.mullvad_state({"state": "disconnected", "details": {"locked_down": True}})["subtitle"],
        )
    ...
    def test_only_saved_vpn_types_are_exposed(self):
        with (
            patch.object(controls.shutil, "which", side_effect=lambda name: name if name == "nmcli" else None),
            patch.object(
                controls,
                "run",
                return_value="first:vpn:Work\\: VPN:\nsecond:bridge:Docker:docker0\nthird:wireguard:Personal:wg0",
            ),
        ):
