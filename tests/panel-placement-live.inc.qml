    ...
    TestCase {
        ...
        function launch(id) {
            ...
            if (Quickshell.env("BINGUX_PANEL_CAPTURE"))
                gesture(findChild(widget, "sidebar-panel-launcher-" + id), widget.barWindow, ["--hover-only"]);
            return widget;
        }
        function test_panel_placement() {
            try {
                ...
                const input = findChild(originals.tasks, "taskInput");
                input.text = "Keep my task draft";
                const oldSize = Qt.size(originals.notes.width, originals.notes.height);
                const editor = desktopCustomiser;
                editor.open();
                tryCompare(controlCentre, "visible", true, 4000);
                ...
                editor.undo();
                compare(editor.containerFor("notes"), "sidebar");
                editor.redo();
                compare(editor.containerFor("notes"), "dock");
                for (const id of Object.keys(originals)) {
                    for (const target of ["top-center", "dock", "control-centre", "sidebar"]) {
                        ...
                        if (target === "sidebar")
                            compare(terminalSidebar.contentType, id);
                        else
                            verify(waitForRendering(widget, 2000));
                ...
                for (const id of Object.keys(destinations))
                    editor.put(id, destinations[id], 0);
                wait(250);
                ...
                for (const id of Object.keys(originals)) {
                    ...
                    if (widget.inlinePanel)
                        verify(originals[id].parent === widget.contentHost || originals[id].parent.parent === widget.contentHost);
                }
                ...
                editor.selectedWidget = "calendar";
                ...
                compare(widgetMenu.widgetId, "notes");
                ...
                for (const id of ["notes", "tasks", "media", "terminal"]) {
                    ...
                    if (id === "notes") {
                        originals.notes.showContextMenu(Qt.point(20, 20));
                        wait(100);
                        const menu = findChild(originals.notes, "notesContextMenu");
                        verify(menu !== null);
                        compare(menu.hostItem, widget.popup.contentItem);
                        compare(menu.visible, true);
                        originals.notes.closeContextMenu();
                    }
                    if (id === "media") {
