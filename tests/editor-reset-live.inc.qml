    ...
    TestCase {
        ...
        function readSettings() { binguxSettings.read(); tryCompare(binguxSettings, "busy", false, 4000); }
        function save() { desktopCustomiser.apply(); tryCompare(desktopCustomiser, "visible", false, 4000); wait(350); }
        function test_reset() {
            try {
                ...
                editor.selectedWidget = "clock"; editor.widgetOption("label", "My clock");
                editor.selectContainer("sidebar"); editor.containerDisplay("both");
                editor.change("dockSize", 40); editor.change("dockAlignment", "left");
                editor.change("dockClick", "focus"); editor.change("dockMiddleClick", "none");
                editor.change("dockScroll", "none"); editor.change("dockScrollDirection", "reverse");
                editor.change("sidebarEdge", "left");
                editor.change("controlCentre", {vpn: false, dnd: false, nightLight: true, power: true, awake: true});
                ...
                verify(reset !== null); verify(reset.enabled, "Reset is enabled after loading defaults");
                click(reset, editor.nativeWindow); wait(350);
                ...
                compare(editor.desktop.dockSize, 56); compare(editor.desktop.dockAlignment, "center");
                compare(editor.desktop.dockClick, "toggle"); compare(editor.desktop.dockMiddleClick, "launch");
                compare(editor.desktop.dockScroll, "cycle"); compare(editor.desktop.dockScrollDirection, "natural");
                ...
                editor.undo(); wait(200); compare(editor.snapshot(), customised, "One Undo restores the whole customisation");
                compare(clockPill.parent, terminalSidebar.widgetHost);
                editor.redo(); wait(200); compare(clockPill.parent, centerControls);
                editor.cancel(); wait(350); readSettings();
                ...
                editor.optionsPage = ""; wait(200);
                click(reset, editor.nativeWindow); wait(300);
                if (Quickshell.env("BINGUX_RESET_CAPTURE")) click(reset, editor.nativeWindow, true);
                save(); readSettings();
