...
ShellRoot {
    ...
    QtObject {
        ...
        property var desktop: ({controlLayout: ControlLayout.defaults()})
        property var layout: ({sidebar: ["tray"]})
    }
    FloatingWindow {
        id: window
        implicitWidth: 900; implicitHeight: 760
        color: Theme.background
        Item { id: panel; width: 208; height: 720 }
        Pill {
            id: wrapper
            parent: panel
            horizontalPadding: 0
            Tray { id: tray; parentWindow: window; serviceEnabled: false }
        }
        WidgetPreview { id: preview; x: 420; width: 320; height: 240; widgetId: "tray" }
        TestCase {
            when: window.visible
            property var calls: []
            function buttons(item) {
                if (item.modelData?.id?.startsWith("sample-")) return [item];
            ...
            function init() { wrapper.panelLayout = false; }
            ...
            function test_wrapping_and_actions() {
                try {
                    tray.trayItems = Array.from({length: 18}, (_, index) => ({
                        id: "sample-" + index, title: "Application " + index,
                        tooltipTitle: "Application " + index, icon: Quickshell.iconPath("applications-other"),
                        menu: null, hasMenu: false, onlyMenu: false,
                    ...
                    wrapper.panelLayout = true;
                    if ("panelLayout" in tray) tray.panelLayout = true;
                    ...
                    for (const width of [208, 376, 208]) {
                        ...
                        for (const button of originals) {
                            tryVerify(() => {
                                ...
                                return p.x >= 0 && p.y >= 0 && p.x + button.width <= tray.width + .1
                                    && p.y + button.height <= tray.height + .1;
                    ...
                    last.forceActiveFocus(); keyClick(Qt.Key_Return);
                    compare(calls[calls.length - 1], "activate:17");
                    tray.presentation = {custom: true, showText: true, showIcon: true,
                        labelOverridden: true, label: "A very long application label that must fit this panel", iconOverridden: false};
                    tryVerify(() => findChild(last, "widgetFaceLabel").truncated);
                    verify(last.width <= tray.width);
                    tray.presentation = null;
                    tray.panelLayout = false; wrapper.panelLayout = false;
                    tryCompare(wrapper, "width", barWidth); tryCompare(wrapper, "height", barHeight);
                ...
                } catch (error) { console.error("TRAY_FIT_FAILED", error.message, error.stack, "tray", tray.width, tray.height, "host", panel.width); throw error; }
            }
        }
    }
}
