...
BINGUX_SETTINGS_REPORT="$test_dir/report" NO_AT_BRIDGE=1 PATH="$test_dir/bin:$PATH" XDG_CONFIG_HOME="$test_dir/config" timeout 25s dbus-run-session -- "${BINGUX_QUICKSHELL:-quickshell}" -p "$test_dir" --no-color >"$test_dir/log" 2>&1 || {
    cat "$test_dir/log"
    exit 1
}
cat "$test_dir/report"
grep 'BINGUX_SETTINGS_PASS' "$test_dir/report" || {
    cat "$test_dir/log"
    exit 1
}
if grep -E 'ReferenceError|TypeError|Binding loop|FAIL!' "$test_dir/log"; then
    cat "$test_dir/log"
    exit 1
fi
