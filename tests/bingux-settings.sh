...
cp "$repo_dir"/shell/bingux/{BinguxSettings,BinguxPreferences,Theme,ActionButton,SymbolicIcon,ControlSwitch,ControlRow,ControlCentreButtonSurface,MarqueeText,IconButton,ShellTooltip,TooltipBubble}.qml "$test_dir/"
...
sed -i -e '/import QtQuick$/a import QtQuick.Window' -e 's/^FloatingWindow {/Window {\n    flags: Qt.Window | Qt.WindowDoesNotAcceptFocus/' -e 's/^    implicitWidth:/    width:/' -e 's/^    implicitHeight:/    height:/' "$test_dir/BinguxSettings.qml"
