...
with tempfile.TemporaryDirectory() as directory:
    ...
    engines = [
        ...
        },
        {
            ...
            "name": "Documentation",
            "shortcut": "docs",
            "url": "https://docs.example.test/{query}",
            "enabled": True,
        },
    ]
    env = os.environ | {
        ...
        "OPENED_URL": str(base / "opened"),
    }
    for disabled in (False, True):
        engines[1]["enabled"] = not disabled
        (base / "config/bingux/settings.json").write_text(
            json.dumps(
                {
                    "search": {
                        "engines": engines,
                        "defaultEngine": "default",
                        "disabledProviders": ["applications", "files", "external"],
                        "fileRoots": [],
                    }
                }
            )
        ...
        daemon = subprocess.Popen(
            [str(binary), "--config", str(path)], env=env, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE
        )
        try:
            ...
            def request(kind, request_id, **fields):
                connection.sendall(
                    (json.dumps({"protocolVersion": 1, "type": kind, "requestId": request_id} | fields) + "\n").encode()
                )
            ...
            def query(text):
                ...
                while True:
                    ...
                    if event.get("requestId") == "query" and event.get("complete"):
