...
test("indexed notifications preserve exact matching, order and repeated entries", () => {
    ...
    const entries = [
        null,
        shared,
        { desktopEntry: "other", appName: "Spotify" },
        { desktopEntry: "spotify.desktop" },
        { appName: "Music Player" },
        shared,
        { appName: "window-alias" },
        { desktopEntry: "startup" },
        { appName: ".desktop" },
        { desktopEntry: ".desktop", appName: "Spotify" },
        {},
        { appName: "spotify-advert" },
    ];
    const groups = [
        null,
        {},
...
test("player icons resolve through the OS provider, including browser identities and packaged IDs", () => {
    ...
    assert.equal(
        api.playerDesktopEntry(
            { identity: "Helium", dbusName: "org.mpris.MediaPlayer2.chromium.instance42" },
            provider,
        ),
        helium,
    );
...
test("matches the desktop entry, startup class and window app IDs", () => {
    ...
    assert.equal(
        api.matches(
            { desktopEntry: "org.videolan.VLC" },
            { id: "vlc", desktopEntry: { id: "org.videolan.VLC.desktop" } },
        ),
        true,
    );
...
test("audio identity outranks generic runtime names and excludes other apps", () => {
    ...
    assert.equal(
        api.matchesAudio(
            { "application.process.binary": "/usr/bin/pikvm-desktop", "application.name": "Chromium" },
            { id: "pikvm-desktop" },
        ),
        true,
    );
    assert.equal(
        api.matchesAudio(
            { "application.process.binary": "pikvm-desktop", "application.name": "Chromium" },
            { id: "chromium" },
        ),
        false,
    );
    assert.equal(
        api.matchesAudio({ "application.name": "Some Browser" }, { desktopEntry: { name: "Some Browser" } }),
        true,
    );
