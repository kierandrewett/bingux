    ...
    TestCase {
        ...
        function test_external_widgets() {
            try {
                ...
                drag(searchPill, topBar, searchPill.width / 2, 16, DesktopEditing.point(header, controlCentre.nativeWindow, 1, 1));
                ...
                editor.undo();
                wait(100);
                verify(searchPill.parent === leftControls);
                editor.redo();
                wait(100);
                verify(searchPill.parent === controlCentre.widgetHost);
                editor.put("clock", "control-centre", 1);
                wait(100);
                verify(clockPill.parent === controlCentre.widgetHost);
                verify(clockPill.y > searchPill.y);
                editor.selectedContainer = "control-centre";
                editor.containerDisplay("text");
                verify(searchPill.presentation.showText && !searchPill.presentation.showIcon);
                editor.selectedWidget = "search";
                editor.widgetOption("label", "Find something");
                editor.widgetOption("display", "both");
                compare(searchPill.presentation.label, "Find something");
                verify(searchPill.presentation.showText && searchPill.presentation.showIcon);
                editor.put("label", "control-centre", 2);
                editor.put("label", "dock", 0);
                editor.put("label", "control-centre", 3);
                ...
                editor.widgetOption("label", "My controls");
                wait(100);
                const initialLabel = topBar.decorationWidgets.find(item => item.widgetId === "label:1");
                editor.put("icon", "control-centre", 2);
                wait(150);
                ...
                for (const id of ["capture", "tray", "privacy", "metrics", "keyboard", "overflow", "controls", "notifications"]) {
                    ...
                    editor.put(id, "control-centre", 0);
                    wait(80);
                    ...
                    if (id === "privacy")
                        compare(privacyContainer.appearance("Camera in use", "camera-web-symbolic").mode, "text", "Privacy indicators inherit the control-centre display mode");
                }
                ...
                for (const container of ["dock", "control-centre"]) {
                    editor.open();
                    editor.put("notifications", container, 0);
                    save();
                ...
                editor.open();
                editor.put("notifications", "top-right", 7);
                save();
                ...
                privacyContainer.systemMetrics = {
                    screenSharing: false,
                    locationInUse: true,
                ...
                editor.open();
                editor.put("privacy", "control-centre", 0);
                editor.selectedContainer = "control-centre";
                editor.containerDisplay("text");
                save();
                controlCentre.visible = true;
                ...
                editor.open();
                editor.put("privacy", "top-right", 2);
                editor.change("containers", containerOptions);
                save();
                editor.open();
                editor.put("controls", "control-centre", 0);
                save();
                controlCentre.visible = true;
                tryCompare(controlCentre, "revealScale", 1, 3000);
                wait(150);
                ...
                editor.open();
                editor.put("controls", "top-right", 6);
                editor.put("search", "control-centre", 0);
                editor.put("clock", "control-centre", 1);
                editor.put("label", "control-centre", 2);
                editor.put("icon", "control-centre", 3);
                editor.selectedWidget = "label:1";
                editor.widgetOption("label", "My controls");
                save();
                controlCentre.visible = true;
                ...
                verify(widgetMenu.anchorWindow === controlCentre.nativeWindow);
                ...
                verify(calendarPopup.anchorWindow === controlCentre.nativeWindow);
                ...
                tryVerify(() => agenda.mapToItem(calendarPopup.body, 0, agenda.height).y <= calendarPopup.body.height + 0.01, 1000, "The complete agenda remains inside the moved calendar popup");
                ...
                tryCompare(searchOverlay, "visible", true, 4000);
