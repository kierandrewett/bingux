...
ShellRoot {
    FileView {
        id: report
        path: Quickshell.env("BINGUX_SETTINGS_REPORT")
    }
    BinguxSettings {
        id: settings
        visible: true
    ...
    TestCase {
        ...
        function screenshot(page) {
            ...
            wait(250);
            captured = false;
            settings.contentItem.grabToImage(result => {
                captured = result.saveToFile('/tmp/bingux-settings-' + page.toLowerCase() + (settings.wideLayout ? '' : '-narrow') + '.png');
            });
        ...
        function test_settings() {
            try {
                ...
                for (const page of ['Search', 'AI', 'Previews', 'Desktop', 'TopBar', 'Controls', 'Sidebar', 'Dock', 'Providers', 'Engines'])
                    screenshot(page);
                ...
                globalFilter.text = 'no-such-preference';
                compare(settings.searchMatches.length, 0);
                globalFilter.clear();
                mouseClick(findChild(settings.contentItem, 'settingsSearchToggle'));
                verify(!settings.searchOpen);
