...
try:
 ...
 target=found[-1]; old=target['index']; node=target['properties']['object.id']
 subprocess.run(['pw-cli','destroy',node],check=True,capture_output=True)
 for _ in range(40):
  live=streams()
  if any(s['index']!=old and s['properties'].get('target.object')==target['properties'].get('target.object') for s in live):
   print('PASS monitor recovered after server-side disconnection');break
  time.sleep(.1)
 else: raise AssertionError('monitor did not reconnect')
finally:
