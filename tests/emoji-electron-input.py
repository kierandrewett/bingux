...
def drive(mode, pid, query):
    ...
    with socket.socket(socket.AF_UNIX) as bridge:
        ...
        before = (
            anchor()
            if X11
            else wait_for(lambda: r if (r := anchor()) and r.get("caret") else None, "No native Electron caret")
        )
        if not X11:
        ...
        def key(symbol, down):
            bus.call_sync(
                "org.gnoblin.EmojiTest",
                "/org/gnoblin/EmojiTest",
                "org.gnoblin.EmojiTest",
                "Key",
                GLib.Variant("(ub)", (symbol, down)),
                None,
        ...
        try:
            ...
            time.sleep(1.1 if X11 else 0.5)
...
""".replace("CONFIG_URL", (gnoblin / "src/gnome-shell-overlay/js/ui/components/gnoblinConfig.js").as_uri()).replace(
    ...
    with (CONFIG / "emoji-shell.log").open("w") as log:
        ...
        try:
            ...
            subprocess.run(
                [
                    os.environ["BINGUX_TEST_ELECTRON"],
                    "--ozone-platform=" + ("x11" if X11 else "wayland"),
                    str(ROOT / "tests/emoji-electron-input.cjs"),
                ],
                check=True,
                timeout=45,
            )
    ...
    subprocess.run(
        ["bash", str(ROOT / "tests/control-centre.sh")],
        check=True,
        timeout=35,
