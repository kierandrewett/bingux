...
ShellRoot {
    ...
    PanelWindow {
        ...
        Item { id: panelHost; width: 208; height: 500 }
        ...
        TestCase {
            ...
            function test_panel_fit_data() {
                return ["sidebar", "control-centre"].map(container => ({tag: container, container}));
            }
            function test_panel_fit(data) {
                indicators.parent = panelHost;
                indicators.x = 0; indicators.y = 0;
                editor.layout = data.container === "sidebar" ? {sidebar: ["privacy"]} : {};
                editor.desktop = {controlLayout: ControlLayout.move(ControlLayout.defaults(), "control-centre", "privacy", 0), containers: {[data.container]: {display: "both"}}};
                try {
                    ...
                    for (const label of ["", "A deliberately long privacy indicator label to check clipping"]) {
                        editor.desktop = Object.assign({}, editor.desktop, {widgetOptions: label ? {privacy: {label}} : {}});
                        ...
                        for (const child of children) {
                            tryVerify(() => {
                                ...
                                return point.x >= 0 && point.x + child.width <= indicators.width + 1
                                    && point.y + child.height <= indicators.height + 1;
                            ...
                            if (label) verify(text.truncated, "A long custom label ends with an ellipsis");
                        }
                        ...
                        if (capture && label) {
                            let saved = false;
                            verify(indicators.grabToImage(result => { saved = result.saveToFile(capture + "-" + data.container + ".png"); }));
                            tryVerify(() => saved, 2000);
                        }
                    }
                } catch (error) { console.error("PRIVACY_FIT_FAILED", error.message, error.stack); throw error; } finally { indicators.parent = panel.contentItem; indicators.x = 30; indicators.y = 50; }
            }
