...
parser.add_argument('--case', choices=('desktop-layout', 'control-layout', 'control-audio', 'control-media', 'control-groups', 'dock-unpin'), default='desktop-layout')
...
with tempfile.TemporaryDirectory(prefix='bingux-layout-live-') as directory:
    ...
    if case in ('control-layout', 'control-groups'): shell = 'import "ControlLayout.js" as ControlLayout\n' + shell
    base_shell = shell
    ...
    if case in ('control-layout', 'control-groups'):
    ...
    if case == 'control-groups':
        (fixture / 'shell.qml').write_text(base_shell.rstrip()[:-1] + (repo / 'tests/control-groups-reload.inc.qml').read_text() + '\n}\n')
        ...
        print('PASS: saved group placements, child membership and display overrides survive a fresh shell process')
