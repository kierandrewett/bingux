...
ShellRoot {
    ...
    TestCase {
        ...
        function test_close_multiple() {
            ...
            function created() {
                return ToplevelManager.toplevels.values.filter(w => !original.includes(w) && w.appId.includes("Nautilus"));
            }
            ...
            for (let i = 0; i < dock.testItems.count; i++) {
                ...
                if (item.currentGroup.windows.includes(created()[0]))
                    button = item;
            }
            verify(button !== null);
            ...
            for (let remaining = 2; remaining > 0; remaining--) {
                ...
                if (remaining > 1)
                    verify(button.menuOpen, "Menu remains open after closing a window");
                wait(400);
            }
        ...
        function cleanupTestCase() {
            for (const window of ToplevelManager.toplevels.values)
                if (!original.includes(window) && window.appId.includes("Nautilus"))
                    window.close();
            finish.start();
        }
    }
