...
config.write_text(Path(__file__).with_suffix(".qml").read_text().replace("../shell/bingux", (repo / "shell/bingux").as_uri()))
...
result = subprocess.run([os.environ.get("QS_TEST_BIN", "qs"), "-p", str(config)], env=env, text=True, capture_output=True, timeout=40)
...
assert changes == [["--wait", "10", "connection", action, "uuid", identity] for action, identity in [
