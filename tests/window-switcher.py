...
def state():
    ...
    output = subprocess.run([qs, 'ipc', '-p', str(qml), 'call', 'switcher', 'status'],
...
try:
    ...
    tap(1)
    key(56, False)
    print('PASS: real Alt+Tab, stable MRU, reverse, Escape, outside click, rapid switching, closed windows, script/config reload, Quickshell restart and invalid config recovery')
finally:
    shell.terminate()
    shell.wait(timeout=5)
    qslog.close()
