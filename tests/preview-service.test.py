...
import preview_service as service
from preview_limits import MAX_FILE_BYTES
...
class PreviewServiceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_real_service_prepares_and_serves_pages_over_its_pipe(self):
        ...
        context.paint()
        surface.finish()
        process = await asyncio.create_subprocess_exec(sys.executable, str(SHELL / 'preview-document.py'), 'serve',
            stdin=asyncio.subprocess.PIPE, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE,
            limit=16 * 1024 * 1024, env=dict(os.environ, XDG_CACHE_HOME=str(self.directory / 'cache')))
        try:
            ...
            for mode, options in [('info', []), ('page', ['0', '932'])]:
                process.stdin.write((json.dumps({'op': 'request', 'id': mode, 'args': [mode, str(path), *options]}) + '\n').encode())
                record = json.loads(await asyncio.wait_for(process.stdout.readline(), 5))
