...
class ApplicationLaunch(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for identity in ("org.test.desktop", "org.test.desktop.desktop"):
                ...
                result = subprocess.run(
                    [sys.executable, str(launcher), identity],
    ...
    def test_host_manager_timeout_is_reported(self):
        ...
        spec.loader.exec_module(module)
        with (
            patch.object(module.subprocess, "run", side_effect=subprocess.TimeoutExpired("systemd-run", 5)),
    ...
    def test_dock_launch_falls_back_when_host_manager_rejects_unit(self):
        ...
        with (
            patch.object(
                module.subprocess, "run", return_value=subprocess.CompletedProcess([], 1, "No space left on device", "")
            ),
    ...
    def test_host_feedback_reports_missing_entry(self):
        ...
        result = subprocess.run(
            [sys.executable, str(launcher), "--dock-feedback", "bingux-no-such-test-app-984a5"],
            capture_output=True,
            text=True,
            timeout=10,
        )
    ...
    def test_early_process_failure_is_reported(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = subprocess.run(
                [sys.executable, str(launcher), "bingux-failure"],
                env=os.environ | {"XDG_DATA_HOME": temporary, "XDG_DATA_DIRS": temporary},
                capture_output=True,
                text=True,
                timeout=6,
    ...
    def test_blocked_gio_activation_is_reported_and_does_not_stick(self):
        ...
        module.LAUNCH_ACCEPT_TIMEOUT = 0.05
        with (
            patch.object(module.GioUnix.DesktopAppInfo, "new", return_value=entry),
            patch("sys.stderr", new_callable=io.StringIO) as stderr,
        ):
    ...
    def test_missing_executable_and_crash_are_reported(self):
        for command, expected in (
            ("/bingux-no-such-executable", "executable is unavailable"),
            ...
            with self.subTest(command=command), tempfile.TemporaryDirectory() as temporary:
                ...
                result = subprocess.run(
                    [sys.executable, str(launcher), "bingux-broken"],
                    env=os.environ | {"XDG_DATA_HOME": temporary, "XDG_DATA_DIRS": temporary},
                    capture_output=True,
                    text=True,
                    timeout=6,
    ...
    def test_desktop_action_and_field_codes(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for flags, expected in (([], "normal"), (["--new-window"], "new-window")):
                record.unlink(missing_ok=True)
                result = subprocess.run(
                    [sys.executable, str(launcher), *flags, "bingux-launch-test"],
                    env=env,
                    capture_output=True,
                    text=True,
                )
