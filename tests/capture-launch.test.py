...
spec = importlib.util.spec_from_file_location("launch", Path(__file__).resolve().parents[1] / "shell/bingux/capture-launch.py")
...
class CaptureLaunch(unittest.TestCase):
    ...
    def test_timeout_is_not_retried(self):
        ...
        with self.assertRaises(subprocess.TimeoutExpired): module.launch(["qs"], run, Mock())
        self.assertEqual(run.call_count, 1)
