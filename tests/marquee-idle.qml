...
ShellRoot {
    FileView { id: report; path: Quickshell.env("BINGUX_NOTES_TEST_RESULTS"); blockWrites: true }
    Timer { id: finish; interval: 100; onTriggered: Qt.quit() }
    Window {
        id: window
        visible: true
        width: 160; height: 60
        MarqueeText { id: marquee; width: 80; text: "A long title that must scroll when visible"; active: true }
        TestCase {
            when: window.visible
