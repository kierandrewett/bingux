    ...
    TestCase {
        ...
        function test_sidebar_controls() {
            ...
            try {
                ...
                terminalSidebar.selectContent("notes"); terminalSidebar.open(); wait(300);
                ...
                output.node = testOutput; input.node = testInput; media.player = firstPlayer;
                ...
                drag(grip, controlCentre.nativeWindow, grip.width / 2, grip.height / 2,
                    Qt.point(sidebar.screenRect.x + 40, sidebar.screenRect.y + Theme.barHeight + 12));
                compare(audio.parent, terminalSidebar.widgetHost); verify(!audio.barLayout && !output.barLayout);
                compare(audio.height, nativeAudioHeight); compare(output.parent, audio); compare(input.parent, audio);
                editor.undo(); wait(100); verify(audio.parent !== terminalSidebar.widgetHost);
                editor.redo(); wait(100); compare(audio.parent, terminalSidebar.widgetHost);
                for (const item of controlCentre.movableWidgets) {
                    editor.put(item.widgetId, "sidebar", 0); wait(30);
                ...
                compare(output.parent, audio); compare(settings.parent, header); compare(network.parent, quick);
                ...
                viewport.cancelFlick(); viewport.contentY = 0; wait(100);
                ...
                terminalSidebar.widgetViewport.contentY = 64; wait(100);
                ...
                terminalSidebar.widgetViewport.contentY = 0; wait(100);
                editor.selectedContainer = "sidebar"; editor.containerDisplay("both"); wait(100);
                ...
                editor.selectedWidget = "control-settings"; editor.widgetOption("display", "both"); editor.widgetOption("label", "Prefs");
                save();
                ...
                controlCentre.visible = false; wait(200);
                compare(settings.presentation.label, "Prefs"); verify(settings.presentation.showText && settings.presentation.showIcon);
                ...
                tryCompare(widgetMenu, "visible", true, 3000); compare(widgetMenu.anchorWindow, terminalSidebar.editWindow); widgetMenu.visible = false;
                ...
                tryCompare(controlCentre, "visible", true, 3000); compare(controlCentre.detailPage, "audio");
                compare(controlCentre.movedAnchor, output); compare(controlCentre.anchorWindow, terminalSidebar.editWindow);
                controlCentre.visible = false; wait(200);
                ...
                tryCompare(controlCentre, "visible", true, 3000); compare(controlCentre.detailPage, "network");
                compare(controlCentre.movedAnchor, network); controlCentre.visible = false; wait(200);
                revealWidget(play);
                ...
                terminalSidebar.popOut(); tryCompare(terminalSidebar.detachedSurface, "visible", true, 3000); wait(200);
                revealWidget(output);
