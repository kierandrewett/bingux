...
class ExtendedPreviewTests(unittest.TestCase):
    ...
    def test_database_pagination_quotes_and_read_only(self):
        ...
        connection.executemany(
            ...
            [(index, "Row " + str(index), b"\x00\x01") for index in range(125)],
        )
        connection.commit()
        connection.close()
        original = path.read_bytes()
    ...
    def test_formatted_markdown_and_html_do_not_execute_or_fetch(self):
        for name, text in [
            ...
            (
                "page.html",
                '<h1>Heading</h1><script>alert(1)</script><img src="https://example.com/image.png"><p>Content</p>',
            ),
        ]:
            path = self.path / name
    ...
    def test_gif_frames_and_video_metadata(self):
        ...
        subprocess.run(
            ["ffmpeg", "-v", "error", "-f", "lavfi", "-i", "color=c=red:s=160x90:d=1", "-c:v", "mpeg4", str(path)],
            check=True,
        )
        result = preview.metadata(path)
